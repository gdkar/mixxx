...
DetectionFunction::DetectionFunction( DFConfig Config ) 
    ...
    , m_phaseHistory(0)
    , m_phaseHistoryOld(0)
    , m_magPeaks(0){initialise( Config );}
DetectionFunction::~DetectionFunction(){deInitialise();}
...
double DetectionFunction::HFC(unsigned int length, double *src){
    unsigned int i;
    double val = 0;
    for( i = 0; i < length; i++){val += src[ i ] * ( i + 1);}
    return val;
}
...
double* DetectionFunction::getSpectrumMagnitude(){return m_magnitude;}
