...
GetKeyMode::GetKeyMode( int sampleRate, float tuningFrequency, float hpcpAverage, float medianAverage ) :
    ...
    m_keyStrengths(0)
{
    ...
    m_ChromaConfig.FS        = lrint(sampleRate/(float)m_DecimationFactor);
    ...
    // Chromagram inst.
    m_Chroma = std::make_unique<Chromagram>( m_ChromaConfig );
...
//    std::cerr << "chroma frame size = " << m_ChromaFrameSize << ", decimation factor = " << m_DecimationFactor << " therefore block size = " << getBlockSize() << std::endl;
    ...
    m_ChromaBuffer = new float[m_BPO * m_ChromaBuffersize];
    std::fill ( &m_ChromaBuffer[0],&m_ChromaBuffer[m_BPO * m_ChromaBuffersize], 0 );
    
    m_MeanHPCP = new float[m_BPO];
    
    m_MajCorr = new float[m_BPO];
    m_MinCorr = new float[m_BPO];
    m_Keys  = new float[2*m_BPO];
    
    m_MedianFilterBuffer = new int[ m_MedianWinsize ];
    std::fill ( &m_MedianFilterBuffer[0],&m_MedianFilterBuffer[m_MedianWinsize],0 );
    
    m_SortedBuffer = new int[ m_MedianWinsize ];
    std::fill ( &m_SortedBuffer[0],&m_SortedBuffer[m_MedianWinsize],0);
    
    m_Decimator = std::make_unique< Decimator>
...
float GetKeyMode::krumCorr(float *pData1, float *pData2, size_t length)
{
    float retVal= 0.0;
    
    float num = 0;
    float den = 0;
    float mX = MathUtilities::mean( pData1, length );
    float mY = MathUtilities::mean( pData2, length );
...
*/
    ...
    for( k = 0; k < m_BPO; k++ )
    {
        ...
        m_MeanHPCP[k] = mnVal/m_ChromaBufferFilling;
    }
    for( k = 0; k < m_BPO; k++ )
    {
...
*/
    ...
    key = 1 + (int)ceil( (float)MathUtilities::getMax( m_Keys, 2* m_BPO, &dummy )/3 );
//    std::cout << "key pre-sorting: " << key << std::endl;
    ...
    std::sort ( &m_SortedBuffer[0],&m_SortedBuffer[m_MedianBufferFilling]);
/*
  std::cout << "sorted: ";
...
*/
    ...
    auto midpoint = static_cast<off_t>(ceil(static_cast<float>(sortlength)/2));
...
bool GetKeyMode::isModeMinor( int key ) { return (key > 12);}
