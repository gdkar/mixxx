...
public:
	ChromaVector(size_t uSize = 12) : std::valarray<double>(){ resize(uSize, 0.0f); }
...
public:
	TCSVector() : std::valarray<double>(){ resize(6, 0.0f); }
	virtual ~TCSVector() {};
	void printDebug(){
		for (size_t i = 0; i < size(); i++)
			std::cout <<  (*this)[i] << ";";
	...
	double magnitude() const{
		double dMag = 0.0;
		for (size_t i = 0; i < 6; i++)dMag += std::pow((*this)[i], 2.0);
