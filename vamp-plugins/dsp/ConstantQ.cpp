...
//        std::cerr << "using precalculated kernel" << std::endl;
	...
	const unsigned hammingLength = (int) ceil( m_dQ * m_FS / ( m_FMin * pow(2,((float)(k))/(float)m_BPO)));

        unsigned origin = m_FFTLength/2 - hammingLength/2;

	for (unsigned i=0; i<hammingLength; i++) 
	{
	    const float angle = 2*PI*m_dQ*i/hammingLength;
...
void ConstantQ::initialise( CQConfig Config )
{
    ...
    m_dQ = 1/(pow(2,(1/(float)m_BPO))-1);	// Work out Q value for Filter bank
                        ...
                        float *CQRe, float *CQIm)
{
	...
	const float & r1  = real[i];
	const float & i1  = imag[i];
	const float & r2  = FFTRe[ m_FFTLength - col - 1 ];
	const float & i2  = FFTIm[ m_FFTLength - col - 1 ];
	// add the multiplication
