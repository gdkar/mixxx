...
//    std::cerr << "chroma frame size = " << m_ChromaFrameSize << ", decimation factor = " << m_DecimationFactor << " therefore block size = " << getBlockSize() << std::endl;
    ...
    memset( m_ChromaBuffer, 0, sizeof(float) * m_BPO * m_ChromaBuffersize);
...
float GetKeyMode::krumCorr(float *pData1, float *pData2, unsigned int length)
{
    float retVal= 0.0;
...
*/
    ...
    key = 1 + (int)(std::distance(m_Keys,std::max_element(m_Keys, m_Keys + 2* m_BPO))/3 );
