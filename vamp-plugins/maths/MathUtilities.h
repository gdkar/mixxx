...
namespace MathUtilities  
{
    ...
    template<class T>
    using ComplexData = typename std::complex<T>;

    template<class T>
    constexpr std::enable_if_t<std::is_floating_point<T>::value,T> M_PI_t = static_cast<T>(M_PI);

    template<class T>
    constexpr std::enable_if_t<std::is_floating_point<T>::value,T> EPS_t = std::numeric_limits<T>::epsilon();
    /**
