...
void main(void) {
    ...
    if (distanceToRigthSignal.x < 0.0 && distanceToLeftSignal.x < 0.0) outputColor += lowColor;
    if( distanceToRigthSignal.y < 0.0 && distanceToLeftSignal.y < 0.0) outputColor += midColor;
    if( distanceToRigthSignal.z < 0.0 && distanceToLeftSignal.z < 0.0) outputColor += highColor;
    */
    ...
    for( float i = firstPixelPosition; i < lastPixelPosition; i += 2.0) {
        ...
        if( uv.y > 0.5) {
            ...
            if( currentData.x > uv.y-0.5) //low   accumulatedData.x += pixelWeigth;
            if( currentData.y > uv.y-0.5) //Mid   accumulatedData.y += pixelWeigth;
            if( currentData.z > uv.y-0.5) //High  accumulatedData.z += pixelWeigth;
        }
        else {
    ...
    if( accumulatedData.x > 0) outputColorColor = mix( outputColorColor, low, clamp(accumulatedData.x,0.1f,0.9f));
    if( accumulatedData.y > 0) outputColorColor = mix( outputColorColor, mid, clamp(accumulatedData.y,0.1f,0.9f));
    if( accumulatedData.z > 0) outputColorColor = mix( outputColorColor, high, clamp(accumulatedData.z,0.1f,0.9f));
