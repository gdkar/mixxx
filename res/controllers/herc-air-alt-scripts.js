...
HerculesAir.shift = false
HerculesAir.meta  = false
...
HerculesAir.rateRelative = {
    '[Channel1]': new FakeRelative(
        '[Channel1]'
        ,'rate'
        ,function() { return !HerculesAir.shift; }
...
HerculesAir.shiftButton = function(midino, control, value, status, group) {
	HerculesAir.shift = (value == 0x7f);
    midi.sendShortMsg(status, control, value);
}
HerculesAir.metaButton = function(midino, control, value, status, group) {
	HerculesAir.meta = (value == 0x7f);
    midi.sendShortMsg(status, control, value);
}
...
HerculesAir.hotcue = function(midino, control, value, status, group) {
    ...
    var hotcue_string = "hotcue_" + cue + "_"
	if(value != 0x00) {
        var enabled = engine.getValue(group, hotcue_string + "enabled")
        if(HerculesAir.meta && HerculesAir.shift) {
            engine.setValue(group, hotcue_string+"clear", 1)
        } else if(!HerculesAir.meta && HerculesAir.shift) {
            engine.setValue(group, hotcue_string+"set", 1)
        } else if(!HerculesAir.shift && HerculesAir.meta) {
            if(enabled)
                engine.setValue(group, hotcue_string+"goto", 1)
		} else {
            if(enabled)
                engine.setValue(group, hotcue_string+"goto", 1)
            else
