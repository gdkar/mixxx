...
def method_call(method_name, args):
    return '%(method_name)s(%(args)s)' % {'method_name': method_name,'args': ', '.join(args)}
...
def write_channelmixer_autogen(output, num_channels):
    ...
    def write_mixchannels(ramping, output):
        if ramping: header = 'void ChannelMixer::mixChannelsRamping('
        else: header = 'void ChannelMixer::mixChannels('
        ...
        def write(data, depth=0): output.append(' ' * (BASIC_INDENT * depth) + data)
        write('int totalActive = activeChannels->size();', depth=1)
        write('if (totalActive == 0) {', depth=1)
        write('ScopedTimer t("EngineMaster::mixChannels%(variant)s_0active");' % {'variant': 'Ramping' if ramping else ''}, depth=2)
        ...
        for i in xrange(1, num_channels+1):
            ...
            if ramping: write('CSAMPLE_GAIN oldGain[%(i)d];' % {'i': i}, depth=2)
            ...
            for j in xrange(i):
                ...
                if ramping: write('oldGain[%(j)d] = gainCache%(j)d.m_gain;' % {'j': j}, depth=2)
...
def copy_with_ramping_gain(output, base_indent_depth, num_channels):
    ...
    arg_groups = ['CSAMPLE* _RESTRICT pDest'] + [
        ...
        for i in xrange(num_channels)] + ['int iNumSamples']
    output.extend(hanging_indent(header, arg_groups, ',', ') {',depth=base_indent_depth))
    for i in xrange(num_channels):
        ...
        if (num_channels > 1):
            ...
            write('%s;' % method_call(copy_with_ramping_gain_method_name(num_channels - 1), args),depth=2)
        else:
           write('clear(pDest, iNumSamples);', depth=2)
        write('return;', depth=2)
        write('}', depth=1)
...
def main(args):
    ...
    output = (open(args.sampleutil_autogen_h, 'w') if args.sampleutil_autogen_h else sys.stdout)
    ...
    output = (open(args.channelmixer_autogen_cpp, 'w') if args.channelmixer_autogen_cpp else sys.stdout)
