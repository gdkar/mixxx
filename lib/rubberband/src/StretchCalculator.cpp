...
StretchCalculator::setKeyFrameMap(const std::map<size_t, size_t> &mapping)
{
    ...
    if (!m_keyFrameMap.empty()) {
        if (m_keyFrameMap.find(0) == m_keyFrameMap.end()) {m_keyFrameMap[0] = 0;}
    }
}
...
StretchCalculator::calculate(double ratio, size_t inputDuration,
    ...
    Profiler profiler("RubberBandStretcher::StretchCalculator::calculate");
    ...
    if (m_debugLevel > 0) {std::cerr << "StretchCalculator::calculate(): inputDuration " << inputDuration << ", ratio " << ratio << ", outputDuration " << outputDuration;}
...
//            std::cerr << "note: i (=" << i << ") == peaks.size(); regionEndChunk " << regionEndChunk << " -> " << totalCount << ", regionEnd " << regionEnd << " -> " << outputDuration << std::endl;
        ...
        dfRegion.reserve(regionEndChunk-regionStartChunk);
        for (size_t j = regionStartChunk; j != regionEndChunk; ++j) {dfRegion.push_back(stretchDf[j]);}
        if (m_debugLevel > 1) {std::cerr << "distributeRegion from " << regionStartChunk << " to " << regionEndChunk << " (samples " << regionStart << " to " << regionEnd << ")" << std::endl;}
        ...
        for (size_t j = 0; j < regionIncrements.size(); ++j) {
            const int incr = regionIncrements[j];
...
StretchCalculator::calculateSingle(double ratio,
                                   float df,
                                   size_t increment){
    Profiler profiler("RubberBandStretcher::StretchCalculator::calculateSingle");
...
StretchCalculator::findPeaks(const std::vector<float> &rawDf){
    Profiler profiler("RubberBandStretcher::StretchCalculator::findPeaks");
...
//            std::cerr << "have " << mediansize << " in median buffer" << std::endl;
        ...
        std::cerr << "median calculation on an array of size "<<sorted.size()<<std::endl;
//        if (m_debugLevel > 2) {
...
//            std::cerr << "i = " << i << ", maxindex = " << maxindex << ", middle = " << middle << ", so peak at " << peak << std::endl;
    ...
    while (!hardPeakCandidates.empty() || !softPeakCandidates.empty()) {
        ...
        peak.chunk = softPeak;
        bool ignore = false;
        if (haveHardPeak && (!haveSoftPeak || hardPeak <= softPeak)) {
            ...
            if (!peaks.empty() &&
                ...
                if (m_debugLevel > 2) {std::cerr << "(ignoring, as we just had a hard peak)"<< std::endl;}
                ignore = true;
            }
        }            
        if (haveSoftPeak && peak.chunk == softPeak) {softPeakCandidates.erase(softPeakCandidates.begin());}
...
StretchCalculator::smoothDF(const std::vector<float> &df){
    ...
    Profiler profiler("RubberBandStretcher::StretchCalculator::smoothDF");
...
StretchCalculator::distributeRegion(const std::vector<float> &dfIn,
    ...
    Profiler profiler("RubberBandStretcher::StretchCalculator::distributeRegions");
    ...
    for (size_t i = 1; i < df.size()/2; ++i) {
        if (df[i] < df[i-1]) {
            if (m_debugLevel > 1) {std::cerr << "stretch peak offset: " << i-1 << " (peak " << df[i-1] << ")" << std::endl;}
...
//            std::cerr << "WARNING: totalDisplacement == 0 (duration " << duration << ", " << df.size() << " values in df)" << std::endl;
        ...
        if (extremeIncrement < 0) {
            if (m_debugLevel > 0) {std::cerr << "NOTE: extreme increment " << extremeIncrement << " < 0, adjusting" << std::endl;}
            tooShort = true;
        } else {
            if (ratio < 1.0) {
...
StretchCalculator::calculateDisplacements(const std::vector<float> &df,
                                          ...
                                          float adj) const
{
    Profiler profiler("RubberBandStretcher::StretchCalculator::calculateDisplacements");
    ...
    for (size_t i = 0; i < df.size(); ++i) {
        ...
        totalDisplacement += displacement;
        if (i == 0 || displacement > maxDisplacement) {maxDisplacement = displacement;}
    }
}
}
