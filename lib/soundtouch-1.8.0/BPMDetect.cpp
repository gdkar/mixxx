...
int BPMDetect::decimate(CSAMPLE *dest, const CSAMPLE *src, int numsamples)
{
    ...
    for (count = 0; count < numsamples; count ++) 
    {
        ...
        for (j = 0; j < channels; j ++){decimateSum += src[j];}
        src += j;
        decimateCount ++;
        if (decimateCount >= decimateBy) {
            // Store every Nth sample only
            out = (CSAMPLE)(decimateSum / (decimateBy * channels));
            decimateSum = 0;
            decimateCount = 0;
...
void BPMDetect::inputSamples(const CSAMPLE *samples, int size){
    ...
    if ((int)buffer->size() > windowLen) {
        int processLength;
        // how many samples are processed
        processLength = (int)buffer->size() - windowLen;
