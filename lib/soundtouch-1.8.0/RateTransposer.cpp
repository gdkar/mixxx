...
void RateTransposer::putSamples(const CSAMPLE *samples, uint nSamples){processSamples(samples, nSamples);}
...
bool RateTransposer::empty() const
{
    if(!FIFOProcessor::empty())return false;
...
{
    int numSrcSamples = src.size();
    int sizeDemand = (int)((float)numSrcSamples / rate) + 8;
    int numOutput;
    CSAMPLE *psrc = src.begin();
    CSAMPLE *pdest = dest.end(sizeDemand);
...
void TransposerBase::setRate(float newRate){rate = newRate;}
