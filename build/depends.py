...
class Qt(Dependence):
    ...
    def configure(self, build, conf):
        ...
        build.flags['qt_sqlite_plugin'] = util.get_flags(build.env, 'qt_sqlite_plugin', 0)
        # Enable Qt include paths
        if build.platform_is_linux:
            if  not conf.CheckForPKG('Qt5Core', '5.0'): raise Exception('Qt >= 5.0 not found')
            ...
            include_paths = ['$QTDIR/include/%s' % module for module in qt_modules]
...
class FidLib(Dependence):
    def sources(self, build):
        symbol = None
        if build.platform_is_windows:
            if build.toolchain_is_msvs:  symbol = 'T_MSVC'
            elif build.crosscompile:     symbol = 'T_LINUX'
            elif build.toolchain_is_gnu: symbol = 'T_MINGW'
        else:                            symbol = 'T_LINUX'
        return [build.env.StaticObject('#lib/fidlib-0.9.10/fidlib.c',CPPDEFINES=symbol)]
    def configure(self, build, conf):build.env.Append(CPPPATH='#lib/fidlib-0.9.10/')
class ReplayGain(Dependence):
    def sources(self, build): return ["#lib/replaygain/replaygain.cpp"]
    def configure(self, build, conf): build.env.Append(CPPPATH="#lib/replaygain")
class SoundTouch(Dependence):
    ...
    def configure(self, build, conf, env=None):
        ...
        optimize = (build.flags['optimize'] if 'optimize' in build.flags else Optimize.get_optimization_level())
        if optimize == Optimize.LEVEL_OFF: env.Append(CPPDEFINES='SOUNDTOUCH_DISABLE_X86_OPTIMIZATIONS')
...
class MixxxCore(Feature):
    ...
    def sources(self, build):
        sources = ["mixxxkeyboard.cpp",
                   ...
                   "preferences/dlgpreferences.cpp",
                   "preferences/dlgprefsound.cpp",
                   "preferences/dlgprefsounditem.cpp",
                   "controllers/dlgprefcontroller.cpp",
                   "controllers/dlgcontrollerlearning.cpp",
                   "controllers/dlgprefcontrollers.cpp",
                   "preferences/dlgpreflibrary.cpp",
                   "preferences/dlgprefcontrols.cpp",
                   "preferences/dlgprefwaveform.cpp",
                   "preferences/dlgprefautodj.cpp",
                   "preferences/dlgprefkey.cpp",
                   "preferences/dlgprefreplaygain.cpp",
                   "preferences/dlgprefnovinyl.cpp",
                   "preferences/dlgabout.cpp",
                   "preferences/dlgprefeq.cpp",
                   "preferences/dlgprefeffects.cpp",
                   "preferences/dlgprefcrossfader.cpp",
                   "preferences/dlgtagfetcher.cpp",
                   "preferences/dlgtrackinfo.cpp",
                   "preferences/dlganalysis.cpp",
                   "preferences/dlgautodj.cpp",
                   "preferences/dlghidden.cpp",
                   "preferences/dlgmissing.cpp",
                   "preferences/dlgdevelopertools.cpp",
                   "preferences/dlgcoverartfullsize.cpp",
        ...
        ui_files = [
            ...
            'preferences/dlgaboutdlg.ui',
            'preferences/dlganalysis.ui',
            'preferences/dlgautodj.ui',
            'preferences/dlgcoverartfullsize.ui',
            'preferences/dlgdevelopertoolsdlg.ui',
            'preferences/dlghidden.ui',
            'preferences/dlgmissing.ui',
            'preferences/dlgprefbeatsdlg.ui',
            'preferences/dlgprefcontrolsdlg.ui',
            'preferences/dlgprefwaveformdlg.ui',
            'preferences/dlgprefautodjdlg.ui',
            'preferences/dlgprefcrossfaderdlg.ui',
            'preferences/dlgprefkeydlg.ui',
            'preferences/dlgprefeqdlg.ui',
            'preferences/dlgprefeffectsdlg.ui',
            'preferences/dlgpreferencesdlg.ui',
            'preferences/dlgprefnovinyldlg.ui',
            'preferences/dlgpreflibrarydlg.ui',
            'preferences/dlgprefrecorddlg.ui',
            'preferences/dlgprefreplaygaindlg.ui',
            'preferences/dlgprefsounddlg.ui',
            'preferences/dlgprefsounditem.ui',
            'preferences/dlgprefvinyldlg.ui',
            'preferences/dlgrecording.ui',
            'preferences/dlgtagfetcher.ui',
            'preferences/dlgtrackinfo.ui',
        ]
        ...
        if build.platform_is_windows:
            ...
            # installation -- bkgood
            if build.toolchain_is_msvs: build.env.Append(LINKFLAGS="/MANIFEST")
        elif build.platform_is_osx:
    ...
    def configure(self, build, conf):
        ...
        if build.toolchain_is_gnu:
            ...
            if build.architecture_is_x86:
                if build.machine_is_64bit: build.env.Append(LINKFLAGS='/MACHINE:X64')
                else:                      build.env.Append(LINKFLAGS='/MACHINE:X86')
            elif build.architecture_is_arm:build.env.Append(LINKFLAGS='/MACHINE:ARM')
            else: raise Exception('Invalid machine type for Windows build.')
        ...
        if build.platform_is_linux or build.platform_is_bsd:
            ...
            share_path = os.path.join (prefix, build.env.get('SHAREDIR', default='share'), 'mixxx')
            build.env.Append(CPPDEFINES=('UNIX_SHARE_PATH', r'\"%s\"' % share_path))
            lib_path = os.path.join(prefix, build.env.get('LIBDIR', default='lib'), 'mixxx')
