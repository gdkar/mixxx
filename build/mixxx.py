...
class MixxxBuild(object):
    ...
    def __init__(self, target, machine, build, toolchain, available_features):
        ...
        if target is None:  target = self.host_platform
        if machine is None: machine = self.host_machine
        if toolchain is None:
            if self.host_platform == 'windows':
                raise Exception('must specify toolchain on Windows (msvs or gnu)')
            else:toolchain = 'gnu'
        if build is None: build = 'debug'
        ...
        if flags_force32 and flags_force64:
            ...
            Script.Exit(1)
        if flags_force32:
            if self.machine in ['powerpc', 'powerpc64', 'ppc', 'ppc64']: self.machine = 'powerpc'
            else:                                                        self.machine = 'x86'
        elif flags_force64:
            if self.machine in ['powerpc', 'powerpc64', 'ppc', 'ppc64']: self.machine = 'powerpc64'
            else:                                                       self.machine = 'x86_64'
        ...
        if self.platform_is_windows:
            logging.info("Static dependencies: {}".format ("YES" if self.static_dependencies else "NO"))
        if self.crosscompile:
            logging.info("Host Platform: {}".format( self.host_platform))
            logging.info("Host Machine: {}" .format( self.host_machine))
        ...
        if self.crosscompile:
            if self.platform_is_windows: tools.append('crossmingw')
            if self.platform_is_osx:     tools.append('crossosx')
        ...
        if self.toolchain_is_gnu:
            if flags_force32:       self.env.Append(CCFLAGS='-m32')
            elif flags_force64:     self.env.Append(CCFLAGS='-m64')
    ...
    def detect_platform(self):
        if os.name == 'nt' or sys.platform == 'win32': return 'windows'
        # Should cover {Net,Open,Free,DragonFly}BSD, but only tested on OpenBSD
        if 'bsd' in sys.platform:                      return 'bsd'
        if sys.platform in ['linux2', 'linux3']:       return 'linux'
        if sys.platform == 'darwin':                   return 'osx'
        logging.error("Couldn't determine platform. os.name: {} sys.platform: {}".format(os.name, sys.platform))
        return 'invalid'
    def detect_machine(self):
        return platform.machine()
    def setup_platform_sdk(self):
        if self.platform_is_windows:self.setup_windows_platform_sdk()
        elif self.platform_is_osx:  self.setup_osx_platform_sdk()
    def setup_windows_platform_sdk(self):
        ...
        if os.path.exists(mfc_path): env_update_tuples.append(('INCLUDE', mfc_path))
        atl_path = os.path.join(include_path, 'atl')
        if os.path.exists(atl_path): env_update_tuples.append(('INCLUDE', atl_path))
        bin_path = os.path.join(mssdk_dir, 'Bin')
        if self.machine_is_64bit:    bin_path = os.path.join(bin_path, 'x64')
        ...
        if self.machine_is_64bit:    lib_path = os.path.join(lib_path, 'x64')
    ...
    def setup_osx_platform_sdk(self):
        ...
        if sysroot: self.env.Append(CCFLAGS=['-isysroot', sysroot])
        ...
        if self.platform_is_osx:
            ...
            if osx_stdlib == 'libc++': sdk_version_default = '10.9'
            else:                      sdk_version_default = '10.5'
            ...
            for sdk in sdk_versions:
                sdk_path = os.popen('xcodebuild -version -sdk macosx%s Path' % sdk).readline().strip()
                if sdk_path:
                    print "Automatically selected OS X SDK:", sdk_path
                    common_flags = ['-isysroot', sdk_path,'-mmacosx-version-min={}'.format(min_sdk_version)]
                    link_flags = [
                        '-Wl,-syslibroot,{}'.format(sdk_path),
                        '-stdlib={}'.format(osx_stdlib)
                    ]
    ...
    def read_environment_variables(self):
        ...
        if 'CC' in os.environ:          self.env['CC'] = os.environ['CC']
        if 'CFLAGS' in os.environ:      self.env['CFLAGS'] += SCons.Util.CLVar(os.environ['CFLAGS'])
        if 'CXX' in os.environ:         self.env['CXX'] = os.environ['CXX']
        if 'CXXFLAGS' in os.environ:    self.env['CXXFLAGS'] += SCons.Util.CLVar(os.environ['CXXFLAGS'])
        if 'LDFLAGS' in os.environ:     self.env['LINKFLAGS'] += SCons.Util.CLVar(os.environ['LDFLAGS'])

        # Allow installation directories to be specified.
        prefix = Script.ARGUMENTS.get('prefix', '/usr/local')
        if os.environ.has_key('LIBDIR'):    self.env['LIBDIR'] = os.path.relpath(os.environ['LIBDIR'], prefix)
        if os.environ.has_key('BINDIR'):    self.env['BINDIR'] = os.path.relpath(os.environ['BINDIR'], prefix)
        if os.environ.has_key('SHAREDIR'):  self.env['SHAREDIR'] = os.path.relpath(os.environ['SHAREDIR'], prefix)
    ...
    def get_cache_dir(self):
        ...
        if 'CACHEDIR' not in self.env:              self.env['CACHEDIR'] = str(Script.Dir('#cache/'))
        if not os.path.isdir(self.env['CACHEDIR']): os.mkdir(self.env['CACHEDIR'])
    ...
    def install_options(self):
        ...
        vars.Add('debug_assertions_fatal','Whether debug assertions are fatal.', False)
    ...
    def virtualize_build_dir(self):
        ...
        if not should_virtualize: return
        ...
        if not is_branch_different: return
        ...
        with open(sconsign_branch_file, 'w+') as f:
            ...
            f.write(branch_name)
    def get_features(self): return self.available_features
class Dependence(object):
    def _get_name(self): return self.__class__.__name__
    name = property(_get_name)
    def sources(self, build): return []
    def satisfy(self):        pass
    def depends(self, build): return []
    def configure(self, build, conf): pass
    def post_dependency_check_configure(self, build, conf): pass
class Feature(Dependence):
    def _get_name(self): return self.__class__.__name__
    name = property(_get_name)
    status = ""
    def satisfy(self, build): raise NotImplementedError()
    def description(self):    raise NotImplementedError()
    def enabled(self, build): return False
    def add_options(self, build, vars): pass
