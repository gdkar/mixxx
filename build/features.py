...
class Vamp(Feature):
    ...
    def configure(self, build, conf):
        ...
        if not conf.CheckLib('vamp-hostsdk', autoadd=False) or not conf.CheckForPKG('vamp-sdk', '2.7.1'):
            self.INTERNAL_LINK = True
        ...
        if have_fftw3_h and (have_fftw3 or have_fftw3f) and build.platform_is_linux:
            build.env.Append(CPPDEFINES='HAVE_FFTW3')
            build.env.ParseConfig(
                'pkg-config fftw3f fftw3 --silence-errors --cflags --libs')
...
>>>>>>> tweak features / actually find vamp
