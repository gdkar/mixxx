...
(function(){
    ...
    svg.getHooksPattern = function(){
        var hookNames = [],
            that = this;
        for( var i in this.templateHooks ) hookNames.push(i);
        // hook_name( arg1 [, arg2]... )
        if( hookNames.length ){
    ...
    svg.templateHooks.prop = function( propName, varName ){
        ...
        if( (varName in global) ){
            ...
            if( isNumber(value) ){out = propName + ':' + value + ';';}
            else if( value.length ) {out = propName + ':' + value + ';';}
        } else {console.log( 'Unable to find ' + varName + ' for prop hook.' );}
        return out;
    }
})();
