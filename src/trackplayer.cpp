...
TrackPlayer::TrackPlayer(ConfigObject<ConfigValue>* pConfig,
        ...
        : QObject(pParent),
          m_group(group),
          m_pConfig(pConfig),
          m_waveformZoom(new CVAtom("waveformZooom",this))
        , m_bpm(new CVAtom("bpm", this))
        , m_key(new CVAtom("key",this))
        , m_replayGain(new CVAtom("replayGain",this))
        , m_preGain(new CVAtom("preGain",this))
        , m_speed(new CVAtom("speed",this))
        , m_pitch(new CVAtom("pitch",this))
        , m_duration(new CVAtom("duration",this))
        , m_position(new CVAtom("position",this)),
          m_pLoadedTrack(),
          m_pLowFilter(NULL),
          m_pMidFilter(NULL),
          m_pHighFilter(NULL),
    ...
    m_pChannel = new EngineDeck(channelGroup, pConfig, pMixingEngine,0, EngineChannel::CENTER, this);
    EngineBuffer* pEngineBuffer = m_pChannel->getEngineBuffer();
    pMixingEngine->addChannel(m_pChannel);
    // Set the routing option defaults for the master and headphone mixes.
    m_pChannel->setMaster(true);
    m_pChannel->setPfl   (false);
...
EngineDeck* TrackPlayer::getEngineDeck() const {return m_pChannel;}
