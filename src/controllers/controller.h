...
class Controller : public QObject, ConstControllerPresetVisitor {
  ...
  public:
    ...
    inline bool isOutputDevice() const {return m_bIsOutputDevice;}
    inline bool isInputDevice() const {return m_bIsInputDevice;}
    inline QString getName() const {return m_sDeviceName;}
    inline QString getCategory() const {return m_sDeviceCategory;}
    inline bool debugging() const {return m_bDebug;}
    virtual bool isMappable() const = 0;
    inline bool isLearning() const {return m_bLearning;}
  ...
  protected:
    ...
    inline ControllerEngine* getEngine() const {return m_pEngine;}
    inline void setDeviceName(QString deviceName) {m_sDeviceName = deviceName;}
    inline void setDeviceCategory(QString deviceCategory) {m_sDeviceCategory = deviceCategory;}
    inline void setOutputDevice(bool outputDevice) {m_bIsOutputDevice = outputDevice;}
    inline void setInputDevice(bool inputDevice) {m_bIsInputDevice = inputDevice;}
    inline void setOpen(bool open) {m_bIsOpen = open;}
  private slots:
    virtual int open() = 0;
    virtual int close() = 0;
