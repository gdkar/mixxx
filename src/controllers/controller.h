...
class Controller : public QObject, public ConstControllerPresetVisitor {
    Q_OBJECT
  public:
    Controller();
    ...
    virtual void setPreset(const ControllerPreset& preset) {
    ...
    virtual bool isOutputDevice() const {return m_bIsOutputDevice;}
    virtual bool isInputDevice() const {return m_bIsInputDevice;}
    virtual QString getName() const {return m_sDeviceName;}
    virtual QString getCategory() const {return m_sDeviceCategory;}
    virtual  bool debugging() const {return m_bDebug;}
    virtual bool isMappable() const = 0;
    virtual bool isLearning() const {return m_bLearning;}
  ...
  protected:
    ...
    virtual ControllerEngine* getEngine() const {return m_pEngine;}
    virtual void setDeviceName(QString deviceName) {m_sDeviceName = deviceName;}
    virtual void setDeviceCategory(QString deviceCategory) {m_sDeviceCategory = deviceCategory;}
    virtual void setOutputDevice(bool outputDevice) {m_bIsOutputDevice = outputDevice;}
    virtual void setInputDevice(bool inputDevice) {m_bIsInputDevice = inputDevice;}
    virtual void setOpen(bool open) {m_bIsOpen = open;}
  private slots:
    virtual int open() = 0;
    virtual int close() = 0;
