...
ControllerEngineConnectionScriptValue::ControllerEngineConnectionScriptValue(ControllerEngineConnection conn)
:m_conn(conn)
{ }
QString ControllerEngineConnectionScriptValue::readId() const
{
    return m_conn.id;
}
...
void ControllerEngine::callFunctionOnObjects(QList<QString> scriptFunctionPrefixes,
    ...
    for (const auto & prefixName : scriptFunctionPrefixes) {
...
QScriptValue ControllerEngine::resolveFunction(const QString& function) const {
    auto i = m_scriptValueCache.find(function);
...
bool ControllerEngine::execute(QScriptValue function, const QByteArray data,mixxx::Duration timestamp)
{
    Q_UNUSED(timestamp);
    if (m_pEngine == nullptr)
        return false;
...
bool ControllerEngine::checkException()
{
    ...
    if (m_pEngine->hasUncaughtException()) {
        ...
        if (filename.isEmpty())
            errorText = tr("Uncaught exception at line %1 in passed code: %2").arg(line, errorMessage);
...
ControlObjectScript* ControllerEngine::getControlObjectScript(const QString& group, const QString& name) {
    ...
    if(auto coScript = m_controlCache.value(key, nullptr))
        return coScript;
    else{
        // create COT
...
QScriptValue ControllerEngine::connectControl(
    ...
    if (callback.isString()) {
        ...
        if (!strcmp(qmeta->className(), "ControllerEngineConnectionScriptValue")) {
            if(auto proxy = qobject_cast<ControllerEngineConnectionScriptValue*>(qobject))
                proxy->disconnect();
        }
    } else {
...
{
    auto coScript = getControlObjectScript(conn.key.group, conn.key.item);
    if (!m_pEngine || !coScript)
        return;
                                 ...
                                 bool oneShot)
{
    ...
    auto ctxt = m_pEngine->currentContext();
...
void ControllerEngine::timerEvent(QTimerEvent *event)
{
    ...
    // by value. *boggle*
    const auto &timerTarget = it.value();
...
void ControllerEngine::setPopups(bool bPopups)
{
    m_bPopups = bPopups;
}
QList<QString> ControllerEngine::getScriptFunctionPrefixes()
{
