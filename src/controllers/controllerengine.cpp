...
void ControllerEngine::gracefulShutdown() {
    ...
    while (it != end) {
        ConfigKey key = *it;
        ControlObjectSlave* cot = m_controlCache.take(key);
        delete cot;
        ++it;
    }
...
void ControllerEngine::initializeScripts(const QList<ControllerPreset::ScriptFileInfo>& scripts) {
    ...
    QJSValueList args;
    args << m_pEngine->toScriptValue(m_pController->getName());
    args << m_pEngine->toScriptValue(m_bDebug);
...
bool ControllerEngine::execute(QString function) {
    ...
    if (scriptFunction.isError() || scriptFunction.isUndefined())
        return false;
    if (!scriptFunction.isCallable())
        return false;
    QJSValue result = scriptFunction.call(QJSValueList());
    if (result.isError())
        return false;
    return true;
}
...
bool ControllerEngine::execute(QJSValue functionObject, QJSValueList args) {
    ...
    QJSValue rc = functionObject.call(args);
    if (rc.isError()) {
...
bool ControllerEngine::execute(QJSValue function, const QByteArray data) {
    ...
    QJSValueList args;
    args << m_pEngine->toScriptValue(data);//QScriptValue(m_pBaClass->newInstance(data));
    args << m_pEngine->toScriptValue(data.size());
...
ControlObjectSlave* ControllerEngine::getControlObjectSlave(QString group, QString name) {
    ...
    ControlObjectSlave* cot = m_controlCache.value(key, nullptr);
    if (cot == nullptr) {
        // create COT
...
double ControllerEngine::getValue(QString group, QString name) {
    ControlObjectSlave* cot = getControlObjectSlave(group, name);
    if (cot == nullptr) {
...
void ControllerEngine::disconnectControl(const ControllerEngineConnection conn) {
    ControlObjectSlave* cot = getControlObjectSlave(conn.key.group, conn.key.item);
...
void ControllerEngine::slotValueChanged(double value) {
    ControlObjectSlave* senderCOT = dynamic_cast<ControlObjectSlave*>(sender());
    ...
    if (m_connectedControls.contains(key)) {
        ...
        for (int i = 0; i < conns.size(); ++i) {
            ...
            args << m_pEngine->toScriptValue(value);
            args << m_pEngine->toScriptValue(key.group);
            args << m_pEngine->toScriptValue(key.item);
