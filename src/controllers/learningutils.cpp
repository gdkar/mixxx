...
struct MessageStats {
    ...
    void addMessage(const MidiKeyAndValue& message) {
        ...
        if (message_count == 0) {last_value = message.second;}

        auto opcode = MidiUtils::opCodeFromStatus(message.first.status);
        auto channel = MidiUtils::channelFromStatus(message.first.status);
        ...
        auto absolute_difference = abs(static_cast<int>(message.second) -static_cast<int>(last_value));
...
MidiInputMappings LearningUtils::guessMidiInputMappings(
    ...
    auto stats_by_control = QMap<unsigned char, MessageStats>{};
    auto stats = MessageStats{};
    // Analyze the message
    ...
    for(auto opcode: stats.opcodes) {qDebug() << "Opcode:" << opcode;}
    for(auto channel: stats.channels) {qDebug() << "Channel:" << channel;}
    for(auto control: stats.controls) {qDebug() << "Control:" << control;}
    for (auto it = stats.value_histogram.constBegin(),end=stats.value_histogram.constEnd();
        it!=end; ++it) {
        qDebug() << "Overall Value:" << it.key() << "count" << it.value();
    }
    for (auto control_it = stats_by_control.constBegin(),end=stats_by_control.constEnd();control_it!=end; ++control_it) {
        auto controlName = QString("Control %1").arg(control_it.key());
        for (auto it = control_it->value_histogram.constBegin(),end=control_it->value_histogram.constEnd();it != end; ++it) {
            qDebug() << controlName << "Value:" << it.key() << "count" << it.value();
        }
    }
    MidiInputMappings mappings;
    auto one_control = stats.controls.size() == 1;
    auto one_channel = stats.channels.size() == 1;
    auto only_note_on = stats.opcodes.size() == 1 && stats.opcodes.contains(MIDI_NOTE_ON);
    auto only_note_on_and_note_off = stats.opcodes.size() == 2 &&
            stats.opcodes.contains(MIDI_NOTE_ON) &&
            stats.opcodes.contains(MIDI_NOTE_OFF);

    auto has_cc = stats.opcodes.contains(MIDI_CC);
    auto only_cc = stats.opcodes.size() == 1 && has_cc;
    auto num_cc_controls = 0;
    for (auto it = stats_by_control.constBegin(),end=stats_by_control.constEnd();it != end; ++it) {
        if (it->opcodes.contains(MIDI_CC)) {num_cc_controls++;}
    }
    auto two_values_7bit_max_and_min = stats.value_histogram.size() == 2 &&
            stats.value_histogram.contains(0x00) && stats.value_histogram.contains(0x7F);
    auto one_value_7bit_max_or_min = stats.value_histogram.size() == 1 &&
            (stats.value_histogram.contains(0x00) || stats.value_histogram.contains(0x7F));
    auto multiple_one_or_7f_values = stats.value_histogram.value(0x01, 0) > 1 ||
            stats.value_histogram.value(0x7F, 0) > 1;
    auto under_8_distinct_values = stats.value_histogram.size() < 8;
    auto no_0x00_value = !stats.value_histogram.contains(0x00);
    auto no_0x40_value = !stats.value_histogram.contains(0x40);
    auto multiple_values_around_0x40 = stats.value_histogram.value(0x41, 0) > 1 &&
    ...
    // is <= 0x7F.
    auto only_7bit_values = !stats.value_histogram.isEmpty() &&
    ...
    auto abs_differences_above_60 = !stats.abs_diff_histogram.isEmpty() &&
            (stats.abs_diff_histogram.end() - 1).key() >= 0x60;

    if (one_control && one_channel && two_values_7bit_max_and_min && only_note_on_and_note_off) {
    ...
    } else if (one_control && one_channel && two_values_7bit_max_and_min && only_note_on) {
        ...
        note_on.control = *stats.controls.begin();
        mappings.append(MidiInputMapping(note_on, options, control));
    } else if (one_control && one_channel && one_value_7bit_max_or_min && (only_note_on || only_cc)) {
        ...
        mappings.append(MidiInputMapping(note_on, options, control));
               ...
               no_0x00_value && (abs_differences_above_60 || (under_8_distinct_values && multiple_one_or_7f_values))) {
        ...
        bool hasResetControl = ControlObject::getControl(resetControl) != nullptr;
        ...
        // LSB.
        auto control1 = *stats.controls.begin();
        auto control2 = *(stats.controls.begin() + 1);

        auto control1_max_abs_diff = (stats_by_control[control1].abs_diff_histogram.end() - 1).key();
        auto control2_max_abs_diff = (stats_by_control[control1].abs_diff_histogram.end() - 1).key();
        ...
        auto lsb_control = control1_max_abs_diff > control2_max_abs_diff ? control1 : control2;
        auto msb_control = control1_max_abs_diff > control2_max_abs_diff ? control2 : control1;
    ...
    for (auto it = mappings.begin(),end=mappings.end();it!=end;it++){
        auto  &mapping = *it;
        mapping.description = QString("MIDI Learned from %1 messages.") .arg(messages.size());
    }
    return mappings;
}
