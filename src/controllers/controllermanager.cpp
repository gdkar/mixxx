...
const int kPollIntervalMillis = 2;
#else
          ...
          m_skipPoll(false)
{
    ...
    qDebug() << "Created " << m_enumerators.size() << "enumerator";
...
void ControllerManager::slotSetUpDevices()
{
    ...
    for(auto pController: deviceList) {
        ...
        if (pController->isOpen())
            pController->close();
        // The filename for this devickcue name.
...
{
    return m_pMainThreadPresetEnumerator.get();
}
...
// static
QString ControllerManager::getAbsolutePath(const QString& pathOrFilename,const QStringList& paths)
{
...
bool ControllerManager::importScript(const QString& scriptPath,QString* newScriptFileName)
{
    ...
    while (userPresets.exists(scriptFileName)) {
        ...
        auto localScriptChecksum = quint16{0};
...
void ControllerManager::savePresets(bool onlyActive)
{
...
/* static */ std::atomic<EnumeratorFactory*> ControllerManager::m_factories{nullptr};
