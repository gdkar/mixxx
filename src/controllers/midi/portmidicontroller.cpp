...
PortMidiController::PortMidiController(const PmDeviceInfo* inputDeviceInfo,
    ...
    if (m_pInputDeviceInfo) {setInputDevice(m_pInputDeviceInfo->input);}
    if (m_pOutputDeviceInfo) {setOutputDevice(m_pOutputDeviceInfo->output);}
}
PortMidiController::~PortMidiController() {close();}
