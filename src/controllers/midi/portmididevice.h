...
class PortMidiDevice {
  public:
    ...
    virtual PmError poll() { return Pm_Poll(m_pStream); }
    virtual int read(PmEvent* buffer, int32_t length) { return Pm_Read(m_pStream, buffer, length); }
    virtual PmError writeShort(int32_t message) { return Pm_WriteShort(m_pStream, 0, message); }
    virtual PmError writeSysEx(unsigned char* message) { return Pm_WriteSysEx(m_pStream, 0, message); }
  private:
