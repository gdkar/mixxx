...
DlgPrefControllers::DlgPrefControllers(DlgPreferences* pPreferences,
    ...
    connect(&m_buttonMapper, SIGNAL(mapped(QString)),this, SLOT(slotOpenLocalFile(QString)));
    connect(btnOpenUserPresets, SIGNAL(clicked()),&m_buttonMapper, SLOT(map()));
...
void DlgPrefControllers::slotUpdate()
{
    ...
    for(auto pControllerWindows: m_controllerWindows) pControllerWindows->slotUpdate();
}

void DlgPrefControllers::slotCancel()
{
    // Update our sub-windows.
    for(auto pControllerWindows: m_controllerWindows)pControllerWindows->slotCancel();
}
void DlgPrefControllers::slotApply()
{
    // Update our sub-windows.
    for(auto pControllerWindows: m_controllerWindows) pControllerWindows->slotApply();
...
bool DlgPrefControllers::handleTreeItemClick(QTreeWidgetItem* clickedItem)
{
    ...
    if (controllerIndex >= 0) {
        ...
        if (controllerWidget) m_pDlgPreferences->switchToPage(controllerWidget);
        return true;
...
void DlgPrefControllers::slotHighlightDevice(DlgPrefController* dialog, bool enabled)
{
    ...
    if (!controllerWindowLink)  return;
