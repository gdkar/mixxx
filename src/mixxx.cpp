...
void MixxxMainWindow::initialize(QApplication* pApp, const CmdlineArgs& args)
{
    ...
    auto rescan = pConfig->getValueString(
    ...
    pConfig->set(ConfigKey("[Library]", "SupportedFileExtensions"),QStringList(SoundSourceProxy::getSupportedFileExtensions()).join(","));
    ...
    // 1 and 2:
    auto musicFiles = args.getMusicFiles();
    for (auto i = 0; i < (int)m_pPlayerManager->numDecks()
...
void MixxxMainWindow::finalize()
{
    ...
    auto pMenuBar = QPointer<QWidget>(menuBar());
    ...
    if (leakedControls.size() > 0) {
        ...
        if (CmdlineArgs::Instance().getDeveloper()) {
            for(auto key: leakedConfigKeys) {
                ...
                // pointer each time.
                if(auto pCo = ControlObject::getControl(key, false))
                    delete pCo;
            }
        }
...
void MixxxMainWindow::initializeWindow()
{
    ...
    auto MenuBarBackground = QColor(m_pMenuBar->palette().color(QPalette::Background));
...
int MixxxMainWindow::noSoundDlg(void)
{
    ...
    auto retryButton = msgBox.addButton(tr("Retry"),QMessageBox::ActionRole);
    auto reconfigureButton = msgBox.addButton(tr("Reconfigure"),QMessageBox::ActionRole);
    auto wikiButton = msgBox.addButton(tr("Help"),QMessageBox::ActionRole);
...
void MixxxMainWindow::connectMenuBar()
{
    ...
    if (m_pLibrary) {
        connect(m_pMenuBar, SIGNAL(createCrate()),      m_pLibrary, SLOT(slotCreateCrate()));
        connect(m_pMenuBar, SIGNAL(createPlaylist()),   m_pLibrary, SLOT(slotCreatePlaylist()));
        connect(m_pLibrary, SIGNAL(scanStarted()),      m_pMenuBar, SLOT(onLibraryScanStarted()));
        connect(m_pLibrary, SIGNAL(scanFinished()),     m_pMenuBar, SLOT(onLibraryScanFinished()));
        connect(m_pMenuBar, SIGNAL(rescanLibrary()),    m_pLibrary, SLOT(scan()));
    }
...
void MixxxMainWindow::slotFileLoadSongPlayer(int deck)
{
    ...
    if (ControlObject::get(ConfigKey(group, "play")) > 0.0) {
        auto ret = QMessageBox::warning(this, Version::applicationName(),
            deckWarningMessage + "\n" + areYouSure,
...
void MixxxMainWindow::slotDeveloperTools(bool visible)
{
    if (visible) {
        if (m_pDeveloperToolsDlg == nullptr) {
            ...
            connect(m_pDeveloperToolsDlg, SIGNAL(destroyed()),          m_pMenuBar, SLOT(onDeveloperToolsHidden()));
        }
...
void MixxxMainWindow::slotDeveloperToolsClosed() { m_pDeveloperToolsDlg = NULL; }
...
{
    auto about = new DlgAbout(this);
    about->show();
}
void MixxxMainWindow::setToolTipsCfg(mixxx::TooltipsPreference tt)
{
    auto pConfig = m_pSettingsManager->settings();
    pConfig->set(ConfigKey("[Controls]","Tooltips"),ConfigValue(static_cast<int>(tt)));
    m_toolTipsCfg = tt;
}
void MixxxMainWindow::rebootMixxxView()
{
    ...
    auto wasFullScreen = isFullScreen();
