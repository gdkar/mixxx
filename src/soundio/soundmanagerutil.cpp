 ...
 * @note This method is static.
 */
QString AudioPath::getTrStringFromType(AudioPathType type, uint8_t index) {
    switch (type) {
    case INVALID:
        // this shouldn't happen but g++ complains if I don't
        // handle this -- bkgood
...
// static
uint8_t AudioPath::minChannelsForType(AudioPathType type)
{
    switch (type) {
...
// static
uint8_t AudioPath::maxChannelsForType(AudioPathType type)
{
    switch (type) {
    default:
    return 2;
...
AudioOutput AudioOutput::fromXML(const QDomElement &xml)
{
    ...
    uint32_t index(xml.attribute("index", "0").toUInt());
    uint32_t channel(xml.attribute("channel", "0").toUInt());
    uint32_t channels(xml.attribute("channel_count", "0").toUInt());
