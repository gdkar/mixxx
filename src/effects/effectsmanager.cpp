...
EffectsManager::EffectsManager(QObject* pParent, ConfigObject<ConfigValue>* pConfig)
    ...
    auto requestPipes = TwoWayMessagePipe<EffectsRequest*, EffectsResponse>::makeTwoWayMessagePipe(2048, 2048, false, false);
...
const QList<QString> EffectsManager::getAvailableEffects() const
{
    ...
    for(auto pBackend: m_effectsBackends) {
        ...
        for(auto effectId: backendEffects) {
...
void EffectsManager::setupDefaults()
{
    ...
    pStandardRack->addEffectChainSlot();
    pStandardRack->addEffectChainSlot();
    auto pChain = EffectChainPointer(new EffectChain(this, "org.mixxx.effectchain.flanger"));
    pChain->setName(tr("Flanger"));
    auto pEffect = instantiateEffect("org.mixxx.effects.flanger");
    ...
    pChain = EffectChainPointer(new EffectChain(this, "org.mixxx.effectchain.reverb"));
...
{
    if (m_pRequestPipe.isNull()) return;
