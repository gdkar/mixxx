...
class EffectParameter : public QObject {
    Q_OBJECT
  public:
    EffectParameter(Effect* pEffect, EffectsManager* pEffectsManager,int iParameterNumber, const EffectManifestParameter& parameter);
  ...
  private:
    QString debugString() const {return QString("EffectParameter(%1)").arg(m_parameter.name());}
    static bool clampValue(double* pValue,const double& minimum, const double& maximum);
    bool clampValue();
    bool clampDefault();
    bool clampRanges();
    Effect* m_pEffect;
