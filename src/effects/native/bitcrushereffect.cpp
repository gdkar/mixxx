...
void BitCrusherEffect::processChannel(const ChannelHandle& handle,
    ...
    auto downsample = static_cast<CSAMPLE>(m_pDownsampleParameter ? m_pDownsampleParameter->value() : 0);
    auto bit_depth = static_cast<CSAMPLE>(m_pBitDepthParameter ? m_pBitDepthParameter->value() : 16);
    // divided by two because we use float math which includes the sing bit anyway
    auto scale = std::pow(2.0f, bit_depth) / 2;
    ...
    auto gainCorrection = static_cast<CSAMPLE>(17 - bit_depth) / 8;
    auto kChannels = 2;
    for (auto i = decltype(numSamples){0}; i < numSamples; i += kChannels) {
        ...
        pOutput[i+0] = pState->hold_l;
        pOutput[i+1] = pState->hold_r;
    }
}
QString BitCrusherEffect::debugString() const{return getId();}
