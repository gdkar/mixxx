...
EffectSlot::EffectSlot(const QString& group,
    ...
    m_pControlNumParameters->connectValueChangeRequest(this, SLOT(slotNumParameters(double)));
    m_pControlNumParameterSlots = new ControlObject(ConfigKey(m_group, "num_parameterslots"));
    m_pControlNumParameterSlots->connectValueChangeRequest(this, SLOT(slotNumParameterSlots(double)));
    m_pControlNumButtonParameters = new ControlObject(ConfigKey(m_group, "num_button_parameters"));
    m_pControlNumButtonParameters->connectValueChangeRequest(this, SLOT(slotNumParameters(double)));
    ...
    m_pControlEnabled->setProperty("buttonMode",QVariant::fromValue(ControlPushButton::ButtonMode::PowerWindow));
    // Default to enabled. The skin might not show these buttons.
    m_pControlEnabled->setDefaultValue(true);
    m_pControlEnabled->set(true);
    connect(m_pControlEnabled, SIGNAL(valueChanged(double)),this, SLOT(slotEnabled(double)));
    m_pControlNextEffect = new ControlPushButton(ConfigKey(m_group, "next_effect"));
    connect(m_pControlNextEffect, SIGNAL(valueChanged(double)),this, SLOT(slotNextEffect(double)));
    m_pControlPrevEffect = new ControlPushButton(ConfigKey(m_group, "prev_effect"));
    connect(m_pControlPrevEffect, SIGNAL(valueChanged(double)),this, SLOT(slotPrevEffect(double)));
    // Ignoring no-ops is important since this is for +/- tickers.
    m_pControlEffectSelector = new ControlObject(ConfigKey(m_group, "effect_selector"), false);
    connect(m_pControlEffectSelector, SIGNAL(valueChanged(double)),this, SLOT(slotEffectSelector(double)));
    m_pControlClear = new ControlPushButton(ConfigKey(m_group, "clear"));
    connect(m_pControlClear, SIGNAL(valueChanged(double)),this, SLOT(slotClear(double)));
...
EffectParameterSlotPointer EffectSlot::addEffectParameterSlot() {
    auto pParameter = EffectParameterSlotPointer( new EffectParameterSlot(m_group, m_parameters.size()));
    m_parameters.append(pParameter);
    m_pControlNumParameterSlots->setAndConfirm(m_pControlNumParameterSlots->get() + 1);
    return pParameter;
}
EffectButtonParameterSlotPointer EffectSlot::addEffectButtonParameterSlot() {
    auto pParameter = EffectButtonParameterSlotPointer(new EffectButtonParameterSlot(m_group, m_buttonParameters.size()));
    m_buttonParameters.append(pParameter);
    m_pControlNumButtonParameterSlots->setAndConfirm(m_pControlNumButtonParameterSlots->get() + 1);
    return pParameter;
}
EffectPointer EffectSlot::getEffect() const {return m_pEffect; }
unsigned int EffectSlot::numParameterSlots() const {return m_parameters.size(); }
unsigned int EffectSlot::numButtonParameterSlots() const {return m_buttonParameters.size(); }
...
void EffectSlot::slotEnabled(double v) { if (m_pEffect) {m_pEffect->setEnabled(v > 0);} }
...
void EffectSlot::loadEffect(EffectPointer pEffect) {
    if (pEffect) {
        ...
        connect(pEffect.data(), SIGNAL(enabledChanged(bool)),this, SLOT(slotEffectEnabledChanged(bool)));
        ...
        for(auto pParameter: m_parameters) {pParameter->loadEffect(pEffect);}
        for(auto pParameter: m_buttonParameters) { pParameter->loadEffect(pEffect);}
...
void EffectSlot::clear() {
    if (m_pEffect) { m_pEffect->disconnect(this); }
    ...
    for(auto pParameter: m_parameters) { pParameter->clear();}
    for(auto pParameter: m_buttonParameters) { pParameter->clear();}
    m_pEffect.clear();
    emit(updated());
}
void EffectSlot::slotPrevEffect(double v) {if (v > 0) {slotEffectSelector(-1);}}
void EffectSlot::slotNextEffect(double v) {if (v > 0) {slotEffectSelector(1);}}
...
void EffectSlot::slotClear(double v) {if (v > 0) {emit(clearEffect(m_iEffectNumber));}}
...
void EffectSlot::syncSofttakeover() {for (int i = 0; i < m_parameters.size(); ++i) {m_parameters[i]->syncSofttakeover();}}
