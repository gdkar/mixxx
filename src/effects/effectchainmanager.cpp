...
EqualizerRackPointer EffectChainManager::addEqualizerRack() {
    EqualizerRackPointer pRack(new EqualizerRack(m_pEffectsManager, this, m_equalizerEffectRacks.size()));
...
QuickEffectRackPointer EffectChainManager::addQuickEffectRack() {
    QuickEffectRackPointer pRack(new QuickEffectRack(m_pEffectsManager, this, m_quickEffectRacks.size()));
...
bool EffectChainManager::loadEffectChains() {
    ...
    for (int i = 0; i < chains.count(); ++i) {
        ...
        if (chainNode.isElement()) {
            EffectChainPointer pChain = EffectChain::fromXML(m_pEffectsManager, chainNode.toElement());
