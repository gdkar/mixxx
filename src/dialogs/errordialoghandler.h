...
class ErrorDialogProperties {
  public:
    ...
    inline void setInfoText(QString text) {m_infoText = text;}
    /** Set detailed text (causes "Show Details" button to appear.) */
    inline void setDetails(QString text) {m_details = text;}
    ...
    inline void setShouldQuit(bool shouldQuit) {m_shouldQuit = shouldQuit;}
    ...
    inline void setIcon(QMessageBox::Icon icon) {m_icon = icon;}
    /** Add a standard button to the box */
    void addButton(QMessageBox::StandardButton);
    /** Set the default button to highlight */
    inline void setDefaultButton(QMessageBox::StandardButton button) {m_defaultButton = button;}
    /** Set the button to click if the Escape key is pressed */
    inline void setEscapeButton(QMessageBox::StandardButton button) {m_escapeButton = button;}
  private:
...
class ErrorDialogHandler : public QObject {
 ...
 public:
    static ErrorDialogHandler* instance() {
      static ErrorDialogHandler *l_pInstance = new ErrorDialogHandler();
      return l_pInstance;
    }
    ...
    // will shut down.
    bool requestErrorDialog(DialogType type, QString message,bool shouldQuit=false);
