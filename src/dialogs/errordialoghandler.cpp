...
ErrorDialogHandler::ErrorDialogHandler()
    ...
    connect(this, SIGNAL(showErrorDialog(ErrorDialogProperties*)),this, SLOT(errorDialog(ErrorDialogProperties*)));
}
ErrorDialogHandler::~ErrorDialogHandler() {}
...
bool ErrorDialogHandler::requestErrorDialog(DialogType type, QString message,bool shouldQuit) {
    ...
    if (shouldQuit) {props->setShouldQuit(shouldQuit);}
    switch (type) {
...
bool ErrorDialogHandler::requestErrorDialog(ErrorDialogProperties* props) {
    ...
    DEBUG_ASSERT_AND_HANDLE(!text.isEmpty()) {return false;}
...
void ErrorDialogHandler::errorDialog(ErrorDialogProperties* pProps) {
    ...
    while (!props->m_buttons.isEmpty()) {msgBox->addButton(props->m_buttons.takeFirst());}
