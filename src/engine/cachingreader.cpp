...
void CachingReader::freeAllChunks() {
    m_allocatedChunks.clear();
    m_mruChunk = NULL;
    for (QVector<Chunk*>::const_iterator it = m_chunks.constBegin();it != m_chunks.constEnd(); ++it) {
        Chunk* pChunk = *it;
...
Chunk* CachingReader::allocateChunk(int chunk) {
    if (m_freeChunks.isEmpty()) {return NULL;}
...
void CachingReader::freshenChunk(Chunk* pChunk) {
    ...
    if (pChunk == m_lruChunk && pChunk->prev_lru != NULL) {m_lruChunk = pChunk->prev_lru;}
...
Chunk* CachingReader::lookupChunkAndFreshen(int chunk_number) {
    ...
    if (pChunk != NULL) {freshenChunk(pChunk);}
    return pChunk;
}
...
void CachingReader::newTrack(TrackPointer pTrack) {
    ...
    m_pAudioSource = openAudioSourceForReading(m_pTrack, audioSrcCfg);
...
void CachingReader::process() {
    ...
    while (m_readerStatusFIFO.read(&status, 1) == 1) {
        ...
        if (status.status == TRACK_NOT_LOADED) {
            ...
            DEBUG_ASSERT_AND_HANDLE(pChunk != NULL) {continue;}
...
/*    if (m_readerStatus != TRACK_LOADED) {
    ...
    for (HintVector::const_iterator it = hintList.constBegin();
        ...
        const int start_frame = math_clamp(frame, 0, m_iTrackNumFramesCallbackSafe);
        const int start_chunk = chunkForFrame(start_frame);
        int end_frame = math_clamp(frame + (frame_count - 1), 0, m_iTrackNumFramesCallbackSafe);
        const int end_chunk = chunkForFrame(end_frame);
...
//                             << current;
//                }
                m_pScheduler->workerReady(new CachingReaderWorker(m_group,request,m_readerStatusFIFO,
                  m_pAudioSource));
