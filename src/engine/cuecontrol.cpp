    ...
    if (pCue) {
        if (v) {
            ...
            if (pCue->getPosition() == -1) {
                ...
                if (!m_bPreviewingHotcue && m_pPlayButton->toBool())  hotcueGoto(pControl, v);
                else                                                  hotcueActivatePreview(pControl, v);
            }
        } else if (pCue->getPosition() != -1)                         hotcueActivatePreview(pControl, v);
    }
    else {
        if (v)
        {
...
void CueControl::hotcuePositionChanged(HotcueControl* pControl, double newPosition)
{
    ...
    if (pCue)
    {
        ...
        {
            auto position = static_cast<int>(newPosition);
...
void CueControl::playStutter(double v)
{
    ...
    if (v != 0.0)
    {
        if (m_pPlayButton->toBool()) cueGoto(1.0);
...
{
    return ConfigKey(m_group,QString("hotcue_%1_%2").arg(QString::number(hotcue+1),name));
}
