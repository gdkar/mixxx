          ...
          m_pShoutcastStatus(new ControlObject(ConfigKey(SHOUTCAST_PREF_KEY, "status"),this))
{
    ...
    m_pUpdateShoutcastFromPrefs = new ControlObjectSlave( m_pShoutcastNeedUpdateFromPrefs->getKey(),this);
    // Initialize libshout
    shout_init();
...
void EngineShoutcast::updateFromPreferences() {
    ...
    auto baCodec = codec.toLocal8Bit();
    ...
    if (m_format_is_mp3)     format = SHOUT_FORMAT_MP3;
    else if (m_format_is_ov) format = SHOUT_FORMAT_OGG;
    else
    {
    ...
    auto iMasterSamplerate = static_cast<int>(m_pMasterSamplerate->get());
...
bool EngineShoutcast::serverConnect() {
    ...
    if(m_pMetaData) m_pMetaData.clear();
   ...
   auto iMaxTries = 3;
    while (!m_bQuit && m_iShoutFailures < iMaxTries)
    {
        ...
        if (m_iShoutStatus == SHOUTERR_SUCCESS) m_iShoutStatus = SHOUTERR_CONNECTED;
...
void EngineShoutcast::process(const CSAMPLE* pBuffer, int iBufferSize)
{
    //Check to see if Shoutcast is enabled, and pass the samples off to be broadcast if necessary.
    auto prefEnabled = (m_pConfig->getValueString(ConfigKey(SHOUTCAST_PREF_KEY,"enabled")).toInt() == 1);
    if (!prefEnabled)
    {
        if (isConnected())
        {
    ...
    if (!connected || m_pUpdateShoutcastFromPrefs->get() > 0.0)
    {
        ...
        if (serverConnect()) infoDialog(tr("Mixxx has successfully connected to the streaming server"), "");
        else
        {
    ...
    if (m_iShoutStatus != SHOUTERR_CONNECTED) return;
    // If we are connected, encode the samples.
    if (iBufferSize > 0 && m_encoder) m_encoder->encodeBuffer(pBuffer, iBufferSize);
    // Check if track metadata has changed and if so, update.
    if (metaDataHasChanged()) updateMetaData();
}
bool EngineShoutcast::metaDataHasChanged()
{
    ...
    if (m_pMetaData)
    {
        ...
        {
            if ((pTrack->getArtist() == m_pMetaData->getArtist()) && (pTrack->getTitle() == m_pMetaData->getArtist()))
            {
                return false;
            }
        } else if (pTrack->getId() == m_pMetaData->getId()) return false;
    }
    m_pMetaData = pTrack;
    return true;
}
...
{
    if (!m_pShout || !m_pShoutMetaData) return;
    /**
    ...
    if (!m_custom_metadata && (m_format_is_mp3 || m_ogg_dynamic_update)) {
        if (m_pMetaData)
        {
            ...
            else
            {
                ...
                auto metadataFinal = m_metadataFormat;
                do 
                {
