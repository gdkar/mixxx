...
KeyControl::KeyControl(QString group, ConfigObject<ConfigValue>* pConfig, QObject *p)
        : EngineControl(group, pConfig,p)
{
    ...
    connect(m_pPitch, SIGNAL(valueChanged(double)),this, SLOT(slotPitchChanged(double)),Qt::DirectConnection);
    ...
    m_pPitchAdjust->setParent(this);
    // Course adjust by full semitone steps.
    m_pPitchAdjust->setStepCount(6);
    // Fine adjust with semitone / 10 = 10 ct;.
    m_pPitchAdjust->setSmallStepCount(60);
    connect(m_pPitchAdjust, SIGNAL(valueChanged(double)),this, SLOT(slotPitchAdjustChanged(double)),Qt::DirectConnection);
    m_pButtonSyncKey = new ControlPushButton(ConfigKey(group, "sync_key"),false,this);
    connect(m_pButtonSyncKey, SIGNAL(valueChanged(double)),this, SLOT(slotSyncKey(double)),Qt::DirectConnection);
    m_pButtonResetKey = new ControlPushButton(ConfigKey(group, "reset_key"),false,this);
    connect(m_pButtonResetKey, SIGNAL(valueChanged(double)),this, SLOT(slotResetKey(double)),Qt::DirectConnection);
    m_pFileKey = new ControlObject(ConfigKey(group, "file_key"),this);
    connect(m_pFileKey, SIGNAL(valueChanged(double)),this, SLOT(slotFileKeyChanged(double)),Qt::DirectConnection);
    m_pEngineKey = new ControlObject(ConfigKey(group, "key"));
    connect(m_pEngineKey, SIGNAL(valueChanged(double)),this, SLOT(slotSetEngineKey(double)),Qt::DirectConnection);
    m_pEngineKeyDistance = new ControlPotmeter(ConfigKey(group, "visual_key_distance"), -0.5, 0.5);
    m_pEngineKeyDistance->setParent(this);
    connect(m_pEngineKeyDistance, SIGNAL(valueChanged(double)),this, SLOT(slotSetEngineKeyDistance(double)),Qt::DirectConnection);
    m_keylockMode = new ControlPushButton(ConfigKey(group, "keylockMode"),false,this);
    m_keylockMode->setButtonMode(ControlPushButton::TOGGLE);
    // In case of vinyl control "rate" is a filtered mean value for display
    if((m_pRateSlider = ControlObject::getControl(ConfigKey(group, "rate"))))
    {
      connect(m_pRateSlider, SIGNAL(valueChanged(double)),this, SLOT(slotRateChanged()),Qt::DirectConnection);
      connect(m_pRateSlider, SIGNAL(valueChangedFromEngine(double)),this, SLOT(slotRateChanged()),Qt::DirectConnection);
    }
    if((m_pRateRange = ControlObject::getControl(ConfigKey(group, "rateRange"))))
    {
      connect(m_pRateRange, SIGNAL(valueChanged(double)),this, SLOT(slotRateChanged()),Qt::DirectConnection);
      connect(m_pRateRange, SIGNAL(valueChangedFromEngine(double)),this, SLOT(slotRateChanged()),Qt::DirectConnection);
    }
    if((m_pRateDir = ControlObject::getControl(ConfigKey(group, "rate_dir"))))
    {
      connect(m_pRateDir, SIGNAL(valueChanged(double)),this, SLOT(slotRateChanged()),Qt::DirectConnection);
      connect(m_pRateDir, SIGNAL(valueChangedFromEngine(double)),this, SLOT(slotRateChanged()),Qt::DirectConnection);
    }
    m_pVCEnabled = ControlObject::getControl(ConfigKey(group, "vinylcontrol_enabled"));
    if (m_pVCEnabled)
    {
        connect(m_pVCEnabled, SIGNAL(valueChanged(double)),this, SLOT(slotRateChanged()),Qt::DirectConnection);
        connect(m_pVCEnabled, SIGNAL(valueChangedFromEngine(double)),this, SLOT(slotRateChanged()),Qt::DirectConnection);
    }
    m_pVCRate = ControlObject::getControl(ConfigKey(group, "vinylcontrol_rate"));
    if (m_pVCRate)
    {
        connect(m_pVCRate, SIGNAL(valueChanged(double)),this, SLOT(slotRateChanged()),Qt::DirectConnection);
        connect(m_pVCRate, SIGNAL(valueChangedFromEngine(double)),this, SLOT(slotRateChanged()),Qt::DirectConnection);
    }
    m_pKeylock = ControlObject::getControl(ConfigKey(group, "keylock"));
    connect(m_pKeylock, SIGNAL(valueChanged(double)),this, SLOT(slotRateChanged()),Qt::DirectConnection);
    connect(m_pKeylock, SIGNAL(valueChangedFromEngine(double)),this, SLOT(slotRateChanged()),Qt::DirectConnection);
}
...
KeyControl::PitchTempoRatio KeyControl::getPitchTempoRatio() {
    ...
    if (m_updatePitchRequest.exchange(false))        updatePitch();
    if (m_updatePitchAdjustRequest.exchange(false))  updatePitchAdjust();
    if (m_updateRateRequest.exchange(false))         updateRate();
...
void KeyControl::updateRate() {
    ...
    // caused by it.
    if(m_pVCEnabled && m_pVCEnabled->toBool()) m_pitchRateInfo.tempoRatio = m_pVCRate->get();
    else                                       m_pitchRateInfo.tempoRatio = 1.0 + m_pRateDir->get() * m_pRateRange->get() * m_pRateSlider->get();
    if (!m_pitchRateInfo.tempoRatio)  return;
    // |-----------------------|-----------------|
    ...
    if (m_pKeylock->toBool())
    {
        ...
        {
            // Enabling Keylock
            if (m_keylockMode->get() == kLockCurrentKey) speedSliderPitchRatio = m_pitchRateInfo.tempoRatio;
            else                                         speedSliderPitchRatio = 1.0;
...
void KeyControl::setEngineKey(double key, double key_distance)
{
    ...
    if (thisFileKey == mixxx::track::io::key::INVALID || newKey == mixxx::track::io::key::INVALID)
        return;
...
{
    Q_UNUSED(pitch)
    m_updatePitchRequest.store(true);
    updatePitch();
}
void KeyControl::updatePitch()
{
    ...
    auto dFileKey = m_pFileKey->get();
    m_pPitchAdjust->set( KeyUtils::powerOf2ToSemitoneChange(m_pitchRateInfo.pitchTweakRatio));
...
void KeyControl::slotPitchAdjustChanged(double pitchAdjust)
{
    ...
    m_updatePitchAdjustRequest.store(true) ;
    updatePitchAdjust();
}
...
{
    if (v > 0) slotSetEngineKey(m_pFileKey->get());
}
bool KeyControl::syncKey(EngineBuffer* pOtherEngineBuffer)
{
    if (!pOtherEngineBuffer) return false;
    auto thisFileKey = KeyUtils::keyFromNumericValue(m_pFileKey->get());
    ...
    auto otherKey = KeyUtils::keyFromNumericValue(dKey);
    auto otherDistance = ControlObject::get(ConfigKey(pOtherEngineBuffer->getGroup(), "visual_key_distance"));
    if (thisFileKey == mixxx::track::io::key::INVALID || otherKey == mixxx::track::io::key::INVALID)
        return false;
    auto stepsToTake = KeyUtils::shortestStepsToCompatibleKey(thisFileKey, otherKey);
    auto pitchToTakeOctaves = (stepsToTake + otherDistance) / 12.0;
    m_pPitch->set(pitchToTakeOctaves * 12);
    slotPitchChanged(pitchToTakeOctaves * 12);
    return true;
}
