        ...
        m_sGroup(group)
{
    ...
    m_pAdjustBeatsFaster = new ControlObject(ConfigKey(group, "beats_adjust_faster"),this, false);
    connect(m_pAdjustBeatsFaster, SIGNAL(valueChanged(double)),
            this, SLOT(slotAdjustBeatsFaster(double)),
            Qt::AutoConnection);
    m_pAdjustBeatsSlower = new ControlObject(ConfigKey(group, "beats_adjust_slower"),this, false);
    connect(m_pAdjustBeatsSlower, SIGNAL(valueChanged(double)),
            this, SLOT(slotAdjustBeatsSlower(double)),
            Qt::AutoConnection);
    m_pTranslateBeatsEarlier = new ControlObject(ConfigKey(group, "beats_translate_earlier"),this, false);
    connect(m_pTranslateBeatsEarlier, SIGNAL(valueChanged(double)),
            this, SLOT(slotTranslateBeatsEarlier(double)),
            Qt::AutoConnection);
    m_pTranslateBeatsLater = new ControlObject(ConfigKey(group, "beats_translate_later"),this, false);
    ...
    m_pButtonTap = new ControlObject(ConfigKey(group, "bpm_tap"),this);
    ...
    m_pButtonSync = new ControlObject(ConfigKey(group, "beatsync"),this);
    connect(m_pButtonSync, SIGNAL(valueChanged(double)),
            this, SLOT(slotControlBeatSync(double)),
            Qt::AutoConnection);

    m_pButtonSyncPhase = new ControlObject(ConfigKey(group, "beatsync_phase"),this);
    connect(m_pButtonSyncPhase, SIGNAL(valueChanged(double)),
            this, SLOT(slotControlBeatSyncPhase(double)),
            Qt::AutoConnection);

    m_pButtonSyncTempo = new ControlObject(ConfigKey(group, "beatsync_tempo"),this);
    connect(m_pButtonSyncTempo, SIGNAL(valueChanged(double)),
            this, SLOT(slotControlBeatSyncTempo(double)),
            Qt::AutoConnection);

    m_pTranslateBeats = new ControlObject(ConfigKey(group, "beats_translate_curpos"),this);
    connect(m_pTranslateBeats, SIGNAL(valueChanged(double)),
            this, SLOT(slotBeatsTranslate(double)),
            Qt::AutoConnection);

    m_pBeatsTranslateMatchAlignment = new ControlObject(ConfigKey(group, "beats_translate_match_alignment"),this);
...
void BpmControl::slotFileBpmChanged(double bpm) {
    ...
    if (m_pBeats) {
        auto beats_bpm = m_pBeats->getBpmAroundPosition(getCurrentSample(),kLocalBpmSpan);
        if (beats_bpm != -1) {
...
void BpmControl::slotAdjustBeatsFaster(double v)
{
    if (v > 0 && m_pBeats && (m_pBeats->getCapabilities() & Beats::BEATSCAP_SETBPM)) {
        auto new_bpm = math_min(200.0, m_pBeats->getBpm() + .01);
...
{
    if (v > 0 && m_pBeats && (m_pBeats->getCapabilities() & Beats::BEATSCAP_SETBPM)) {
        auto new_bpm = math_max(10.0, m_pBeats->getBpm() - .01);
        m_pBeats->setBpm(new_bpm);
    }
}
...
void BpmControl::slotTranslateBeatsEarlier(double v)
{
    if (v > 0 && m_pTrack && m_pBeats &&
        ...
        auto translate_dist = m_pTrack->getSampleRate() * -.01;
...
void BpmControl::slotSetEngineBpm(double bpm)
{
    ...
    auto ratedir = m_pRateDir->get();
    auto raterange = m_pRateRange->get();

    if (localbpm && ratedir && raterange) {
        auto newRateRatio = bpm / localbpm;
...
void BpmControl::slotTapFilter(double averageLength, int numSamples)
{
    ...
    // beat) = Y beats/minute
    auto averageBpm = 60.0 * 1000.0 / averageLength;
    auto dRate = calcRateRatio();
...
bool BpmControl::syncTempo()
{
    ...
    auto fThisBpm = m_pEngineBpm->get();
    auto fThisLocalBpm = m_pLocalBpm->get();

    auto fOtherBpm = pOtherEngineBuffer->getBpm();
    auto fOtherLocalBpm = pOtherEngineBuffer->getLocalBpm();
    ...
    if (fOtherBpm > 0.0 && fThisBpm > 0.0) {
        ...
        auto desiredRate = fOtherBpm / fThisLocalBpm;
        ...
        auto fFileBpmDelta = std::abs(fThisLocalBpm - fOtherLocalBpm);
        if (std::abs(fThisLocalBpm * 2.0 - fOtherLocalBpm) < fFileBpmDelta) {
            desiredRate /= 2.0;
        } else if (std::abs(fThisLocalBpm - 2.0 * fOtherLocalBpm) < fFileBpmDelta) {
            desiredRate *= 2.0;
        }

        // Subtract the base 1.0, now fDesiredRate is the percentage
        // increase/decrease in playback rate, not the playback rate.
        auto desiredRateShift = desiredRate - 1.0;
        ...
        if (desiredRateShift < 1.0 && desiredRateShift > -0.5)
        {
            ...
            auto desiredRateSlider = desiredRateShift / (m_pRateRange->get() * m_pRateDir->get());
...
double BpmControl::shortestPercentageChange(double current_percentage,double target_percentage)
{
    if (current_percentage == target_percentage) {
        ...
        return (fabs(forwardDistance) < std::abs(backwardsDistance)) ?
...
double BpmControl::calcSyncedRate(double userTweak) {
    ...
    auto loop_size = (m_pLoopEndPosition->get() -
...
double BpmControl::getBeatDistance(double dThisPosition) const {
    ...
    // sync against itself.
    auto dPrevBeat = m_pPrevBeat->get();
    auto dNextBeat = m_pNextBeat->get();
    ...
    auto dBeatPercentage = dBeatLength == 0.0 ? 0.0 :
...
double BpmControl::updateLocalBpm()
{
    ...
    auto local_bpm = 0;
    if (m_pBeats) {
        local_bpm = m_pBeats->getBpmAroundPosition(getCurrentSample(),kLocalBpmSpan);
        if (local_bpm == -1) {
