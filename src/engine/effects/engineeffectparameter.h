...
class EngineEffectParameter {
  public:
    ...
    inline int toBool() const { return m_value > 0.0; }
    inline double defaultValue() const { return m_defaultValue; }
    inline void setDefaultValue(const double default_value) {m_defaultValue = default_value;}
    inline double minimum() const { return m_minimum;}
    inline void setMinimum(const double minimum) { m_minimum = minimum;}
    inline double maximum() const { return m_maximum; }
    inline void setMaximum(const double maximum) { m_maximum = maximum;}
  private:
