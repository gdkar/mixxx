          ...
          m_busRightHandle(registerChannelGroup("[BusRight]"))
{
    ...
    m_pAudioLatencyUsage = new ControlPotmeter(ConfigKey(group, "audio_latency_usage"),this, 0.0, 0.25);
    m_pAudioLatencyOverload  = new ControlPotmeter(ConfigKey(group, "audio_latency_overload"),this, 0.0, 1.0);
    ...
    auto default_bpm = pConfig->getValueString(ConfigKey("[InternalClock]", "bpm"),"124.0").toDouble();
    ControlObject::set(ConfigKey("[InternalClock]","bpm"),default_bpm);
    // Crossfader
    m_pCrossfader = new ControlPotmeter(ConfigKey(group, "crossfader"), this,-1., 1.);
    // Balance
    m_pBalance = new ControlPotmeter(ConfigKey(group, "balance"), this,-1., 1.);
    // Master gain
    m_pMasterGain = new ControlAudioTaperPot(ConfigKey(group, "gain"),this, -14, 14, 0.5);
    ...
    // Headphone volume
    m_pHeadGain = new ControlAudioTaperPot(ConfigKey(group, "headGain"),this, -14, 14, 0.5);
    ...
    for (auto o = int(EngineChannel::LEFT); o <= EngineChannel::RIGHT; ++o) {
...
{
    thread_local auto haveSetName = false;
    Trace t("EngineMaster::process");
    if (!haveSetName) {
#ifdef __LINUX__
        prctl(PR_SET_NAME, "Engine");
#endif
    ...
    auto iSampleRate = static_cast<uint32_t>(m_pMasterSampleRate->get());
    ...
    if (masterEnabled) {
        ...
        SampleUtil::copy(m_pMaster, m_pOutputBusBuffers[EngineChannel::LEFT], iBufferSize);
        SampleUtil::addWithGain(m_pMaster, m_pOutputBusBuffers[EngineChannel::CENTER],1, iBufferSize);
        SampleUtil::addWithGain(m_pMaster, m_pOutputBusBuffers[EngineChannel::RIGHT],1, iBufferSize);
        if (!m_pMasterTalkoverMix->toBool()) {
            SampleUtil::addWithGain(m_pMaster, m_pTalkover,1, iBufferSize);
        }
        // Process master channel effects
        if (m_pEngineEffectsManager) {
    ...
    if (headphoneEnabled) {
        ...
        // Head volume
        auto headphoneGain = CSAMPLE(m_pHeadGain->get());
