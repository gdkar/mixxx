...
class ReadAheadManager : public QObject{
  Q_OBJECT
  public:
    explicit ReadAheadManager(QObject *); // Only for testing: ReadAheadManagerMock
    explicit ReadAheadManager(CachingReader* reader,
                              LoopingControl* pLoopingControl, QObject*);
  ...
  private:
    ...
    struct ReadLogEntry {
        ...
        ReadLogEntry(double start,
                     double end)
        : virtualPlaypositionStart(start)
        , virtualPlaypositionEndNonInclusive(end)
        {
        }
