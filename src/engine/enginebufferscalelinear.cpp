...
CSAMPLE* EngineBufferScaleLinear::getScaled(unsigned long buf_size) {
    ...
    if (rate_add_new * rate_add_old < 0) {
        ...
        if (extra_samples > 0) {
            if (extra_samples % 2 != 0) extra_samples++;
            //qDebug() << "extra samples" << extra_samples;
            samples_read += m_pReadAheadManager->getNextSamples(rate_add_new, m_bufferInt, extra_samples);
        }
        // force a buffer read:
        m_bufferIntSize=0;
    ...
    m_samplesRead = samples_read;
    emit(samplesReadChanged(samples_read));
    return result;
}
/** Stretch a specified buffer worth of audio using linear interpolation */
CSAMPLE* EngineBufferScaleLinear::do_scale(CSAMPLE* buf, unsigned long buf_size, int* samples_read) {
    ...
    // Guard against buf_size == 0
    if (iRateLerpLength == 0) {return buf;}
    // We check for scratch condition in the public function, so this shouldn't
    // happen
    if (rate_add_new * rate_add_old < 0) {qDebug() << "ERROR: EBSL did not detect scratching correctly.";}
    ...
    if (m_dNextFrame - floor(m_dNextFrame) + frames - floor(frames) > 1.0) {unscaled_frames_needed++;}
    ...
    while (i < iRateLerpLength) {
        ...
        if (static_cast<int>(floor(m_dCurrentFrame)) * 2 + 1 < m_bufferIntSize && m_dCurrentFrame >= 0.0) {
            // take floor_sample form the buffer of the previous run
            floor_sample[0] = m_bufferInt[static_cast<int>( floor(m_dCurrentFrame)) * 2];
            floor_sample[1] = m_bufferInt[static_cast<int>( floor(m_dCurrentFrame)) * 2 + 1];
        } else {
        ...
        float rate_add = static_cast<float>(i) * (rate_add_diff) / static_cast<float>(iRateLerpLength) + rate_add_old;
        ...
        ceil_sample[0] = m_bufferInt[static_cast<int>(ceil(m_dCurrentFrame)) * 2];
        ceil_sample[1] = m_bufferInt[static_cast<int>(ceil(m_dCurrentFrame)) * 2 + 1];
        ...
        buf[i] = static_cast<float>(floor_sample[0]) + frac * (static_cast<float>(ceil_sample[0]) -
                 static_cast<float>(floor_sample[0]));
        buf[i + 1] = static_cast<float>(floor_sample[1]) + frac * (static_cast<float>(ceil_sample[1]) -
                     static_cast<float>(floor_sample[1]));
        m_floorSampleOld[0] = floor_sample[0];
        m_floorSampleOld[1] = floor_sample[1];
        // increment the index for the next loop
        m_dNextFrame = m_dCurrentFrame + (i < iRateLerpLength ? fabs(rate_add) : fabs(rate_add_new));
        i += 2 ;
    }
