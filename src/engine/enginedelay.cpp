...
EngineDelay::EngineDelay(const char* group, ConfigKey delayControl)
    ...
    m_pDelayPot.reset(new ControlPotmeter(delayControl, 0, kdMaxDelayPot, false, true, false, 0.0, true));
    m_pDelayPot->setDefaultValue(0);
    connect(m_pDelayPot.data(), SIGNAL(valueChanged(double)), this,
            SLOT(slotDelayChanged()), Qt::DirectConnection);

    m_pSampleRate.reset( new ControlObjectSlave(group, "samplerate", this));
