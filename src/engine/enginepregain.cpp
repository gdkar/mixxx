...
EnginePregain::EnginePregain(QString group,QObject *p)
    ...
    m_pPotmeterPregain->setParent(this);
    //Replay Gain things
...
void EnginePregain::process(CSAMPLE* pInOut, const int iBufferSize) {
    ...
    else
    {
        ...
        if (m_bSmoothFade)
        {
            auto seconds = static_cast<double>(m_timer.elapsed()) * 1e-9;
            if (seconds < kFadeSeconds)
            {
                // Fade smoothly
                auto fadeFrac = seconds / kFadeSeconds;
                fReplayGainCorrection = m_fPrevGain * (1.0 - fadeFrac) + fadeFrac * fullReplayGainBoost;
            }
            else
            {
                m_bSmoothFade = false;
        ...
        } else  fReplayGainCorrection = fullReplayGainBoost;
    }
    ...
    auto  kThresholdSpeed = CSAMPLE{0.070}; // Scale volume if playback speed is below 7%.
    if (std::abs(m_dSpeed) < kThresholdSpeed) totalGain *= std::abs(m_dSpeed) / kThresholdSpeed;
    ...
    } else  SampleUtil::applyGain(pInOut, totalGain, iBufferSize);
}
