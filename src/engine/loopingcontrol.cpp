...
{
    return ConfigKey(group,ctrlName.arg(num));
}
// static
QList<double> LoopingControl::getBeatSizes() {
    auto result = QList<double>{};
    for (auto i = decltype(sizeof(s_dBeatSizes)){0}; i < (sizeof(s_dBeatSizes) / sizeof(s_dBeatSizes[0])); ++i) {
        result.append(s_dBeatSizes[i]);
    }
    return result;
}
LoopingControl::LoopingControl(QString group,ConfigObject<ConfigValue>* _config,QObject*p)
        : EngineControl(group, _config,p)
{
    ...
    m_pLoopInButton = new ControlPushButton(ConfigKey(group, "loop_in"),false,this);
    connect(m_pLoopInButton, SIGNAL(valueChanged(double)),this, SLOT(slotLoopIn(double)),Qt::DirectConnection);
    m_pLoopInButton->set(0);
    m_pLoopOutButton = new ControlPushButton(ConfigKey(group, "loop_out"),false,this);
    connect(m_pLoopOutButton, SIGNAL(valueChanged(double)),this, SLOT(slotLoopOut(double)),Qt::DirectConnection);
    m_pLoopOutButton->set(0);
    m_pLoopExitButton = new ControlPushButton(ConfigKey(group, "loop_exit"),false,this);
    connect(m_pLoopExitButton, SIGNAL(valueChanged(double)),this, SLOT(slotLoopExit(double)),Qt::DirectConnection);
    m_pLoopExitButton->set(0);
    m_pReloopExitButton = new ControlPushButton(ConfigKey(group, "reloop_exit"),false,this);
    connect(m_pReloopExitButton, SIGNAL(valueChanged(double)),this, SLOT(slotReloopExit(double)),Qt::DirectConnection);
    m_pReloopExitButton->set(0);
    m_pCOLoopEnabled = new ControlObject(ConfigKey(group, "loop_enabled"),this);
    m_pCOLoopEnabled->set(0.0);
    m_pCOLoopStartPosition =new ControlObject(ConfigKey(group, "loop_start_position"),this);
    m_pCOLoopStartPosition->set(kNoTrigger);
    connect(m_pCOLoopStartPosition, SIGNAL(valueChanged(double)),this, SLOT(slotLoopStartPos(double)),Qt::DirectConnection);
    m_pCOLoopEndPosition =new ControlObject(ConfigKey(group, "loop_end_position"),this);
    m_pCOLoopEndPosition->set(kNoTrigger);
    connect(m_pCOLoopEndPosition, SIGNAL(valueChanged(double)),this, SLOT(slotLoopEndPos(double)),Qt::DirectConnection);
    ...
    m_pCOBeatLoop->setParent(this);
    connect(m_pCOBeatLoop, SIGNAL(valueChanged(double)), this,SLOT(slotBeatLoop(double)), Qt::DirectConnection);
    // Here we create corresponding beatloop_(SIZE) CO's which all call the same
    // BeatControl, but with a set value.
    for (auto i = decltype(sizeof(s_dBeatSizes)){0}; i < (sizeof(s_dBeatSizes) / sizeof(s_dBeatSizes[0])); ++i) {
        auto  pBeatLoop = new BeatLoopingControl(group, s_dBeatSizes[i],this);
        connect(pBeatLoop, SIGNAL(activateBeatLoop(BeatLoopingControl*)),this, SLOT(slotBeatLoopActivate(BeatLoopingControl*)),Qt::DirectConnection);
        connect(pBeatLoop, SIGNAL(activateBeatLoopRoll(BeatLoopingControl*)),this, SLOT(slotBeatLoopActivateRoll(BeatLoopingControl*)),Qt::DirectConnection);
        connect(pBeatLoop, SIGNAL(deactivateBeatLoop(BeatLoopingControl*)),this, SLOT(slotBeatLoopDeactivate(BeatLoopingControl*)),Qt::DirectConnection);
        connect(pBeatLoop, SIGNAL(deactivateBeatLoopRoll(BeatLoopingControl*)),this, SLOT(slotBeatLoopDeactivateRoll(BeatLoopingControl*)),Qt::DirectConnection);
        m_beatLoops.append(pBeatLoop);
    }
    m_pCOBeatJump = new ControlObject(ConfigKey(group, "beatjump"), false);
    m_pCOBeatJump->setParent(this);
    connect(m_pCOBeatJump, SIGNAL(valueChanged(double)), this, SLOT(slotBeatJump(double)), Qt::DirectConnection);
    // Create beatjump_(SIZE) CO's which all call beatjump, but with a set
    // value.
    for (auto i = decltype(sizeof(s_dBeatSizes)){0}; i < (sizeof(s_dBeatSizes) / sizeof(s_dBeatSizes[0])); ++i) {
        auto pBeatJump = new BeatJumpControl(group, s_dBeatSizes[i],this);
        connect(pBeatJump, SIGNAL(beatJump(double)),this, SLOT(slotBeatJump(double)),Qt::DirectConnection);
        m_beatJumps.append(pBeatJump);
    }
    m_pCOLoopMove = new ControlObject(ConfigKey(group, "loop_move"), false);
    m_pCOLoopMove->setParent(this);
    connect(m_pCOLoopMove, SIGNAL(valueChanged(double)),this, SLOT(slotLoopMove(double)), Qt::DirectConnection);
    // Create loop_move_(SIZE) CO's which all call loop_move, but with a set
    // value.
    for (auto i = decltype(sizeof(s_dBeatSizes)){0}; i < (sizeof(s_dBeatSizes) / sizeof(s_dBeatSizes[0])); ++i) {
        auto pLoopMove = new LoopMoveControl(group, s_dBeatSizes[i],this);
        connect(pLoopMove, SIGNAL(loopMove(double)),this, SLOT(slotLoopMove(double)),Qt::DirectConnection);
        m_loopMoves.append(pLoopMove);
    }
    m_pCOLoopScale = new ControlObject(ConfigKey(group, "loop_scale"), false);
    m_pCOLoopScale->setParent(this);
    connect(m_pCOLoopScale, SIGNAL(valueChanged(double)),this, SLOT(slotLoopScale(double)));
    m_pLoopHalveButton = new ControlPushButton(ConfigKey(group, "loop_halve"),false,this);
    connect(m_pLoopHalveButton, SIGNAL(valueChanged(double)),this, SLOT(slotLoopHalve(double)));
    m_pLoopDoubleButton = new ControlPushButton(ConfigKey(group, "loop_double"),false,this);
...
void LoopingControl::slotLoopScale(double scale) {
    ...
    auto loop_length = m_iLoopEndSample - m_iLoopStartSample;
    auto old_loop_end = m_iLoopEndSample;
    auto samples = m_pTrackSamples->get();
    loop_length *= scale;
    ...
    if (!even(m_iLoopEndSample)) m_iLoopEndSample--;
    ...
    if (m_iLoopEndSample == m_iLoopStartSample) {
        if ((m_iLoopEndSample+2) >= samples)  m_iLoopStartSample -= 2;
        else                                  m_iLoopEndSample   += 2;
    }
    // Do not allow loops to go past the end of the song
    else if (m_iLoopEndSample > samples) m_iLoopEndSample = samples;
    ...
    if (m_bLoopingEnabled && scale < 1.0)
        seekInsideAdjustedLoop(m_iLoopStartSample, old_loop_end,m_iLoopStartSample, m_iLoopEndSample);
}
void LoopingControl::slotLoopHalve(double v) {
    ...
    if (v > 0.0) {
        ...
        if (m_pActiveBeatLoop) {
            auto active_index = m_beatLoops.indexOf(m_pActiveBeatLoop);
            if (active_index - 1 >= 0)
            {
                if (m_bLoopingEnabled)
                {
                    ...
                    // it fits.
                    auto old_loop_in = m_iLoopStartSample;
                    auto old_loop_out = m_iLoopEndSample;
                    slotBeatLoopActivate(m_beatLoops[active_index - 1]);
                    seekInsideAdjustedLoop(old_loop_in, old_loop_out,m_iLoopStartSample, m_iLoopEndSample);
                } 
                else
                {
...
void LoopingControl::slotLoopDouble(double v)
{
    ...
    if (v > 0.0)
    {
        ...
        if (m_pActiveBeatLoop)
        {
            ...
            {
                if (m_bLoopingEnabled) slotBeatLoopActivate(m_beatLoops[active_index + 1]);
                else
                {
                    // Calling scale clears the active beatloop.
                    slotLoopScale(2.0);
        ...
        else slotLoopScale(2.0);
    }
}
double LoopingControl::process(double dRate,double currentSample,double totalSamples,int iBufferSize) {
    Q_UNUSED(totalSamples);
    Q_UNUSED(iBufferSize);
    m_iCurrentSample = currentSample;
    if (!even(m_iCurrentSample)) m_iCurrentSample--;
    auto reverse = dRate < 0;
    auto retval = static_cast<double>(kNoTrigger);
    if (m_bLoopingEnabled && m_iLoopStartSample != kNoTrigger && m_iLoopEndSample != kNoTrigger)
    {
        if( currentSample >= m_iLoopEndSample || currentSample <= m_iLoopStartSample)
...
double LoopingControl::nextTrigger(double dRate,double currentSample,double totalSamples,int iBufferSize) {
    Q_UNUSED(currentSample);
    Q_UNUSED(totalSamples);
    Q_UNUSED(iBufferSize);
    auto bReverse = dRate < 0;
    if (m_bLoopingEnabled) {
        if (bReverse) return m_iLoopStartSample;
        else          return m_iLoopEndSample;
    }
    return kNoTrigger;
}

double LoopingControl::getTrigger(double dRate,double currentSample,double totalSamples,int iBufferSize) {
    Q_UNUSED(currentSample);
    Q_UNUSED(totalSamples);
    Q_UNUSED(iBufferSize);
    auto bReverse = dRate < 0;
    if (m_bLoopingEnabled) {
        if (bReverse) return m_iLoopEndSample;
        else          return m_iLoopStartSample;
    }
    return kNoTrigger;
}
...
void LoopingControl::slotLoopIn(double val) {
    ...
    if (val)
    {
        ...
        auto pos = (m_pQuantizeEnabled->get() > 0.0 && m_pClosestBeat->get() != -1) ?
        ...
        if (m_bLoopingEnabled && (m_iLoopEndSample - pos) < MINIMUM_AUDIBLE_LOOP_SIZE)
        {
...
void LoopingControl::slotLoopOut(double val) {
    ...
    if (val)
    {
        ...
        if (m_iLoopStartSample == kNoTrigger || pos < m_iLoopStartSample) return;
        ...
        if (pos - m_iLoopStartSample < MINIMUM_AUDIBLE_LOOP_SIZE)
        {
            ...
            if (m_pQuantizeEnabled->get() > 0.0 && m_pBeats)
            {
                // 1 would have just returned loop_in, so give 2 to get the beat
                // following loop_in
                auto nextbeat = static_cast<int>(m_pBeats->findNthBeat(m_iLoopStartSample, 2));
                pos += (nextbeat - pos) * s_dBeatSizes[0];
            } 
            else  pos += MINIMUM_AUDIBLE_LOOP_SIZE;
        }
        if (pos != -1 && !even(pos)) pos++;  // Increment to avoid shortening too-short loops
        clearActiveBeatLoop();
        //set loop out position
        m_iLoopEndSample = pos;
        m_pCOLoopEndPosition->set(m_iLoopEndSample);
        // start looping
        if (m_iLoopStartSample != -1 && m_iLoopEndSample != -1) setLoopingEnabled(true);
    }
}
void LoopingControl::slotLoopExit(double val) {
    ...
    if (val) {
        ...
        if (m_bLoopingEnabled) setLoopingEnabled(false);
    }
}
void LoopingControl::slotReloopExit(double val) {
    ...
    if (val)
    {
        ...
        else
        {
            ...
            if (m_iLoopStartSample != -1 && m_iLoopEndSample != -1 && m_iLoopStartSample <= m_iLoopEndSample)
            {
...
void LoopingControl::slotLoopStartPos(double pos) {
    if (!m_pTrack) return;
    auto newpos = static_cast<int>(pos);
    if (newpos != -1 && !even(newpos))  newpos--;
    if (m_iLoopStartSample == newpos)  return;
    clearActiveBeatLoop();
...
void LoopingControl::setLoopingEnabled(bool enabled) {
    ...
    if (m_pActiveBeatLoop) {
        if (enabled) m_pActiveBeatLoop->activate();
        else         m_pActiveBeatLoop->deactivate();
    }
}
...
{
    Q_UNUSED(pTrack);
    if (m_pTrack) disconnect(m_pTrack.data(), SIGNAL(beatsUpdated()),this, SLOT(slotUpdatedTrackBeats()));
    m_pTrack.clear();
    m_pBeats.clear();
    clearActiveBeatLoop();
}
...
{
    if (m_pTrack) m_pBeats = m_pTrack->getBeats();
}
...
void LoopingControl::slotBeatLoop(double beats, bool keepStartPoint) {
    ...
    if (beats > 0)
    {
        if (keepStartPoint) loop_in = m_iLoopStartSample;
        else 
        {
            ...
            if (m_pQuantizeEnabled->get() > 0.0 && prevBeat != -1) {
                if (beats >= 1.0) loop_in = prevBeat;
                    ...
                    auto beat_len = nextBeat - prevBeat;
                    auto loops_per_beat = 1.0 / beats;
                    auto beat_pos = cur_pos - prevBeat;
                    auto beat_frac = static_cast<int>(std::floor((beat_pos / beat_len) * loops_per_beat));
                    loop_in = prevBeat + beat_len / loops_per_beat * beat_frac;
                }

            } else  loop_in = floor(cur_pos);
            if (!even(loop_in)) loop_in--;
        }
        auto  fullbeats = static_cast<int>(beats);
        auto fracbeats = beats - static_cast<double>(fullbeats);
        ...
        if (fullbeats > 0) {
            ...
            auto this_beat = m_pBeats->findNthBeat(loop_in, 1);
            auto nth_beat = m_pBeats->findNthBeat(loop_in, 1 + fullbeats);
        ...
        if (fracbeats > 0) {
            ...
            // TODO: figure out how to convert this to a findPrevNext call.
            auto loop_out_beat = m_pBeats->findNthBeat(loop_out, 1);
            auto loop_out_next_beat = m_pBeats->findNthBeat(loop_out, 2);
            loop_out += (loop_out_next_beat - loop_out_beat) * fracbeats;
        }
    }
    if ((loop_in == -1) || (loop_out == -1)) return;
    if (!even(loop_in))  loop_in--;
    if (!even(loop_out)) loop_out--;
    if (loop_in == loop_out) {
        if ((loop_out+2) > samples) loop_in -= 2;
        else                        loop_out += 2;
    } else if (loop_out > samples) loop_out = samples;
...
{
    if (!m_pTrack || !m_pBeats)  return;
    auto dPosition = getCurrentSample();
    auto dBeatLength = 0.0;
    if (BpmControl::getBeatContext(m_pBeats, dPosition,nullptr, nullptr, &dBeatLength, nullptr))
    {
...
void LoopingControl::slotLoopMove(double beats)
{
    ...
    if (BpmControl::getBeatContext(m_pBeats, dPosition, nullptr, nullptr, &dBeatLength, nullptr))
    {
        auto old_loop_in   = m_iLoopStartSample;
        auto old_loop_out  = m_iLoopEndSample;
        auto new_loop_in   = static_cast<decltype(old_loop_in)>(old_loop_in + (beats * dBeatLength));
        auto new_loop_out  = static_cast<decltype(old_loop_out)>(old_loop_out + (beats * dBeatLength));
        if (!even(new_loop_in))  --new_loop_in;
        if (!even(new_loop_out))  --new_loop_out;
        ...
        if (m_bLoopingEnabled) seekInsideAdjustedLoop(old_loop_in, old_loop_out,new_loop_in, new_loop_out);
    }
}

void LoopingControl::seekInsideAdjustedLoop(int old_loop_in, int old_loop_out, int new_loop_in, int new_loop_out) {
    if (m_iCurrentSample >= new_loop_in && m_iCurrentSample <= new_loop_out) return;
    auto new_loop_size = new_loop_out - new_loop_in;
    if (!even(new_loop_size)) --new_loop_size;
    ...
    auto adjusted_position = m_iCurrentSample;
...
BeatJumpControl::BeatJumpControl(QString group, double size, QObject *p)
        : QObject(p),
          m_dBeatJumpSize(size)
{
    m_pJumpForward = new ControlPushButton(keyForControl(group, "beatjump_%1_forward", size),false,this);
    connect(m_pJumpForward, SIGNAL(valueChanged(double)),this, SLOT(slotJumpForward(double)),Qt::DirectConnection);
    m_pJumpBackward = new ControlPushButton( keyForControl(group, "beatjump_%1_backward", size),false,this);
    connect(m_pJumpBackward, SIGNAL(valueChanged(double)),this, SLOT(slotJumpBackward(double)),Qt::DirectConnection);
}
BeatJumpControl::~BeatJumpControl() = default;
void BeatJumpControl::slotJumpBackward(double v)
{
    if (v > 0) emit(beatJump(-m_dBeatJumpSize));
}
void BeatJumpControl::slotJumpForward(double v)
{
    if (v > 0)  emit(beatJump(m_dBeatJumpSize));
}
LoopMoveControl::LoopMoveControl(QString group, double size,QObject*p)
        : QObject(p),
          m_dLoopMoveSize(size) {
    m_pMoveForward = new ControlPushButton( keyForControl(group, "loop_move_%1_forward", size),false,this);
    connect(m_pMoveForward, SIGNAL(valueChanged(double)),this, SLOT(slotMoveForward(double)),Qt::DirectConnection);
    m_pMoveBackward = new ControlPushButton(keyForControl(group, "loop_move_%1_backward", size),false,this);
    connect(m_pMoveBackward, SIGNAL(valueChanged(double)), this, SLOT(slotMoveBackward(double)), Qt::DirectConnection);
}
...
BeatLoopingControl::BeatLoopingControl(QString group, double size, QObject *p)
        : QObject(p),
          m_dBeatLoopSize(size)
{
    ...
    m_pLegacy = new ControlPushButton( keyForControl(group, "beatloop_%1", size),false,this);
    m_pLegacy->setButtonMode(ControlPushButton::TOGGLE);
    connect(m_pLegacy, SIGNAL(valueChanged(double)),this, SLOT(slotLegacy(double)),Qt::DirectConnection);
    // A push-button which activates the beatloop.
    m_pActivate = new ControlPushButton(keyForControl(group, "beatloop_%1_activate", size),false,this);
    connect(m_pActivate, SIGNAL(valueChanged(double)),this, SLOT(slotActivate(double)),Qt::DirectConnection);
    // A push-button which toggles the beatloop as active or inactive.
    m_pToggle = new ControlPushButton(keyForControl(group, "beatloop_%1_toggle", size),false,this);
    connect(m_pToggle, SIGNAL(valueChanged(double)),this, SLOT(slotToggle(double)),Qt::DirectConnection);
    // A push-button which activates rolling beatloops
    m_pActivateRoll = new ControlPushButton(keyForControl(group, "beatlooproll_%1_activate", size),false,this);
    connect(m_pActivateRoll, SIGNAL(valueChanged(double)),this, SLOT(slotActivateRoll(double)),Qt::DirectConnection);
...
BeatLoopingControl::~BeatLoopingControl() = default;
...
void BeatLoopingControl::slotLegacy(double v) {
    ...
    if (v > 0)  emit(activateBeatLoop(this));
    else  emit(deactivateBeatLoop(this));
}
...
void BeatLoopingControl::slotActivateRoll(double v) {
    ...
    if (v > 0) emit(activateBeatLoopRoll(this));
    else       emit(deactivateBeatLoopRoll(this));
}
void BeatLoopingControl::slotToggle(double v) {
    //qDebug() << "slotToggle" << m_dBeatLoopSize << "v" << v;
    if (!v)  return;
    if (m_bActive)  emit(deactivateBeatLoop(this));
    else            emit(activateBeatLoop(this));
}
double BeatLoopingControl::getSize() const
{
