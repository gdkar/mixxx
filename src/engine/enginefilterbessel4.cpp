...
EngineFilterBessel4Low::EngineFilterBessel4Low(int sampleRate,double freqCorner1,QObject *pParent)
:EngineFilterIIR<4,IIR_LP>(pParent){
...
int EngineFilterBessel4Low::setFrequencyCornersForIntDelay(double desiredCorner1Ratio, int maxDelay) {
    ...
    if (iDelay >= (int)(sizeof(delayRatioTable) / sizeof(double))) {
        ...
        quantizedRatio = (-(iDelay / kDelayFactor2 / 2)) + std::sqrt((iDelay / kDelayFactor2 / 2)*(iDelay / kDelayFactor2 / 2) + kDelayFactor1 / kDelayFactor2);
    } else {quantizedRatio = delayRatioTable[iDelay];}
    setCoefs("LpBe4", 1, quantizedRatio);
    return iDelay;
}
EngineFilterBessel4Band::EngineFilterBessel4Band(int sampleRate,double freqCorner1,double freqCorner2,QObject*p)
:EngineFilterIIR<8,IIR_BP>(p)
{
    setFrequencyCorners(sampleRate, freqCorner1, freqCorner2);
}
void EngineFilterBessel4Band::setFrequencyCorners(int sampleRate,double freqCorner1,double freqCorner2) {
    setCoefs("BpBe4", sampleRate, freqCorner1, freqCorner2);
}
EngineFilterBessel4High::EngineFilterBessel4High(int sampleRate,double freqCorner1,QObject *p)
:EngineFilterIIR<4,IIR_HP>(p)
{
    setFrequencyCorners(sampleRate, freqCorner1);
}
void EngineFilterBessel4High::setFrequencyCorners(int sampleRate,double freqCorner1) {
