...
double EngineBuffer::fractionalPlayposFromAbsolute(double absolutePlaypos) {
    double dFractionalPlaypos = 0.0;
    if (m_trackSamplesOld != 0.) {
        dFractionalPlaypos = math_min<double>(absolutePlaypos, m_trackSamplesOld);
        dFractionalPlaypos /= m_trackSamplesOld;
    }
    return dFractionalPlaypos;
}
...
void EngineBuffer::requestSyncPhase() {m_iSeekQueued = SEEK_PHASE;}
void EngineBuffer::requestEnableSync(bool enabled) {
    ...
    SyncRequestQueued enable_request = static_cast<SyncRequestQueued>(m_iEnableSyncQueued.load());
    if (enabled) {
...
void EngineBuffer::process(CSAMPLE* pOutput, const int iBufferSize) {
    ...
    if (!bTrackLoading && m_pause.tryLock()) {
        ...
        double speed = m_pRateControl->calculateSpeed(baserate, tempoRatio, paused, iBufferSize, &is_scratching, &is_reverse);
        ...
        } else {SampleUtil::clear(pOutput, iBufferSize);}
