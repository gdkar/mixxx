...
EngineBuffer::EngineBuffer(QObject *p, QString group, UserSettingsPointer pConfig,
        ...
        : EngineObject(p),
          ...
          m_startButton(NULL),
          m_endButton(NULL),
          m_iSeekQueued({0.,SeekRequests(SeekRequest::None)}),
          ...
          m_iLastBufferSize(0)
{
    ...
    // Play button
    auto button = new ControlPushButton(ConfigKey(m_group, "play"),this);
    m_playButton = button;
    ...
    m_playStartButton = new ControlObject(ConfigKey(m_group, "start_play"),this);
    ...
    m_stopStartButton = new ControlObject(ConfigKey(m_group, "start_stop"),this);
    ...
    m_stopButton = new ControlObject(ConfigKey(m_group, "stop"),this);
    connect(m_stopButton,&ControlObject::valueChanged
      , this, [this](double v) { setPlaying(!v);});

    // Start button
    m_startButton = button = new ControlPushButton(ConfigKey(m_group, "start"),this);
    ...
    // End button
    m_endButton = new ControlObject(ConfigKey(m_group, "end"),this);
    connect(m_endButton, &ControlObject::valueChanged
      , this, [this](double v) { if(v){doSeekFractional(1., SeekRequest::Exact);}});


    m_pSlipButton = button = new ControlPushButton(ConfigKey(m_group, "slip_enabled"),this);
    button->setButtonMode(ControlPushButton::TOGGLE);
    ...
    m_pRepeat = button = new ControlPushButton(ConfigKey(m_group, "repeat"),this);
    button->setButtonMode(ControlPushButton::TOGGLE);
    ...
    m_pKeylockEngine->connectValueChanged(SLOT(slotKeylockEngineChanged(double)),Qt::AutoConnection);
    ...
    m_pKeylock = button = new ControlPushButton(ConfigKey(m_group, "keylock"), this,true);
    button->setButtonMode(ControlPushButton::TOGGLE);

    m_pEject = new ControlObject(ConfigKey(m_group, "eject"),this);
    connect(m_pEject, SIGNAL(valueChanged(double)),this, SLOT(slotEjectTrack(double)),Qt::AutoConnection);
    ...
    auto keylock = static_cast<int>(m_pKeylockEngine->get());
    if (static_cast<KeylockEngine>(keylock) == KeylockEngine::SoundTouch) {
...
void EngineBuffer::queueNewPlaypos(double newpos, enum SeekRequest seekType) {
    ...
    // condition effect
    DEBUG_ASSERT_AND_HANDLE(seekType != SeekRequest::Phase) {
        // SEEK_PHASE with a position is not supported
        // use SEEK_STANDARD for that
        seekType = SeekRequest::Standard;
    }
    // set m_queuedPosition valid
    m_iSeekQueued.store(SeekRecord { newpos, seekType});
}
...
void EngineBuffer::requestEnableSync(bool enabled)
{
    ...
    auto enable_request = static_cast<SyncRequest>(m_iEnableSyncQueued.load());
    if (enabled) {
        m_iEnableSyncQueued = SyncRequest::Enable;
    } else {
        ...
        if (enable_request == SyncRequest::Enable) {
            m_iEnableSyncQueued = SyncRequest::Enable_Disable;
        } else {
...
{
    doSeekPlayPos(playPosition, SeekRequest::Standard);
}

// WARNING: This method runs from SyncWorker and Engine Worker
void EngineBuffer::slotControlSeekExact(double playPosition)
{
    doSeekPlayPos(playPosition, SeekRequest::Exact);
}
...
bool EngineBuffer::updateIndicatorsAndModifyPlay(bool newPlay) {
    ...
    if ((!m_pCurrentTrack && m_iTrackLoading.load() == 0) ||
            (m_pCurrentTrack && m_iTrackLoading.load() == 0 &&
             ...
             (SeekRequests(m_iSeekQueued.load().req) == SeekRequests(SeekRequest::None)))) {
        // play not possible
        playPossible = false;
    }
...
{
    if (v > 0.0) {
        doSeekFractional(1., SeekRequest::Exact);
    }
}
...
{
    auto enable_request = static_cast<SyncRequest>(
                    m_iEnableSyncQueued.exchange(SyncRequest::None,std::memory_order_release));
    auto mode_request =
            static_cast<SyncMode>(m_iSyncModeQueued.exchange(SYNC_INVALID,std::memory_order_release));
    ...
    case SyncRequest::Disable:
        ...
        break;
    case SyncRequest::Enable_Disable:
        m_pEngineSync->requestEnableSync(m_pSyncControl, true);
        m_pEngineSync->requestEnableSync(m_pSyncControl, false);
        break;
...
void EngineBuffer::processSeek(bool paused)
{
    ...
    // call anyway again.
    auto rec = m_iSeekQueued.exchange(SeekRecord{0.,SeekRequest::None});
    auto seekType = rec.req;
    auto position = rec.pos;

    // Add SEEK_PHASE bit, if any
    if (m_iSeekPhaseQueued.exchange(0,std::memory_order_release)) {
        seekType |= SeekRequest::Phase;
    }

    switch (seekType) {
        case static_cast<int>(SeekRequest::None):
            return;
        case static_cast<int>(SeekRequest::Phase):
            // only adjust phase
            position = m_filepos_play;
            break;
        case static_cast<int>(SeekRequest::Exact):
        case static_cast<int>(SeekRequest::Standard): // = SEEK_EXACT | SEEK_PHASE
    ...
    if ((seekType & SeekRequest::Phase) && !paused && m_pQuantize->toBool()) {
