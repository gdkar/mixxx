...
InternalClock::InternalClock(const char* pGroup, SyncableListener* pEngineSync, QObject*pParent)
        : QObject(pParent),
          m_group(pGroup),
          ...
          m_dClockPosition(0)
{
    ...
    m_pClockBpm=new ControlLinPotmeter(ConfigKey(m_group, "bpm"),1, 200, 1, 0.1, true);
    m_pClockBpm->setParent(this);
    connect(m_pClockBpm, SIGNAL(valueChanged(double)),this, SLOT(slotBpmChanged(double)),Qt::DirectConnection);
    m_pClockBeatDistance=(new ControlObject(ConfigKey(m_group, "beat_distance"),this));
    connect(m_pClockBeatDistance, SIGNAL(valueChanged(double)),this, SLOT(slotBeatDistanceChanged(double)),Qt::DirectConnection);
    m_pSyncMasterEnabled=(new ControlPushButton(ConfigKey(pGroup, "sync_master"),false,this));
...
void InternalClock::onCallbackEnd(int sampleRate, int bufferSize)
{
    ...
    while (m_dClockPosition >= m_dBeatLength) m_dClockPosition -= m_dBeatLength;
    auto beat_distance = getBeatDistance();
    m_pClockBeatDistance->set(beat_distance);
    m_pEngineSync->notifyBeatDistanceChanged(this, beat_distance);
}
QString InternalClock::getGroup() const
{
  return m_group;
}
EngineChannel * InternalClock::getChannel() const
{
  return nullptr;
}
SyncMode InternalClock::getSyncMode() const
{
  return m_mode;
}
bool InternalClock::isPlaying() const
{
  return false;
}
