...
SyncControl::SyncControl(QString group, ConfigObject<ConfigValue>* pConfig,
    ...
    // stopped.
    m_pPlayButton.reset(new ControlObject(ConfigKey(group, "play"), this));
    ...
    m_pSyncMode->setProperty("buttonMode",QVariant::fromValue(ControlPushButton::ButtonMode::Toggle));
    m_pSyncMode->setProperty("numStates",SYNC_NUM_MODES);
    m_pSyncMode->connectValueChangeRequest(this, SLOT(slotSyncModeChangeRequest(double)), Qt::DirectConnection);
    m_pSyncMasterEnabled.reset(new ControlPushButton(ConfigKey(group, "sync_master"),this));
    ...
    m_pSyncEnabled.reset(new ControlPushButton(ConfigKey(group, "sync_enabled"),this));
    m_pSyncEnabled->setProperty("buttonMode",QVariant::fromValue(ControlPushButton::ButtonMode::Latching));
    ...
    m_pPassthroughEnabled.reset(new ControlObject(ConfigKey(group, "passthrough"), this));
    m_pPassthroughEnabled->connectValueChanged(this, SLOT(slotPassthroughChanged(double)),Qt::DirectConnection);

    m_pEjectButton.reset(new ControlObject(ConfigKey(group, "eject"), this));
...
void SyncControl::setEngineControls(RateControl* pRateControl,BpmControl* pBpmControl) {
    ...
    m_pBpm.reset(new ControlObject(ConfigKey(getGroup(), "bpm"), this));
    m_pLocalBpm.reset(new ControlObject(ConfigKey(getGroup(), "local_bpm"), this));
    m_pFileBpm.reset(new ControlObject(ConfigKey(getGroup(), "file_bpm"), this));
    m_pFileBpm->connectValueChanged(this, SLOT(slotFileBpmChanged()),Qt::DirectConnection);
    m_pRateSlider.reset(new ControlObject(ConfigKey(getGroup(), "rate"), this));
    m_pRateSlider->connectValueChanged(this, SLOT(slotRateChanged()),Qt::DirectConnection);
    m_pRateDirection.reset(new ControlObject(ConfigKey(getGroup(), "rate_dir"), this));
    m_pRateDirection->connectValueChanged(this, SLOT(slotRateChanged()),Qt::DirectConnection);
    m_pRateRange.reset(new ControlObject(ConfigKey(getGroup(), "rateRange"), this));
    m_pRateRange->connectValueChanged(this, SLOT(slotRateChanged()),Qt::DirectConnection);
    m_pSyncPhaseButton.reset(new ControlObject(ConfigKey(getGroup(), "beatsync_phase"), this));
#ifdef __VINYLCONTROL__
    m_pVCEnabled.reset(new ControlObject(ConfigKey(getGroup(), "vinylcontrol_enabled"), this));
    ...
    m_pVCEnabled->connectValueChanged(this, SLOT(slotVinylControlChanged(double)),Qt::DirectConnection);
#endif
}

SyncMode SyncControl::getSyncMode() const {return syncModeFromDouble(m_pSyncMode->get()); }
...
double SyncControl::getBeatDistance() const {
    ...
    if (m_masterBpmAdjustFactor == kBpmDouble) {
        ...
        if (m_unmultipliedTargetBeatDistance >= 0.5) {beatDistance += 0.5;}
    } else if (m_masterBpmAdjustFactor == kBpmHalve) {
        if (beatDistance >= 0.5) {beatDistance -= 0.5;}
        beatDistance /= kBpmHalve;
    }
    return beatDistance;
}
double SyncControl::getBaseBpm() const {return m_pLocalBpm->get();}
...
double SyncControl::determineBpmMultiplier(double myBpm, double targetBpm) const {
    auto multiplier = kBpmUnity;
    auto best_margin = fabs((targetBpm / myBpm) - 1.0);
    auto try_margin = fabs((targetBpm * kBpmHalve / myBpm) - 1.0);
    ...
    if (best_margin - try_margin > .0001) {multiplier = kBpmDouble;}
    return multiplier;
}
void SyncControl::updateTargetBeatDistance() {
    auto targetDistance = m_unmultipliedTargetBeatDistance;
    ...
    if (m_masterBpmAdjustFactor == kBpmDouble) {
        ...
        if (m_beatDistance >= 0.5) {targetDistance += 0.5;}
    }
...
void SyncControl::trackLoaded(TrackPointer pTrack) {
    ...
    if (getSyncMode() != SYNC_NONE) {
        ...
        auto dRate = 1.0 + m_pRateDirection->get() * m_pRateRange->get() * m_pRateSlider->get();
...
void SyncControl::setLocalBpm(double local_bpm) {
    ...
    const auto rate = 1.0 + m_pRateSlider->get() * m_pRateRange->get() * m_pRateDirection->get();
    auto  bpm = local_bpm * rate;
    m_pBpm->set(bpm);
