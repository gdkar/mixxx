        ...
        , processSample(nullptr)
{
    ...
    {
    case PREDEF_BP:
        processSample = &EngineFilter::processSampleBp;
        fbuf1 = buf1;
        fbuf2 = buf2;
        break;
    case PREDEF_HP:
        processSample = &EngineFilter::processSampleHp;
        fbuf1 = buf1;
        fbuf2 = buf2;
        break;
    case PREDEF_LP:
        processSample = &EngineFilter::processSampleLp;
        fbuf1 = buf1;
        fbuf2 = buf2;
        break;
    default:
...
void EngineFilter::process(CSAMPLE* pInOut, const int iBufferSize)
{
    ...
    {
        pInOut[i] = (CSAMPLE) processSample(fbuf1,  pInOut[i]);
        pInOut[i + 1] = (CSAMPLE) processSample(fbuf2, pInOut[i + 1]);
    }
}
// delegate to function pointer
/* static */ CSAMPLE EngineFilter::processSampleDynamic( void *voidp, const CSAMPLE sample )
{
  FidData *bufIn = reinterpret_cast<FidData *>(voidp);
  return static_cast<CSAMPLE>(bufIn->funcp ( bufIn->bufp, static_cast<double>(sample) ) );
}

// 250Hz-3Khz Butterworth
/* static */ CSAMPLE EngineFilter::processSampleBp(void *bufIn, const CSAMPLE sample)
{
    CSAMPLE *buf = reinterpret_cast<CSAMPLE*>( bufIn );
    CSAMPLE val = sample;
    CSAMPLE  tmp, fir, iir;
...
//3Khz butterworth
/* static */ CSAMPLE EngineFilter::processSampleHp(void *bufIn, const CSAMPLE sample)
{
    CSAMPLE *buf = reinterpret_cast<CSAMPLE*>( bufIn);
    CSAMPLE  val = sample;
    CSAMPLE  tmp, fir, iir;
...
/* static */ CSAMPLE  EngineFilter::processSampleLp(void *bufIn, const CSAMPLE sample)
{
   CSAMPLE *buf = reinterpret_cast<CSAMPLE*>(bufIn);
   CSAMPLE val = sample;
   CSAMPLE  tmp, fir, iir;
