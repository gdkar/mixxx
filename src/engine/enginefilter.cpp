...
void EngineFilter::process(CSAMPLE* pInOut, const int iBufferSize){
    ...
    for(i = 0; i < iBufferSize; i += 2){
        pInOut[i] = (CSAMPLE) processSample(fdat1, fbuf1,  pInOut[i]);
        pInOut[i + 1] = (CSAMPLE) processSample(fdat2, fbuf2,  pInOut[i + 1]);
    }
}

CSAMPLE processSampleDynamic(void*dat, void *bufIn, const CSAMPLE sample){
  double val = (double)sample;
  FidFunc *funcp = reinterpret_cast<FidFunc*>(dat); 
  return (CSAMPLE)funcp(bufIn,val);
}
// 250Hz-3Khz Butterworth
CSAMPLE processSampleBp(void*,void *bufIn, const CSAMPLE sample){
...
//3Khz butterworth
CSAMPLE processSampleHp(void*,void *bufIn, const CSAMPLE sample){
    CSAMPLE *buf = (CSAMPLE*) bufIn;
    CSAMPLE val = sample;
   CSAMPLE tmp, fir, iir;
...
CSAMPLE processSampleLp(void*,void *bufIn, const CSAMPLE sample){
    CSAMPLE *buf = (CSAMPLE*) bufIn;
    CSAMPLE val = sample;
    CSAMPLE tmp, fir, iir;
   tmp= buf[0]; memmove(buf, buf+1, 7*sizeof(CSAMPLE));
