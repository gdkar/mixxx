...
void EngineBufferScaleST::setScaleParameters(double base_rate,double* pTempoRatio,double* pPitchRatio) {
    ...
    if(base_rate== m_dBaseRate && *pTempoRatio == m_dTempoRatio && *pPitchRatio == m_dPitchRatio)
      return;
    double speed_abs = fabs(*pTempoRatio);
    if (speed_abs > MAX_SEEK_SPEED) {speed_abs = MAX_SEEK_SPEED;
    } else if (speed_abs < MIN_SEEK_SPEED) {speed_abs = 0;}
