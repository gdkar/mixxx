                                                     ...
                                                     double* pPitchRatio)
{
    ...
    constexpr auto kMinSeekSpeed = 1.0 / 128.0;
...
{
    m_pRubberBand->reset();
    m_preroll = m_pRubberBand->getLatency();
}

SINT EngineBufferScaleRubberBand::retrieveAndInterleave(
        CSAMPLE* pBuffer,
        SINT frames)
{
    auto frames_available = SINT(m_pRubberBand->available());
    while(m_preroll) {
        auto skip_chunk = std::min({SINT(frames_available), SINT(m_preroll), SINT(MAX_BUFFER_LEN)});
        if(skip_chunk) {
            auto skipped = m_pRubberBand->retrieve((float*const*)m_retrieve_buffer, skip_chunk);
            m_preroll -= skipped;
            frames_available = m_pRubberBand->available();
        }else{
            break;
        }
    }
    if(m_preroll)
        return 0;
    auto frames_to_read = std::min<SINT>(frames_available, frames);
    auto received_frames = m_pRubberBand->retrieve(
        ...
        SINT iOutputBufferSize)
{
    ...
    auto total_received_frames = SINT{};
    auto total_read_frames = SINT{};
    ...
    while (remaining_frames > 0) {
        ...
        // enough calls to retrieveAndInterleave because CachingReader returns
        // zeros for reads that are not in cache. So it's safe to loop here
        // without any checks for failure in retrieveAndInterleave.
        auto received_frames = retrieveAndInterleave(read, remaining_frames);
