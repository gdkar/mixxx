...
QNetworkReply* NetworkAccessManager::createRequest(Operation op, QNetworkRequest request, QIODevice* outgoingData)
{
...
{
    auto reply = m_timers.key(e->timerId());
    if (reply) reply->abort();
}
void NetworkTimeouts::setTimeout(int msec)
{ 
  m_timeout_msec = msec;
}
