...
AnalyserKey::~AnalyserKey() {delete m_pVamp;}
bool AnalyserKey::initialise(TrackPointer tio, int sampleRate, int totalSamples) {
    ...
    m_bPreferencesKeyDetectionEnabled = static_cast<bool>(
        m_pConfig->getValueString(ConfigKey(KEY_CONFIG_KEY, KEY_DETECTION_ENABLED)).toInt());
    ...
    if (bShouldAnalyze) {qDebug() << "Key calculation started with plugin" << m_pluginId;}
...
void AnalyserKey::cleanup(TrackPointer ) {
    delete m_pVamp;
    m_pVamp = nullptr;
}
...
// static
QHash<QString, QString> AnalyserKey::getExtraVersionInfo(QString pluginId, bool bPreferencesFastAnalysis) {
