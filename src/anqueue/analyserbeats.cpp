...
bool AnalyserBeats::initialise(TrackPointer tio, int sampleRate, int totalSamples)
{
    ...
    auto bPreferencesBeatDetectionEnabled = static_cast<bool>(m_pConfig->getValueString( ConfigKey(BPM_CONFIG_KEY, BPM_DETECTION_ENABLED)).toInt());
    ...
    m_bPreferencesFixedTempo = static_cast<bool>(m_pConfig->getValueString(ConfigKey(BPM_CONFIG_KEY, BPM_FIXED_TEMPO_ASSUMPTION)).toInt());
    m_bPreferencesOffsetCorrection = static_cast<bool>(m_pConfig->getValueString(ConfigKey(BPM_CONFIG_KEY, BPM_FIXED_TEMPO_OFFSET_CORRECTION)).toInt());
    m_bPreferencesReanalyzeOldBpm = static_cast<bool>(m_pConfig->getValueString(ConfigKey(BPM_CONFIG_KEY, BPM_REANALYZE_WHEN_SETTINGS_CHANGE)).toInt());
    m_bPreferencesFastAnalysis = static_cast<bool>(m_pConfig->getValueString(ConfigKey(BPM_CONFIG_KEY, BPM_FAST_ANALYSIS_ENABLED)).toInt());
    ...
    if (bShouldAnalyze)
    {
        m_pVamp = std::make_unique<VampAnalyser>();
...
{
    if(!m_pVamp || !m_pVamp->Process(pIn, iLen)) m_pVamp = nullptr;
}
...
void AnalyserBeats::finalise(TrackPointer tio)
{
    ...
    {
        if (zeroCurrentBpm) qDebug() << "Replacing 0-BPM beatgrid with a" << pBeats->getBpm() << "beatgrid.";
        tio->setBeats(pBeats);
        return;
    }
