...
TrackInfoObject::~TrackInfoObject() = default;
...
void TrackInfoObject::parse(bool parseCoverArt)
{
    ...
    if (!proxy.getType().isEmpty())
    {
        ...
        auto  pCoverArt = static_cast<QImage*>(parseCoverArt ? &coverArt : nullptr);
...
void TrackInfoObject::setBeats(BeatsPointer pBeats)
{
    ...
    if (m_pBeats)
    {
        if(auto pObject = dynamic_cast<QObject*>(m_pBeats.data()) ) disconnect(pObject, SIGNAL(updated()), this, SLOT(slotBeatsUpdated()));
    }
...
double TrackInfoObject::getDuration()  const
{ 
  return m_dDuration.load();
}
void TrackInfoObject::setDuration(double i)
{ 
  if (m_dDuration.exchange(i) != i) setDirty(true);
}
...
void TrackInfoObject::setComposer(const QString& s)
{
    ...
    if (ret != composer)
    {
      emit metadataUpdated();setDirty(true);
    }
}
...
{
  QMutexLocker lock(&m_qMutex);
  return m_iTimesPlayed.load();
}
void TrackInfoObject::setTimesPlayed(int t)
{
  QMutexLocker lock(&m_qMutex);
  if (t != m_iTimesPlayed.exchange(t))setDirty(true);
}
...
ConstWaveformPointer TrackInfoObject::getWaveform() const
{
  return m_waveform;
}
...
double TrackInfoObject::getAnalyserProgress() const
{
  return m_analyserProgress.load();
}
void TrackInfoObject::setCuePoint(double cue)
{
  if ( m_dCuePoint.exchange(cue)!=cue)
  {
    emit metadataUpdated();
    setDirty(true);
  }
}
double TrackInfoObject::getCuePoint() const
{
  return m_dCuePoint.load();
}
...
QList<Cue*> TrackInfoObject::getCuePoints()
{ 
  QMutexLocker lock(&m_qMutex);
  return m_cuePoints;
}
...
{ 
  return m_bLocationChanged.load();
}
int TrackInfoObject::getRating() const
{ 
  return m_Rating.load();
}
void TrackInfoObject::setRating (int rating)
{ 
  if (rating != m_Rating.exchange(rating)) 
  { 
    emit metadataUpdated();
    setDirty(true); 
  } 
...
void TrackInfoObject::setKey(mixxx::track::io::key::ChromaticKey key, mixxx::track::io::key::Source source)
{
    ...
    // Might be INVALID. We don't care.
    auto newKey = m_keys.getGlobalKey();
    lock.unlock();
    emit(keyUpdated(KeyUtils::keyToNumericValue(newKey)));
    emit(keysUpdated());
}
...
{
    QMutexLocker lock(&m_qMutex);
    auto newKeys = KeyFactory::makeBasicKeysFromText(key, source);
