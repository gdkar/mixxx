...
class SampleUtil {
  public:
    ...
    inline
    static void clear(CSAMPLE* pBuffer, int iNumSamples) { std::fill(pBuffer, pBuffer + iNumSamples, 0 ); }
    // Sets every sample in pBuffer to value
    inline
    static void fill(CSAMPLE* pBuffer, CSAMPLE value, int iNumSamples) { std::fill(pBuffer, pBuffer + iNumSamples, value); }
    // Copies every sample from pSrc to pDest
    inline
    static void copy(CSAMPLE* pDest, const CSAMPLE*  pSrc, int iNumSamples) { std::move(pSrc,pSrc + iNumSamples, pDest ); }
    ...
    static void applyAlternatingGain(CSAMPLE* pBuffer, CSAMPLE_GAIN gain1, CSAMPLE_GAIN gain2, int iNumSamples);
    ...
    static void applyRampingGain(CSAMPLE* pBuffer, CSAMPLE_GAIN old_gain, CSAMPLE_GAIN new_gain, int iNumSamples);
    // Copy pSrc to pDest and ramp gain
    // For optimum performance use the in-place function applyRampingGain()
    // if pDest == pSrc!
    static void copyWithRampingGain(CSAMPLE* pDest, const CSAMPLE* pSrc, CSAMPLE_GAIN old_gain, CSAMPLE_GAIN new_gain, int iNumSamples);
    // Add each sample of pSrc, multiplied by the gain, to pDest
    static void addWithGain(CSAMPLE* pDest, const CSAMPLE* pSrc, CSAMPLE_GAIN gain, int iNumSamples);
    // Add each sample of pSrc, multiplied by the gain, to pDest
    static void addWithRampingGain(CSAMPLE* pDest, const CSAMPLE* pSrc, CSAMPLE_GAIN old_gain, CSAMPLE_GAIN new_gain, int iNumSamples);
    // Add to each sample of pDest, pSrc1 multiplied by gain1 plus pSrc2
    // multiplied by gain2
    static void add2WithGain(CSAMPLE* pDest, const CSAMPLE* pSrc1, CSAMPLE_GAIN gain1, const CSAMPLE* pSrc2, CSAMPLE_GAIN gain2, int iNumSamples);
    // Add to each sample of pDest, pSrc1 multiplied by gain1 plus pSrc2
    // multiplied by gain2 plus pSrc3 multiplied by gain3
    static void add3WithGain(CSAMPLE* pDest, const CSAMPLE* pSrc1, CSAMPLE_GAIN gain1, const CSAMPLE* pSrc2, CSAMPLE_GAIN gain2, const CSAMPLE* pSrc3, CSAMPLE_GAIN gain3, int iNumSamples);
    ...
    static void convertFloat32ToS16(SAMPLE* pDest, const CSAMPLE* pSrc, unsigned int iNumSamples);
    ...
    static CLIP_STATUS sumAbsPerChannel(CSAMPLE* pfAbsL, CSAMPLE* pfAbsR, const CSAMPLE* pBuffer, int iNumSamples);
    ...
    static void copyClampBuffer(CSAMPLE* pDest, const CSAMPLE* pSrc, int iNumSamples);
    ...
    static void mixStereoToMono(CSAMPLE* pDest, const CSAMPLE* pSrc, int iNumSamples);
    ...
    static void copyMonoToDualMono(CSAMPLE* pDest, const CSAMPLE* pSrc, int numFrames);
    ...
    // (numFrames * 2) samples will be written into pBuffer
    static void stripMultiToStereo(CSAMPLE* pBuffer, int numFrames, int numChannels);
    ...
    // (numFrames * 2) samples will be written into pDest
    static void copyMultiToStereo(CSAMPLE* pDest, const CSAMPLE* pSrc, int numFrames, int numChannels);
