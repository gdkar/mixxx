...
class SampleUtil {
  public:
    ...
    static void copy(CSAMPLE*  pDest, const CSAMPLE* pSrc,int iNumSamples) {
        ...
        memmove(pDest, pSrc, iNumSamples * sizeof(CSAMPLE));
    }
    ...
    static void applyAlternatingGain(CSAMPLE* pBuffer, const CSAMPLE_GAIN gain1,
    ...
    // (numFrames * 2) samples will be written into pBuffer
    static void stripMultiToStereo(CSAMPLE* pBuffer, const int numFrames,
    ...
    static void copyMultiToStereo(CSAMPLE* pDest, const CSAMPLE* pSrc,
            const int numFrames, const int numChannels);
