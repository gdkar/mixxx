...
{
    if (gain1 == CSAMPLE_GAIN_ZERO)      return add2WithGain(pDest, pSrc2, gain2, pSrc3, gain3, iNumSamples);
    else if (gain2 == CSAMPLE_GAIN_ZERO) return add2WithGain(pDest, pSrc1, gain1, pSrc3, gain3, iNumSamples);
    else if (gain3 == CSAMPLE_GAIN_ZERO) return add2WithGain(pDest, pSrc1, gain1, pSrc2, gain2, iNumSamples);
    // note: LOOP VECTORIZED.
    for (auto i = 0; i < iNumSamples; ++i) pDest[i] += pSrc1[i] * gain1 + pSrc2[i] * gain2 + pSrc3[i] * gain3;
}

// static
void SampleUtil::copyWithGain(CSAMPLE* pDest, const CSAMPLE* pSrc,const CSAMPLE_GAIN gain, const int iNumSamples)
{
...
// static
void SampleUtil::convertS16ToFloat32(CSAMPLE* pDest, const SAMPLE* pSrc,const int iNumSamples)
{
    ...
    auto kConversionFactor = 1.0/CSAMPLE_GAIN{-SAMPLE_MIN};
    // note: LOOP VECTORIZED.
    for (auto i = 0; i < iNumSamples; ++i) pDest[i] = CSAMPLE(pSrc[i]) * kConversionFactor;
}

//static
void SampleUtil::convertFloat32ToS16(SAMPLE* pDest, const CSAMPLE* pSrc,const int iNumSamples)
{
    DEBUG_ASSERT(-SAMPLE_MIN >= SAMPLE_MAX);
    auto kConversionFactor = CSAMPLE(-SAMPLE_MIN);
