...
int ControlModel::rowCount(const QModelIndex& parent) const {
    return parent.isValid()?0:m_controls.size();
}

int ControlModel::columnCount(const QModelIndex& parent) const {
    return parent.isValid()?0:NUM_CONTROL_COLUMNS;
}
...
QVariant ControlModel::data(const QModelIndex& index,
    ...
    QString value;
    switch (column) {
        case CONTROL_COLUMN_GROUP:return control.key.group;
        case CONTROL_COLUMN_ITEM:return control.key.item;
        case CONTROL_COLUMN_VALUE:return control.pControl->get();
        case CONTROL_COLUMN_PARAMETER:return control.pControl->getParameter();
        case CONTROL_COLUMN_TITLE:return control.title;
        case CONTROL_COLUMN_DESCRIPTION:return control.description;
        case CONTROL_COLUMN_FILTER:return control.key.group + "," + control.key.item;
    }
    return QVariant();
}
...
bool ControlModel::setData(const QModelIndex& index,
    ...
    switch (column) {
        case CONTROL_COLUMN_VALUE:control.pControl->set(value.toDouble());return true;
        case CONTROL_COLUMN_PARAMETER:control.pControl->setParameter(value.toDouble());return true;
    }
    return false;
}
Qt::ItemFlags ControlModel::flags(const QModelIndex& index) const {
    ...
    if (index.column() == CONTROL_COLUMN_VALUE ||index.column() == CONTROL_COLUMN_PARAMETER) {
