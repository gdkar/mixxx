...
void TagFetcher::startFetch(const TrackPointer track) {
    ...
    tracks.append(track);
    m_tracks = tracks;
    auto future = QtConcurrent::mapped(m_tracks, getFingerprint);
    m_pFingerprintWatcher = new QFutureWatcher<QString>(this);
    m_pFingerprintWatcher->setFuture(future);
    connect(m_pFingerprintWatcher, SIGNAL(resultReadyAt(int)),SLOT(fingerprintFound(int)));
    for(auto ptrack: m_tracks) {emit(fetchProgress(tr("Fingerprinting track")));}
}
...
void TagFetcher::fingerprintFound(int index) {
    auto watcher = reinterpret_cast<QFutureWatcher<QString>*>(sender());
    if (!watcher || index >= m_tracks.count()) {return;}
    const auto fingerprint = watcher->resultAt(index);
    const auto ptrack = m_tracks[index];
...
void TagFetcher::mbidFound(int index, const QString& mbid) {
    if (index >= m_tracks.count()) {return;}
    const auto pTrack = m_tracks[index];
    if (mbid.isEmpty()) {
        emit(resultAvailable(pTrack, QList<TrackPointer>()));
        return;
...
void TagFetcher::tagsFetched(int index, const MusicBrainzClient::ResultList& results) {
    ...
    const auto originalTrack = m_tracks[index];
    QList<TrackPointer> tracksGuessed;
    for(auto& result: results) {
        auto track = TrackPointer(new TrackInfoObject(originalTrack->getLocation(),
