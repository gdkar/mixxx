...
void AcoustidClient::setTimeout(int msec) {m_timeouts.setTimeout(msec);}
...
void AcoustidClient::requestFinished() {
    ...
    if (reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt() != 200) {
        emit(networkError(reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt(),"AcoustID"));
        return;
    }
    QXmlStreamReader reader(reply);
    QString ID;
    while (!reader.atEnd()) {
        if (reader.readNext() == QXmlStreamReader::StartElement && reader.name()== "results") 
...
QString AcoustidClient::parseResult(QXmlStreamReader& reader) {
    while (!reader.atEnd()) {
        ...
        if (type== QXmlStreamReader::StartElement) {
            ...
            if (name == "id") {return reader.readElementText();}
        }
        if (type == QXmlStreamReader::EndElement && reader.name()=="result") {break;}
    }
    return QString();
}
