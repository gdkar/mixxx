...
void Tooltips::addStandardTooltips() {
    auto dropTracksHere = tr("Drop tracks from library, external file manager, or other decks/samplers here.");
    auto resetToDefault = tr("Reset to default value.");
    auto leftClick = tr("Left-click");
    auto rightClick = tr("Right-click");
    auto scrollWheel = tr("Scroll-wheel");
    ...
    auto scratchMouse = tr("Use the mouse to scratch, spin-back or throw tracks.");
    ...
    auto clippingHelp = tr("(too loud for the hardware and is being distorted).");
    ...
    auto changeAmount = tr("Change the step-size in the Preferences -> Interface menu.");
    ...
    add("filterHigh")
            ...
            << QString("%1: %2").arg(rightClick, resetToDefault);
    auto eqKillLatch = tr("Hold-to-kill or short click for latching.");
    add("filterHighKill")
            << tr("High EQ Kill")
    ...
    add("filterLowKill")
            ...
            << eqKillLatch;
    auto tempoDisplay = tr("Displays the tempo of the loaded track in BPM (beats per minute).");
    add("visual_bpm")
            << tr("Tempo")
            << tempoDisplay;
    add("visual_key")
    ...
    auto quantizeSnap = tr("If quantize is enabled, snaps to the nearest beat.");
    add("quantize")
            << tr("Quantize")
    ...
    auto cueSet = tr("Places a cue-point at the current position on the waveform.");
    add("play_cue_set")
            << tr("Play/Pause")
            << QString("%1: %2").arg(leftClick, tr("Plays or pauses the track."))
            << QString("%1: %2").arg(rightClick, cueSet);
    auto whilePlaying = tr("(while playing)");
    auto whileStopped = tr("(while stopped)");
    ...
    auto trackTags = tr("Information is loaded from the track's metadata tags.");
    add("track_artist")
            << tr("Track Artist")
