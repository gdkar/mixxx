...
namespace {
    QString pixmapCacheKey(quint16 hash, int width) {
        if (width == 0) { return QString("CoverArtCache_%1").arg(QString::number(hash));}
        else { return QString("CoverArtCache_%1_%2") .arg(QString::number(hash)).arg(width);}
    }
    ...
    const auto kMaxCoverSize = 300;
    // The transformation mode when scaling images
    const auto kTransformationMode = Qt::SmoothTransformation;
...
CoverArtCache::~CoverArtCache() { qDebug() << "~CoverArtCache()"; }
QPixmap CoverArtCache::requestCover(const CoverInfo& requestInfo,
    ...
    auto requestId = qMakePair(pRequestor, requestReference);
    if (m_runningRequests.contains(requestId)) { return QPixmap(); }
    ...
    // performance issues).
    auto cacheKey = pixmapCacheKey(requestInfo.hash, desiredWidth);
    QPixmap pixmap;
    if (QPixmapCache::find(cacheKey, &pixmap)) {
        if (signalWhenDone) { emit(coverFound(pRequestor, requestReference, requestInfo, pixmap, true)); }
        return pixmap;
    }
    if (onlyCached) {
        if (sDebug) { qDebug() << "CoverArtCache::requestCover cache miss"; }
        return QPixmap();
    }
    m_runningRequests.insert(requestId);
    auto watcher = new QFutureWatcher<FutureResult>(this);
    auto future = QtConcurrent::run( this, &CoverArtCache::loadCover, requestInfo, pRequestor, requestReference, desiredWidth, signalWhenDone);
...
CoverArtCache::FutureResult CoverArtCache::loadCover(
        ...
        const int desiredWidth,
        const bool signalWhenDone) {
    if (sDebug) { qDebug() << "CoverArtCache::loadCover" << info << desiredWidth << signalWhenDone; }
    FutureResult res;
    ...
    if (res.cover.image.isNull()) {return res; }
...
void CoverArtCache::coverLoaded() {
    auto watcher = reinterpret_cast<QFutureWatcher<FutureResult>*>(sender());
    auto res = watcher->result();
    if (sDebug) { qDebug() << "CoverArtCache::coverLoaded" << res.cover; }
    auto cacheKey = pixmapCacheKey(res.cover.info.hash, res.desiredWidth);
    QPixmap pixmap;
    ...
    if (res.signalWhenDone) {
        emit(coverFound(res.pRequestor, res.requestReference, res.cover.info, pixmap, false));
    }
}
void CoverArtCache::requestGuessCovers(QList<TrackPointer> tracks) { QtConcurrent::run(this, &CoverArtCache::guessCovers, tracks); }
void CoverArtCache::requestGuessCover(TrackPointer pTrack) { QtConcurrent::run(this, &CoverArtCache::guessCover, pTrack); }
void CoverArtCache::guessCover(TrackPointer pTrack) {
    if (pTrack) { pTrack->setCoverArt(CoverArtUtils::guessCoverArt(pTrack)); }
}
void CoverArtCache::guessCovers(QList<TrackPointer> tracks) {
    qDebug() << "CoverArtCache::guessCovers guessing covers for"
             << tracks.size() << "tracks";
    for(auto pTrack: tracks) { guessCover(pTrack); }
}
