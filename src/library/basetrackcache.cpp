...
BaseTrackCache::~BaseTrackCache() { delete m_pQueryParser; }
int BaseTrackCache::columnCount() const { return m_columnCount; }
int BaseTrackCache::fieldIndex(ColumnCache::Column column) const { return m_columnCache.fieldIndex(column); }
int BaseTrackCache::fieldIndex(const QString& columnName) const { return m_columnCache.fieldIndex(columnName); }
QString BaseTrackCache::columnNameForFieldIndex(int index) const {return m_columnCache.columnNameForFieldIndex(index); }
QString BaseTrackCache::columnSortForFieldIndex(int index) const { return m_columnCache.columnSortForFieldIndex(index); }
...
void BaseTrackCache::slotTracksRemoved(QSet<TrackId> trackIds) {
    ...
    for (const auto& trackId : trackIds) { m_trackInfo.remove(trackId); }
}
...
bool BaseTrackCache::isCached(TrackId trackId) const { return m_trackInfo.contains(trackId); }
void BaseTrackCache::ensureCached(TrackId trackId) { updateTrackInIndex(trackId); }
void BaseTrackCache::ensureCached(QSet<TrackId> trackIds) { updateTracksInIndex(trackIds); }
...
void BaseTrackCache::updateTracksInIndex(QSet<TrackId> trackIds) {
    ...
    for (const auto& trackId: trackIds) { idStrings << trackId.toString(); }
...
void BaseTrackCache::filterAndSort(const QSet<TrackId>& trackIds,
    ...
    for (TrackId trackId: dirtyTracks) {
        ...
        if (shouldBeInResultSet) {
            ...
            for (int i = 0; i < m_trackOrder.size(); ++i) { (*trackToIndex)[m_trackOrder[i]] = i; }
