          ...
          m_pQueryParser(new SearchQueryParser(pTrackCollection->getDatabase()))
{
    ...
    for (auto i = 0; i < m_searchColumns.size(); ++i)  m_searchColumnIndices[i] = m_columnCache.fieldIndex(m_searchColumns[i]);
}
...
void BaseTrackCache::slotTracksRemoved(QSet<TrackId> trackIds)
{
    ...
    for (auto trackId : trackIds) m_trackInfo.remove(trackId);
}
...
TrackPointer BaseTrackCache::lookupCachedTrack(TrackId trackId) const
{
    ...
    // dirty.
    if (m_bIsCaching && m_dirtyTracks.contains(trackId)) return m_trackDAO.getTrack(trackId, true);
    return TrackPointer();
}
bool BaseTrackCache::updateIndexWithTrackpointer(TrackPointer pTrack)
{
    ...
    if (trackId.isValid())
    {
        ...
        auto& record = m_trackInfo[trackId];
        // prealocate memory for all columns at once
        record.resize(numColumns);
        for (auto i = 0; i < numColumns; ++i) getTrackValueForColumn(pTrack, i, record[i]);
    }
    return true;
}
bool BaseTrackCache::updateIndexWithQuery(QString queryString)
{
    ...
    auto numColumns = columnCount();
    auto idColumn = query.record().indexOf(m_idColumn);
    while (query.next())
    {
        ...
        auto &record = m_trackInfo[trackId];
        record.resize(numColumns);
        for (int i = 0; i < numColumns; ++i) record[i] = query.value(i);
    }
...
void BaseTrackCache::buildIndex()
{
    ...
    auto queryString = QString("SELECT %1 FROM %2")
    ...
    // we don't see.
    m_trackInfo.clear();
    if (!updateIndexWithQuery(queryString)) qDebug() << "buildIndex failed!";
    m_bIndexBuilt = true;
}
...
void BaseTrackCache::updateTracksInIndex(QSet<TrackId> trackIds)
{
    ...
    for (auto trackId: trackIds) idStrings << trackId.toString();
...
void BaseTrackCache::getTrackValueForColumn(TrackPointer pTrack, int column, QVariant& trackValue) const
{
    ...
    if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_ARTIST) == column)            trackValue.setValue(pTrack->getArtist());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_TITLE) == column)        trackValue.setValue(pTrack->getTitle());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_ALBUM) == column)        trackValue.setValue(pTrack->getAlbum());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_ALBUMARTIST) == column)  trackValue.setValue(pTrack->getAlbumArtist());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_YEAR) == column)         trackValue.setValue(pTrack->getYear());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_DATETIMEADDED) == column)trackValue.setValue(pTrack->getDateAdded());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_GENRE) == column)        trackValue.setValue(pTrack->getGenre());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_COMPOSER) == column)     trackValue.setValue(pTrack->getComposer());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_GROUPING) == column)     trackValue.setValue(pTrack->getGrouping());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_FILETYPE) == column)     trackValue.setValue(pTrack->getType());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_TRACKNUMBER) == column)  trackValue.setValue(pTrack->getTrackNumber());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_LOCATION) == column)     trackValue.setValue(pTrack->getLocation());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_COMMENT) == column)      trackValue.setValue(pTrack->getComment());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_DURATION) == column)     trackValue.setValue(pTrack->getDuration());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_BITRATE) == column)      trackValue.setValue(pTrack->getBitrate());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_BPM) == column)          trackValue.setValue(pTrack->getBpm());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_PLAYED) == column)       trackValue.setValue(pTrack->getPlayed());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_TIMESPLAYED) == column)  trackValue.setValue(pTrack->getTimesPlayed());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_RATING) == column)       trackValue.setValue(pTrack->getRating());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_KEY) == column)          trackValue.setValue(pTrack->getKeyText());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_KEY_ID) == column)       trackValue.setValue(static_cast<int>(pTrack->getKey()));
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_BPM_LOCK) == column)     trackValue.setValue(pTrack->hasBpmLock());
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_COVERART_LOCATION) == column) trackValue.setValue(pTrack->getCoverInfo().coverLocation);
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_COVERART_HASH) == column ||
               fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_COVERART) == column)   trackValue.setValue(pTrack->getCoverInfo().hash);
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_COVERART_SOURCE) == column) trackValue.setValue(static_cast<int>(pTrack->getCoverInfo().source));
    else if (fieldIndex(ColumnCache::COLUMN_LIBRARYTABLE_COVERART_TYPE) == column)  trackValue.setValue(static_cast<int>(pTrack->getCoverInfo().type));
}
QVariant BaseTrackCache::data(TrackId trackId, int column) const
{
    ...
    auto pTrack = lookupCachedTrack(trackId);
                                   ...
                                   QHash<TrackId, int>* trackToIndex)
{
    ...
    for (auto trackId: trackIds)
    {
        ...
        if (m_dirtyTracks.contains(trackId)) dirtyTracks.insert(trackId);
    }
    auto pQuery(parseQuery( searchQuery, extraFilter, idStrings));
    auto filter = pQuery->toSql();
    if (!filter.isEmpty()) filter.prepend("WHERE ");
    QString queryString = QString("SELECT %1 FROM %2 %3 %4") .arg(m_idColumn, m_tableName, filter, orderByClause);
    if (sDebug) qDebug() << this << "select() executing:" << queryString;
    QSqlQuery query(m_database);
    // This causes a memory savings since QSqlCachedResult (what QtSQLite uses)
    // won't allocate a giant in-memory table that we won't use at all.
    ...
    auto idColumn = query.record().indexOf(m_idColumn);
    auto rows = query.size();
    if (sDebug) qDebug() << "Rows returned:" << rows;
    ...
    for (TrackId trackId: dirtyTracks)
    {
        ...
        auto shouldBeInResultSet = searchQuery.isEmpty() || pQuery->match(pTrack);
        // If the track is in this result set.
        auto isInResultSet = trackToIndex->contains(trackId);
        if (shouldBeInResultSet)
        {
            ...
            if (isInResultSet)
            {
                auto index = (*trackToIndex)[trackId];
            ...
            auto insertRow = findSortInsertionPoint(pTrack, sortColumn, sortOrder, m_trackOrder);
            if (sDebug)
                qDebug() << this
            ...
            for (auto i = 0; i < m_trackOrder.size(); ++i)  (*trackToIndex)[m_trackOrder[i]] = i;
        }
...
std::unique_ptr<QueryNode> BaseTrackCache::parseQuery(QString query, QString extraFilter, QStringList idStrings) const
{
    QStringList queryFragments;
    if (!extraFilter.isNull() && extraFilter != "") queryFragments << QString("(%1)").arg(extraFilter);
    if (idStrings.size() > 0) queryFragments << QString("%1 in (%2)") .arg(m_idColumn, idStrings.join(","));
                                           ...
                                           QVector<TrackId> trackIds) const
{
    ...
    auto min = 0;
    auto max = trackIds.size() - 1;
    if (sDebug) qDebug() << this << "Trying to insertion sort:" << trackValue << "min" << min << "max" << max;
    // If trackIds is empty, min is 0 and max is -1 so findSortInsertionPoint
    // returns 0.
    while (min <= max)
    {
        auto mid = min + (max - min) / 2;
        auto otherTrackId = trackIds[mid];
        ...
        auto tableValue = data(otherTrackId, sortColumn);
        auto compare = compareColumnValues(sortColumn, sortOrder, trackValue, tableValue);
        if (sDebug) qDebug() << this << "Comparing" << trackValue << "to" << tableValue << ":" << compare;
        if (compare == 0)
        {
...
int BaseTrackCache::compareColumnValues(int sortColumn, Qt::SortOrder sortOrder,QVariant val1, QVariant val2) const
{
    ...
    {
        // Sort as floats.
        auto delta = val1.toDouble() - val2.toDouble();
        if (std::abs(delta) < .00001) result = 0;
        else if (delta > 0.0) result = 1;
        else result = -1;
    }
    else result = val1.toString().compare(val2.toString(), Qt::CaseInsensitive);
    // If we're in descending order, flip the comparison.
    if (sortOrder == Qt::DescendingOrder) result = -result;
    return result;
}
