...
BrowseFeature::BrowseFeature(QObject* parent,
    ...
    // deletion.
    loadQuickLinks();
    for(const auto &quickLinkPath: m_quickLinkList) {
...
void BrowseFeature::slotLibraryScanStarted() {m_pAddtoLibraryAction->setEnabled(false);}
void BrowseFeature::slotLibraryScanFinished() {m_pAddtoLibraryAction->setEnabled(true);}
...
void BrowseFeature::onRightClickChild(const QPoint& globalPos, QModelIndex index) {
    ...
    for(const auto & str: m_quickLinkList) {if (str == path) {return;}}
