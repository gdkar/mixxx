...
QVariant BrowseFeature::title() {return QVariant(tr("Browse"));}
void BrowseFeature::onAddQuickLink() {
...
void BrowseFeature::onAddToLibrary() {
    if (!m_pLastRightClickedItem) {return;}
    QString spath = m_pLastRightClickedItem->dataPath().toString();
...
void BrowseFeature::onLibraryScanStarted() {m_pAddtoLibraryAction->setEnabled(false);}
void BrowseFeature::onLibraryScanFinished() {m_pAddtoLibraryAction->setEnabled(true);}
void BrowseFeature::onRemoveQuickLink() {
    if (!m_pLastRightClickedItem) {return;}
    QString spath = m_pLastRightClickedItem->dataPath().toString();
...
QIcon BrowseFeature::getIcon() {return QIcon(":/images/library/ic_library_browse.png");}
TreeItemModel* BrowseFeature::getChildModel() {return &m_childModel;}
...
void BrowseFeature::onLazyChildExpandation(const QModelIndex& index) {
    ...
    if (path == DEVICE_NODE) {
        ...
        // populate childs
        MDir dir(path);
        QFileInfoList all = dir.dir().entryInfoList(QDir::Dirs | QDir::NoDotAndDotDot);
...
void BrowseFeature::loadQuickLinks() {
    if (m_pConfig->getValueString(ConfigKey("[Browse]","QuickLinks")).isEmpty()) {m_quickLinkList = getDefaultQuickLinks();
    } else { m_quickLinkList = m_pConfig->getValueString(ConfigKey("[Browse]","QuickLinks")).split(kQuickLinksSeparator);}
}
...
QStringList BrowseFeature::getDefaultQuickLinks() const {
    ...
    QDir osMusicDir(QDesktopServices::storageLocation(QDesktopServices::MusicLocation));
    QDir osDocumentsDir(QDesktopServices::storageLocation(QDesktopServices::DocumentsLocation));
    QDir osHomeDir(QDesktopServices::storageLocation(QDesktopServices::HomeLocation));
    QDir osDesktopDir(QDesktopServices::storageLocation(QDesktopServices::DesktopLocation));
    ...
    if (downloadsExists && !osDownloadsDirIncluded && Sandbox::canAccessFile(osDownloadsDir)) {
