...
{
    connect(this, SIGNAL(requestAddDir(QString)),parent, SLOT(slotRequestAddDir(QString)));
    ...
    connect(m_pAddtoLibraryAction, SIGNAL(triggered()),this, SLOT(slotAddToLibrary()));
    ...
    auto devices_link = new TreeItem(tr("Devices"), DEVICE_NODE, this, rootItem);
    ...
    for(auto drive: drives)
    {
        ...
        auto display_path = drive.filePath();
        if (display_path.endsWith("/")) display_path.chop(1); 
        auto driveLetter = new TreeItem(
    ...
    // /Volumes
    auto devices_link = new TreeItem(tr("Devices"), "/Volumes/", this, rootItem);
    rootItem->appendChild(devices_link);
#else  // LINUX
    auto devices_link = new TreeItem(tr("Removable Devices"), "/media/", this, rootItem);
    rootItem->appendChild(devices_link);

    // show root directory on UNIX-based operating systems
    auto  root_folder_item = new TreeItem(QDir::rootPath(), QDir::rootPath(), this, rootItem);
    ...
    for(auto quickLinkPath: m_quickLinkList)
    {
...
{
    if (!m_pLastRightClickedItem) return;
    auto spath = m_pLastRightClickedItem->dataPath().toString();
    auto name = extractNameFromPath(spath);
    auto item = new TreeItem(name, spath, this, m_pQuickLinkItem);
    m_pQuickLinkItem->appendChild(item);
...
void BrowseFeature::slotAddToLibrary() {
    ...
    auto scanButton = msgBox.addButton(tr("Scan"), QMessageBox::AcceptRole);
    msgBox.addButton(QMessageBox::Cancel);
    msgBox.setDefaultButton(scanButton);
    msgBox.exec();
    if (msgBox.clickedButton() == scanButton)emit(scanLibrary());
}
...
void BrowseFeature::bindWidget(WLibrary* libraryWidget,QObject* keyboard)
{
    Q_UNUSED(keyboard);
    auto edit = new WLibraryTextBrowser(libraryWidget);
...
void BrowseFeature::activateChild(const QModelIndex& index)
{
    ...
    auto path = item->dataPath().toString();
    if (path == QUICK_LINK_NODE || path == DEVICE_NODE) m_browseModel.setPath(MDir());
    else
    {
        ...
        if (!dir.canAccess())
        {
            if (Sandbox::askForAccess(path))dir = MDir(path);
            else return;
        }
...
void BrowseFeature::onRightClickChild(const QPoint& globalPos, QModelIndex index)
{
    ...
    if (!item) return;
    auto path = item->dataPath().toString();
    if (path == QUICK_LINK_NODE || path == DEVICE_NODE) return;
    QMenu menu(nullptr);
...
void BrowseFeature::onLazyChildExpandation(const QModelIndex& index) {
    ...
    if (path == QUICK_LINK_NODE) return;
    ...
    else
    {
        ...
        // populate childs
        MDir dir(path);
        auto all = dir.dir().entryInfoList( QDir::Dirs | QDir::NoDotAndDotDot);
    ...
    // has no subfolders
    if (!folders.isEmpty()) m_childModel.insertRows(folders, 0, folders.size(), index);
}
QString BrowseFeature::getRootViewHtml() const
{
    auto browseTitle = tr("Browse");
    auto browseSummary = tr("Browse lets you navigate, view, and load tracks"
...
{
    m_pConfig->set(ConfigKey("Browse","QuickLinks"),ConfigValue(m_quickLinkList.join(kQuickLinksSeparator)));
}
...
void BrowseFeature::loadQuickLinks()
{
    ...
    {
        m_quickLinkList = m_pConfig->getValueString(ConfigKey("Browse","QuickLinks")).split(kQuickLinksSeparator);
    }
}
...
QStringList BrowseFeature::getDefaultQuickLinks() const
{
    ...
    auto mixxxMusicDirs = m_pTrackCollection->getDirectoryDAO().getDirs();
    auto osMusicDir = QDir(QDesktopServices::storageLocation(QDesktopServices::MusicLocation));
    auto osDocumentsDir = QDir(QDesktopServices::storageLocation(QDesktopServices::DocumentsLocation));
    auto osHomeDir = QDir(QDesktopServices::storageLocation(QDesktopServices::HomeLocation));
    auto osDesktopDir = QDir(QDesktopServices::storageLocation(QDesktopServices::DesktopLocation));
    auto osDownloadsDir = QDir(osHomeDir);
    ...
    // downloads folder.
    auto downloadsExists = osDownloadsDir.cd("Downloads");

    QStringList result;
    auto osMusicDirIncluded = false;
    auto osDownloadsDirIncluded = false;
    auto osDesktopDirIncluded = false;
    auto osDocumentsDirIncluded = false;
    for(auto dirPath: mixxxMusicDirs)
    {
        QDir dir(dirPath);
        // Skip directories we don't have permission to.
        if (!Sandbox::canAccessFile(dir))  continue;
        if (dir == osMusicDir)      osMusicDirIncluded = true;
        if (dir == osDownloadsDir)  osDownloadsDirIncluded = true;
        if (dir == osDesktopDir)    osDesktopDirIncluded = true;
        if (dir == osDocumentsDir)  osDocumentsDirIncluded = true;
        result << dir.canonicalPath() + "/";
    }
    if (!osMusicDirIncluded && Sandbox::canAccessFile(osMusicDir)) result << osMusicDir.canonicalPath() + "/";
    if (downloadsExists && !osDownloadsDirIncluded && Sandbox::canAccessFile(osDownloadsDir))result << osDownloadsDir.canonicalPath() + "/";
    if (!osDesktopDirIncluded && Sandbox::canAccessFile(osDesktopDir))result << osDesktopDir.canonicalPath() + "/";
    if (!osDocumentsDirIncluded && Sandbox::canAccessFile(osDocumentsDir)) result << osDocumentsDir.canonicalPath() + "/";
