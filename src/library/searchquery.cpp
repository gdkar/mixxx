...
bool AndNode::match(const TrackPointer& pTrack) const {
    for (const auto& pNode: m_nodes) {
        if (!pNode->match(pTrack)) {return false;}
    }
...
QString AndNode::toSql() const {
    ...
    for (const auto& pNode: m_nodes) {
        ...
        if (!sql.isEmpty()) {queryFragments << sql;}
    }
...
bool OrNode::match(const TrackPointer& pTrack) const {
    ...
    for (const auto& pNode: m_nodes) {
        if (pNode->match(pTrack)) {return true;}
    }
    return false;
}
...
bool NotNode::match(const TrackPointer& pTrack) const {return !m_pNode->match(pTrack);}
...
bool TextFilterNode::match(const TrackPointer& pTrack) const {
    for (const auto& sqlColumn: m_sqlColumns) {
        ...
        if (!value.isValid() || !qVariantCanConvert<QString>(value)) {continue;}
...
void NumericFilterNode::init(QString argument) {
    ...
    if (parsed) {m_bOperatorQuery = true;}
...
QString NumericFilterNode::toSql() const {
    if (m_bOperatorQuery) {
        QStringList searchClauses;
        for (const auto& sqlColumn: m_sqlColumns) {
            searchClauses << QString("%1 %2 %3").arg(sqlColumn, m_operator, QString::number(m_dOperatorArgument));
        }
        return concatSqlClauses(searchClauses, "OR");
    }
    if (m_bRangeQuery) {
...
bool KeyFilterNode::match(const TrackPointer& pTrack) const {return m_matchKeys.contains(pTrack->getKey());}
QString KeyFilterNode::toSql() const {
    QStringList searchClauses;
    for (const auto& matchKey: m_matchKeys) {searchClauses << QString("key_id IS %1").arg(QString::number(matchKey));}
    return concatSqlClauses(searchClauses, "OR");
}
