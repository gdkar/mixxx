...
class ScannerGlobal {
  public:
    ...
    inline bool shouldCancel() const {return m_shouldCancel;}
    std::atomic<bool> &shouldCancelRef() {return m_shouldCancel;}
    void setShouldCancel(bool shouldCancel) {m_shouldCancel = shouldCancel;}
    inline bool scanFinishedCleanly() const {return m_scanFinishedCleanly;}
    void setScanFinishedCleanly(bool scanFinishedCleanly) {m_scanFinishedCleanly = scanFinishedCleanly;}
  ...
  private:
    ...
    std::atomic<int> m_numAddedTracks;
    std::atomic<int> m_numScannedDirectories;
};
