...
class ScannerTask : public QObject, public QRunnable {
    Q_OBJECT
  public:
    ScannerTask(LibraryScanner* pScanner,const ScannerGlobalPointer scannerGlobal);
    virtual ~ScannerTask();
    virtual void run() = 0;
  signals:
    void taskDone(bool success);
    void queueTask(ScannerTask* pTask);
    void directoryHashedAndScanned(const QString& directoryPath,bool newDirectory, int hash);
    void directoryHashed(const QString& directoryPath, bool newDirectory,int hash);
  ...
  protected:
    void setSuccess(bool success) {m_success.store(success);}
