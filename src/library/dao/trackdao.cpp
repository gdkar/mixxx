...
{
    DEBUG_ASSERT_AND_HANDLE(!m_pTrack.isNull()) {return;}
    ...
    // completes.
    if (m_pTrack->isDirty())emit(saveTrack(m_pTrack));
}
...
// expensive.
const int kRecentTracksCacheSize = 3;
...
void TrackDAO::finish()
{
    ...
    while (it.hasNext())
    {
        ...
        if (!pTrack.isNull()) {
            if (pTrack->isDirty()) saveTrack(pTrack);
    ...
    auto deletedHashDirs = m_libraryHashDao.getDeletedDirectories();
    ...
    for(auto dir: deletedHashDirs) markTrackLocationsAsDeleted(dir);
    transaction.commit();
}
...
TrackId TrackDAO::getTrackId(const QString& absoluteFilePath)
{
    ...
    if (query.exec())
    {
        if (query.next()) trackId = TrackId(query.value(query.record().indexOf("id")));
    } 
    else 
    {
        LOG_FAILED_QUERY(query);
...
QList<TrackId> TrackDAO::getTrackIds(const QList<QFileInfo>& files)
{
    ...
    for (const auto& file: files)  pathList << escaper.escapeString(file.absoluteFilePath());
    QSqlQuery query(m_database);
    ...
    if (query.exec())
    {
        ...
        while (query.next())  trackIds.append(TrackId(query.value(idColumn)));
...
QSet<QString> TrackDAO::getTrackLocations()
{
    ...
    auto locationColumn = query.record().indexOf("location");
    while (query.next())  locations.insert(query.value(locationColumn).toString());
    return locations;
}
...
QString TrackDAO::getTrackLocation(TrackId trackId) {
    ...
    auto locationColumn = query.record().indexOf("location");
    while (query.next())  trackLocation = query.value(locationColumn).toString();
...
void TrackDAO::slotTrackDirty(TrackInfoObject* pTrack)
{
    ...
    // from.
    auto trackId = pTrack->getId();
    if (trackId.isValid()) emit(trackDirty(trackId));
}
void TrackDAO::slotTrackClean(TrackInfoObject* pTrack)
{
    ...
    auto trackId = pTrack->getId();
    if (trackId.isValid()) emit(trackClean(trackId));
}
...
void TrackDAO::slotTrackChanged(TrackInfoObject* pTrack)
{
    ...
    auto trackId = pTrack->getId();
    if (trackId.isValid()) emit(trackChanged(trackId));
}
...
void TrackDAO::bindTrackToLibraryInsert(TrackInfoObject* pTrack, int trackLocationId)
{
    ...
    auto pBeatsBlob = static_cast<const QByteArray*>(nullptr);
    auto beatsVersion = QString{};
    auto beatsSubVersion = QString{};
    auto pBeats = pTrack->getBeats();
    // Fall back on cached BPM
    auto dBpm = pTrack->getBpm();
    if (pBeats)
    {
    ...
    m_pQueryLibraryInsert->bindValue(":keys", pKeysBlob ? *pKeysBlob : QVariant(QVariant::ByteArray));
...
void TrackDAO::addTracksPrepare()
{
    ...
    m_pQueryLibraryInsert->prepare("INSERT INTO library "
            ...
            ":coverart_source, :coverart_type, :coverart_location, :coverart_hash "
            ")");
    m_pQueryLibraryUpdate->prepare("UPDATE library SET mixxx_deleted = 0 WHERE id = :id");
    m_pQueryLibrarySelect->prepare("SELECT location, id, mixxx_deleted from library WHERE location = :location");
}
...
bool TrackDAO::addTracksAdd(TrackInfoObject* pTrack, bool unremove)
{
    ...
    if (!m_pQueryTrackLocationInsert->exec())
    {
        ...
        auto  lastInsert = m_pQueryTrackLocationInsert->lastInsertId();
        ...
        DEBUG_ASSERT_AND_HANDLE(trackLocationId >= 0) {return false;}
...
{
    TrackId trackId;
    auto pTrack = new TrackInfoObject(fileInfo);
    if (pTrack)
    {
...
void TrackDAO::addTrack(TrackInfoObject* pTrack, bool unremove)
{
    ...
    if (!SoundSourceProxy::isFileSupported(pTrack->getFileInfo())) return;
    addTracksPrepare();
    addTracksAdd(pTrack, unremove);
    addTracksFinish(false);
}
QList<TrackId> TrackDAO::addTracks(const QList<QFileInfo>& fileInfoList,bool unremove) {
    QList<TrackId> trackIds;
    ...
    auto index = 0;
    for(const auto&  rFileInfo: fileInfoList) {
    ...
    if (unremove)
    {
        ...
        for (auto trackId: trackIds) idStringList.append(trackId.toString());
    ...
    if (!query.exec()) LOG_FAILED_QUERY(query);
    auto addIndexColumn = query.record().indexOf("add_index");
    auto locationColumn = query.record().indexOf("location");
    while (query.next())
    {
        auto addIndex = query.value(addIndexColumn).toInt();
        auto filePath = query.value(locationColumn).toString();
        const auto & fileInfo = fileInfoList.at(addIndex);
        auto pTrack = new TrackInfoObject(fileInfo);
        addTracksAdd(pTrack, unremove);
        auto trackId = pTrack->getId();
...
{
    QStringList idList;
    for (auto trackId: trackIds) idList.append(trackId.toString());
    QSqlQuery query(m_database);
    query.prepare(QString("UPDATE library SET mixxx_deleted=1 WHERE id in (%1)").arg(idList.join(",")));
    if (!query.exec()) LOG_FAILED_QUERY(query);
    // This signal is received by basetrackcache to remove the tracks from cache
    auto tracksRemovedSet = QSet<TrackId>::fromList(trackIds);
...
void TrackDAO::unhideTracks(const QList<TrackId>& trackIds)
{
    ...
    for (auto trackId: trackIds) idList.append(trackId.toString());
    QSqlQuery query(m_database);
    query.prepare(QString("UPDATE library SET mixxx_deleted=0 WHERE id in (%1)").arg(idList.join(",")));
    if (!query.exec()) LOG_FAILED_QUERY(query);
    auto tracksAddedSet = QSet<TrackId>::fromList(trackIds);
...
void TrackDAO::purgeTracks(const QString& dir)
{
    ...
    // directories.
    auto likeClause = escaper.escapeStringForLike(QDir(dir).absolutePath() + "/", '%') + "%";
...
void TrackDAO::purgeTracks(const QList<TrackId>& trackIds)
{
    ...
    auto locationColumn = queryRecord.indexOf("location");
    auto directoryColumn = queryRecord.indexOf("directory");
    while (query.next())
    {
        auto filePath = query.value(locationColumn).toString();
        locationList << escaper.escapeString(filePath);
        auto directory = query.value(directoryColumn).toString();
        dirs << escaper.escapeString(directory);
    }
    if (locationList.empty()) LOG_FAILED_QUERY(query);
    ...
    query.prepare(QString("DELETE FROM library WHERE id in (%1)").arg(idListJoined));
    if (!query.exec())  LOG_FAILED_QUERY(query);
    ...
    auto dirList = QStringList::fromSet(dirs);
...
bool setTrackBeats(const QSqlRecord& record, const int column,
                   TrackPointer pTrack) {
    auto bpm = record.value(column).toDouble();
    auto beatsVersion = record.value(column + 1).toString();
    auto beatsSubVersion = record.value(column + 2).toString();
    auto beatsBlob = record.value(column + 3).toByteArray();
    auto bpmLocked = record.value(column + 4).toBool();
    auto pBeats = BeatFactory::loadBeatsFromByteArray(pTrack, beatsVersion, beatsSubVersion, &beatsBlob);
    if (pBeats) {
...
bool setTrackKey(const QSqlRecord& record, const int column,TrackPointer pTrack) {
    auto keyText = record.value(column).toString();
    auto keysVersion = record.value(column + 1).toString();
    auto keysSubVersion = record.value(column + 2).toString();
    auto keysBlob = record.value(column + 3).toByteArray();
    auto  keys = KeyFactory::loadKeysFromByteArray( keysVersion, keysSubVersion, &keysBlob);
    if (keys.isValid()) {
        pTrack->setKeys(keys);
...
TrackPointer TrackDAO::getTrackFromDB(TrackId trackId) const {
    ...
    QString columnsStr;
    auto columnsSize = 0;
    auto columnsCount = ARRAYLENGTH(columns);
    for (auto i = 0; i < columnsCount; ++i) columnsSize += qstrlen(columns[i].name) + 1;
    columnsStr.reserve(columnsSize);
    for (auto  i = 0; i < columnsCount; ++i) {
        if (i > 0) columnsStr.append(QChar(','));
    ...
    QSqlRecord queryRecord = query.record();
    auto recordCount = queryRecord.count();
    DEBUG_ASSERT_AND_HANDLE(recordCount == columnsCount) {recordCount = std::min<decltype(columnsCount)>(recordCount, columnsCount);}
    // Location is the first column.
    auto location = queryRecord.value(0).toString();
    auto pTrack = TrackPointer(new TrackInfoObject(location, SecurityTokenPointer(),false),TrackInfoObject::onTrackReferenceExpired);
    pTrack->setId(trackId);
    ...
    connect(pTrack.data(), SIGNAL(dirty(TrackInfoObject*)),this, SLOT(slotTrackDirty(TrackInfoObject*)),Qt::DirectConnection);
    connect(pTrack.data(), SIGNAL(clean(TrackInfoObject*)),this, SLOT(slotTrackClean(TrackInfoObject*)),Qt::DirectConnection);
    connect(pTrack.data(), SIGNAL(changed(TrackInfoObject*)),this, SLOT(slotTrackChanged(TrackInfoObject*)),Qt::DirectConnection);
    ...
    connect(pTrack.data(), SIGNAL(referenceExpired(TrackInfoObject*)),this, SLOT(slotTrackReferenceExpired(TrackInfoObject*)),Qt::QueuedConnection);
    m_sTracksMutex.lock();
    ...
    auto  pCacheItem = new TrackCacheItem(pTrack);
    // Queued connection. We are not in a rush to process cache
    // expirations and it can produce dangerous signal loops.
    // See: https://bugs.launchpad.net/mixxx/+bug/1365708
    connect(pCacheItem, SIGNAL(saveTrack(TrackPointer)),this, SLOT(saveTrack(TrackPointer)),Qt::QueuedConnection);
    ...
    // track's signals.
    if (shouldDirty) emit(trackDirty(trackId));
    ...
    // dirty it.
    if (!pTrack->getHeaderParsed())  pTrack->parse(false);
    return pTrack;
}
TrackPointer TrackDAO::getTrack(TrackId trackId, const bool cacheOnly) const
{
    ...
    auto  pTrackCacheItem = m_recentTracksCache.object(trackId);
    if (pTrackCacheItem)
    {
    ...
    QMutexLocker locker(&m_sTracksMutex);
    if(m_sTracks.contains(trackId))
      pTrack = m_sTracks.value(trackId).toStrongRef();
    ...
    if (pTrack)
    {
        ...
        m_recentTracksCache.insert(trackId, pCacheItem);
        return pTrack;
    } else if (cacheOnly)  return TrackPointer();
...
void TrackDAO::updateTrack(TrackInfoObject* pTrack) {
    ...
    auto trackId = pTrack->getId();
    DEBUG_ASSERT_AND_HANDLE(trackId.isValid()) {return;}
    QSqlQuery query(m_database);
    ...
    auto pBeats = pTrack->getBeats();
    auto pBeatsBlob = static_cast<QByteArray*>(nullptr);
    auto beatsVersion = QString{};
    auto beatsSubVersion = QString{};
    auto dBpm = pTrack->getBpm();
    if (pBeats) {
...
void TrackDAO::markTrackLocationsAsVerified(const QStringList& locations) {
    ...
    FieldEscaper escaper(m_database);
    auto escapedLocations = escaper.escapeStrings(locations);
    QSqlQuery query(m_database);
    ...
    if (!query.exec()) LOG_FAILED_QUERY(query)<< "Couldn't mark track locations as verified.";
}
void TrackDAO::markTracksInDirectoriesAsVerified(const QStringList& directories) {
    ...
    QSqlQuery query(m_database);
    query.prepare(QString("UPDATE track_locations SET needs_verification=0 WHERE directory IN (%1)").arg(escapedDirectories.join(",")));
    if (!query.exec()) {
        LOG_FAILED_QUERY(query)
...
void TrackDAO::markUnverifiedTracksAsDeleted() {
    ...
    if (!query.exec())  LOG_FAILED_QUERY(query) << "Couldn't mark unverified tracks as deleted.";
}
void TrackDAO::markTrackLocationsAsDeleted(const QString& directory)
{
    ...
    QSqlQuery query(m_database);
    query.prepare("UPDATE track_locations SET fs_deleted=1 WHERE directory=:directory");
    query.bindValue(":directory", directory);
    if (!query.exec()) {
        LOG_FAILED_QUERY(query) << "Couldn't mark tracks in" << directory << "as deleted.";
    }
}
...
bool TrackDAO::detectMovedFiles(QSet<TrackId>* pTracksMovedSetOld,
    ...
    auto filenameColumn = queryRecord.indexOf("filename");
    auto durationColumn = queryRecord.indexOf("duration");
    ...
    while (query.next())
    {
        ...
        auto query2idColumn = query2.record().indexOf("id");
        ...
        if (newTrackLocationId.isValid())
        {
            ...
            if (!query3.exec()) LOG_FAILED_QUERY(query3);
            ...
            if (!query3.exec()) LOG_FAILED_QUERY(query3);
            auto query3idColumn = query3.record().indexOf("id");
            if (query3.next()) {
                auto newTrackId = TrackId(query3.value(query3idColumn));
...
void TrackDAO::markTracksAsMixxxDeleted(const QString& dir)
{
    ...
    // dir needs to end in a slash otherwise we might match other
    // directories.
    auto likeClause = escaper.escapeStringForLike(dir + "/", '%') + "%";
    query.prepare(QString("SELECT library.id FROM library INNER JOIN track_locations "
                          "ON library.location = track_locations.id "
                          "WHERE track_locations.location LIKE %1 ESCAPE '%'")
                  .arg(escaper.escapeString(likeClause)));
...
void TrackDAO::detectCoverArtForUnknownTracks(std::atomic<bool>* pCancel,QSet<TrackId>* pTracksChanged) {
    ...
    while (query.next())
    {
        ...
        auto source = static_cast<CoverInfo::Source>( query.value(4).toInt());
...
TrackPointer TrackDAO::getOrAddTrack(const QString& trackLocation,
    ...
    auto  trackId = TrackId(getTrackId(trackLocation));
    auto track_already_in_library = trackId.isValid();
    // Add Track to library -- unremove if it was previously removed.
    if (!trackId.isValid())  trackId = addTrack(trackLocation, true);
    TrackPointer pTrack;
    if (trackId.isValid()) pTrack = getTrack(trackId);
    ...
    if (pTrack.isNull()) 
        pTrack = TrackPointer(new TrackInfoObject(trackLocation, SecurityTokenPointer(), true, false));
    ...
    if (processCoverArt && pTrack && !track_already_in_library) {
        ...
        if (pCache ) pCache->requestGuessCover(pTrack);
    }
    if (pAlreadyInLibrary ) *pAlreadyInLibrary = track_already_in_library;
    return pTrack;
}
