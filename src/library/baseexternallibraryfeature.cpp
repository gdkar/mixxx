...
BaseExternalLibraryFeature::BaseExternalLibraryFeature(QObject* pParent,
    ...
    connect(m_pAddToAutoDJAction, SIGNAL(triggered()), this, SLOT(slotAddToAutoDJ()));
    m_pAddToAutoDJTopAction = new QAction(tr("Add to Auto DJ Queue (top)"), this);
    connect(m_pAddToAutoDJTopAction, SIGNAL(triggered()), this, SLOT(slotAddToAutoDJTop()));
    m_pImportAsMixxxPlaylistAction = new QAction(tr("Import Playlist"), this);
    connect(m_pImportAsMixxxPlaylistAction, SIGNAL(triggered()), this, SLOT(slotImportAsMixxxPlaylist()));
}
...
void BaseExternalLibraryFeature::onRightClick(const QPoint& /*globalPos*/) { m_lastRightClickedIndex = QModelIndex(); }
...
void BaseExternalLibraryFeature::slotAddToAutoDJ() { addToAutoDJ(false); }
void BaseExternalLibraryFeature::slotAddToAutoDJTop() { addToAutoDJ(true); }
...
void BaseExternalLibraryFeature::slotImportAsMixxxPlaylist() {
    ...
    auto playlistId = playlistDao.createUniquePlaylist(&playlist);
    if (playlistId != -1) { playlistDao.appendTracksToPlaylist(trackIds, playlistId);}
    else {
...
void BaseExternalLibraryFeature::appendTrackIdsFromRightClickIndex(QList<TrackId>* trackIds, QString* pPlaylist) {
    if (!m_lastRightClickedIndex.isValid()) { return; }
    ...
    *pPlaylist = m_lastRightClickedIndex.data(Qt::UserRole).toString();
    ...
    pPlaylistModelToAdd->setSort(pPlaylistModelToAdd->fieldIndex( ColumnCache::COLUMN_PLAYLISTTRACKSTABLE_POSITION), Qt::AscendingOrder);
    ...
    for (auto i = 0; i < rows; ++i) {
        ...
        if (index.isValid()) {
            ...
            auto track = pPlaylistModelToAdd->getTrack(index);
            if (!track) { continue;}
