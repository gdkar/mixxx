...
SchemaManager::Result SchemaManager::upgradeToSchemaVersion(
    ...
    if (schemaRoot.isNull()) {return RESULT_SCHEMA_ERROR;}
    ...
    for (int i = 0; i < revisions.count(); i++) {
        ...
        DEBUG_ASSERT_AND_HANDLE(!version.isNull()) {return RESULT_SCHEMA_ERROR;}
    ...
    while (currentVersion < targetVersion) {
        ...
        if (minCompatibleVersion.isNull()) {minCompatibleVersion = QString::number(thisTarget);}
        DEBUG_ASSERT_AND_HANDLE(!eSql.isNull()) {return RESULT_SCHEMA_ERROR;}
        ...
        if (result) {
            ...
            qDebug() << "Failed to move from version" << currentVersion << "to version" << thisTarget;
...
bool SchemaManager::isBackwardsCompatible(SettingsDAO& settings,
                                          int currentVersion,
                                          int targetVersion) {
    QString backwardsCompatibleVersion = settings.getValue(SETTINGS_MINCOMPATIBLE_STRING);
    bool ok = false;
    ...
    if (backwardsCompatibleVersion.isNull() || !ok) {
        ...
        if (currentVersion == 7) {iBackwardsCompatibleVersion = 3;}
