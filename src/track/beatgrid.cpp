...
class BeatGridIterator : public BeatIterator
{
  public:
    ...
    virtual double next()
    {
        auto beat = m_dCurrentSample;
        m_dCurrentSample += m_dBeatLength;
        return beat;
    }
  private:
    double m_dBeatLength    = 0;
    double m_dCurrentSample = 0;
    double m_dEndSample     = 0;
};
...
double BeatGrid::findNthBeat(double dSamples, int n) const
{
    ...
    auto beatFraction = (dSamples - firstBeatSample()) / m_dBeatLength;
    auto prevBeat = floor(beatFraction);
    auto nextBeat = ceil(beatFraction);

    // If the position is within 1/100th of the next or previous beat, treat it
    // as if it is that beat.
    auto kEpsilon = .01;

    if (std::abs(nextBeat - beatFraction) < kEpsilon)
    {
    ...
    auto dResult = std::floor(dClosestBeat + n * m_dBeatLength);
                                 ...
                                 double* dpNextBeatSamples) const
{
    ...
    auto beatFraction = (dSamples - dFirstBeatSample) / dBeatLength;
    auto prevBeat = floor(beatFraction);
    auto nextBeat = ceil(beatFraction);
...
BeatIterator BeatGrid::findBeats(double startSample, double stopSample) const
{
    ...
    if (curBeat == -1.0) return BeatGridIterator{};
    return BeatGridIterator(m_dBeatLength, curBeat, stopSample);
}
...
{
    QMutexLocker locker(&m_mutex);
    if (!isValid() || startSample > stopSample) return -1;
    return bpm();
}
...
void BeatGrid::translate(double dNumSamples)
{
    QMutexLocker locker(&m_mutex);
    if (!isValid()) return;
    auto newFirstBeatFrames = (firstBeatSample() + dNumSamples) / kFrameSize;
...
void BeatGrid::scale(double dScalePercentage)
{
    QMutexLocker locker(&m_mutex);
    if (!isValid()) return;
    auto newBpm = bpm() * dScalePercentage;
...
Beats::CapabilitiesFlags BeatGrid::getCapabilities() const
{
