    ...
    if (!m_skipRender) {
        if (m_type) {   // no regular updates for an empty waveform
            ...
            for (int i = 0; i < m_waveformWidgetHolders.size(); i++) {
                ...
                if (pWaveformWidget->getWidth() > 0 && pWaveformWidget->getWidget()->isVisible()) (void)pWaveformWidget->render();
...
WaveformWidgetType::Type WaveformWidgetFactory::autoChooseWidgetType() const {
    ...
    if (m_openGLAvailable) {
        if (m_openGLShaderAvailable) return WaveformWidgetType::GLSLRGBWaveform;
        else  return WaveformWidgetType::GLRGBWaveform;
    }
