...
VisualPlayPosition::VisualPlayPosition(const QString& key)
        : m_valid(false),
          m_key(key)
{
    m_audioBufferSize = new ControlObjectSlave("[Master]", "audio_buffer_size", this);
    m_audioBufferSize->connectValueChanged(SLOT(slotAudioBufferSizeChanged(double)));
...
void VisualPlayPosition::getPlaySlipAt(int usFromNow, double* playPosition, double* slipPosition)
{
    ...
    if (m_valid) {
        auto data = m_data.getValue();
        auto usElapsed = data.m_referenceTime.elapsed().toIntegerMicros();
...
{
    auto vpp = QSharedPointer<VisualPlayPosition>(m_listVisualPlayPosition.value(group));
    if (vpp.isNull()) {
