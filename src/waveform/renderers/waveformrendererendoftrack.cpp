...
WaveformRendererEndOfTrack::~WaveformRendererEndOfTrack()
{
    if(m_pEndOfTrackControl) delete m_pEndOfTrackControl;
    if(m_pTrackSampleRate)   delete m_pTrackSampleRate;
    if(m_pPlayControl)       delete m_pPlayControl;
    if(m_pLoopControl)       delete m_pLoopControl;
}
...
void WaveformRendererEndOfTrack::draw(QPainter* painter, QPaintEvent* /*event*/)
{
    ...
    auto sampleRate = m_pTrackSampleRate->get();
    ...
    auto trackLength = 0.5 * trackSamples / sampleRate;
    ...
    auto dPlaypos = m_waveformRenderer->getPlayPos();
    auto remainingFrames = (1.0 - dPlaypos) * 0.5 * trackSamples;
    auto remainingTime = remainingFrames / sampleRate;
    ...
    auto elapsed = m_timer.elapsed() % m_blinkingPeriodMillis;
    auto blickIntensity = (double)(2 * abs(elapsed - m_blinkingPeriodMillis/2)) / m_blinkingPeriodMillis;
    auto criticalIntensity = (m_remainingTimeTriggerSeconds - remainingTime) / m_remainingTimeTriggerSeconds;
    painter->save();
    painter->resetTransform();
    painter->setOpacity(0.5 * blickIntensity);
    painter->setPen(m_pen);
    painter->drawRect(1, 1,m_waveformRenderer->getWidth() - 2, m_waveformRenderer->getHeight() - 2);
