...
WaveformSignalColors::WaveformSignalColors() = default;
...
bool WaveformSignalColors::setup(const QDomNode &node, const SkinContext& context)
{
    ...
    if (!m_rgbLowColor.isValid()) m_rgbLowColor = Qt::red;
    ...
    if (!m_rgbMidColor.isValid()) m_rgbMidColor = Qt::green;
    ...
    if (!m_rgbHighColor.isValid()) m_rgbHighColor = Qt::blue;
    ...
    if (!m_axesColor.isValid()) m_axesColor = QColor(245,245,245);
    ...
    if (!m_playPosColor.isValid()) m_playPosColor = m_axesColor;

    m_bgColor.setNamedColor(context.selectString(node, "BgColor"));
    if (!m_bgColor.isValid()) m_bgColor = QColor(0, 0, 0);
    m_bgColor = WSkinColor::getCorrectColor(m_bgColor).toRgb();

    auto filteredColorValid = m_lowColor.isValid() && m_midColor.isValid() && m_highColor.isValid();
    if (m_signalColor.isValid() && filteredColorValid) return true; //default
...
{
    qWarning() << "WaveformSignalColors::fallBackDefaultColor - skin do not provide valid signal colors ! Default colors is use ...";
...
QColor WaveformSignalColors::getSignalColor() const { return m_signalColor; }
QColor WaveformSignalColors::getLowColor() const { return m_lowColor; }
QColor WaveformSignalColors::getMidColor() const { return m_midColor; }
QColor WaveformSignalColors::getHighColor() const { return m_highColor; }
QColor WaveformSignalColors::getRgbLowColor() const { return m_rgbLowColor; }
QColor WaveformSignalColors::getRgbMidColor() const { return m_rgbMidColor; }
QColor WaveformSignalColors::getRgbHighColor() const { return m_rgbHighColor; }
QColor WaveformSignalColors::getAxesColor() const { return m_axesColor; }
QColor WaveformSignalColors::getPlayPosColor() const { return m_playPosColor; }
QColor WaveformSignalColors::getBgColor() const { return m_bgColor; }
WaveformSignalColors::~WaveformSignalColors() = default;
