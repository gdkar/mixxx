...
void WaveformMark::setup(const QString& group, const QDomNode& node,const SkinContext& context,const WaveformSignalColors& signalColors)
{
    ...
    } else  m_color = WSkinColor::getCorrectColor(m_color);
    ...
    if (markAlign.contains("bottom", Qt::CaseInsensitive)) m_align = Qt::AlignBottom;
    else m_align = Qt::AlignTop; // Default
    m_text = context.selectString(node, "Text");
    m_pixmapPath = context.selectString(node, "Pixmap");
    if (!m_pixmapPath.isEmpty()) m_pixmapPath = context.getSkinPath(m_pixmapPath);
}
