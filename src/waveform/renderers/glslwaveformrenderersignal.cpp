...
bool GLSLWaveformRendererSignal::loadShaders() {
    ...
    m_shadersValid = false;
    if (m_frameShaderProgram->isLinked()) { m_frameShaderProgram->release(); }
    m_frameShaderProgram->removeAllShaders();
    if (!m_frameShaderProgram->addShaderFromSourceFile(QGLShader::Vertex, ":shaders/passthrough.vert")) {
        qDebug() << "GLWaveformRendererSignalShader::loadShaders - "
    ...
    if (!m_frameShaderProgram->addShaderFromSourceFile(QGLShader::Fragment, fragmentShader)) {
...
bool GLSLWaveformRendererSignal::loadTexture() {
    ...
    if (m_textureId == 0) {
        ...
        if (error) qDebug() << "GLSLWaveformRendererSignal::loadTexture - m_textureId" << m_textureId << "error" << error;
    }
    glBindTexture(GL_TEXTURE_2D, m_textureId);
    int error = glGetError();
    if (error) qDebug() << "GLSLWaveformRendererSignal::loadTexture - bind error" << error;
...
void GLSLWaveformRendererSignal::createFrameBuffers()
{
    ...
    if (!m_framebuffer->isValid()) qWarning() << "GLSLWaveformRendererSignal::createFrameBuffer - frame buffer not valid";
...
bool GLSLWaveformRendererSignal::onInit() {
    m_loadedWaveform = 0;
    if (!m_frameShaderProgram) m_frameShaderProgram = new QGLShaderProgram();
...
void GLSLWaveformRendererSignal::onResize() { createFrameBuffers(); }
void GLSLWaveformRendererSignal::draw(QPainter* painter, QPaintEvent* /*event*/) {
    if (!m_frameBuffersValid || !m_shadersValid) { return;}
    auto trackInfo = m_waveformRenderer->getTrackInfo();
    if (!trackInfo) { return; }
    ...
    //paint into frame buffer
    {
        ...
        m_frameShaderProgram->setUniformValue("axesColor", QVector4D(m_axesColor_r, m_axesColor_g,m_axesColor_b, m_axesColor_a));
