...
WaveformWidgetRenderer::~WaveformWidgetRenderer()
{
    ...
    for (int i = 0; i < m_rendererStack.size(); ++i) delete m_rendererStack[i];
...
bool WaveformWidgetRenderer::init()
{
    ...
    {
        if (!m_rendererStack[i]->init()) return false;
    }
...
void WaveformWidgetRenderer::onPreRender(VSyncThread* vsyncThread)
{
    ...
    auto truePlayPos = m_visualPlayPosition->getAtNextVSync(vsyncThread);
...
void WaveformWidgetRenderer::setTrack(TrackPointer track) {
    ...
    m_trackSamples = -1.0;
    for (int i = 0; i < m_rendererStack.size(); ++i) {m_rendererStack[i]->onSetTrack();}
}
