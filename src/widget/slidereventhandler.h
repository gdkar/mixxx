...
class SliderEventHandler {
  public:
    ...
    void mouseMoveEvent(T* pWidget, QMouseEvent* e)
    {
        ...
        {
            if (m_bHorizontal) m_dPos = e->x() - m_dHandleLength / 2;
            else               m_dPos = e->y() - m_dHandleLength / 2;
            ...
            if (m_bEventWhileDrag) pWidget->setControlParameter(newParameter);
            // Update display
            pWidget->update();
        }
    }
    void mousePressEvent(T* pWidget, QMouseEvent* e)
    {
        ...
        else
        {
            ...
            {
                if (m_bHorizontal) m_dStartMousePos = e->x() - m_dHandleLength / 2;
                else               m_dStartMousePos = e->y() - m_dHandleLength / 2;
    ...
    void mouseReleaseEvent(T* pWidget, QMouseEvent* e)
    {
        ...
        if (e->button() == Qt::RightButton) m_bRightButtonPressed = false;
        else pWidget->setControlParameter(m_dOldParameter);
    }
    void wheelEvent(T* pWidget, QWheelEvent* e)
    {
        // For legacy reasons this is tuned to 127.
        auto wheelAdjustment = ((QWheelEvent *)e)->delta() / (120.0 * 127.0);
    ...
    {
        if (m_dSliderLength - m_dHandleLength <= 0.0) return 0.0;
        if (!m_bHorizontal)  parameter = 1.0 - parameter;
    ...
    double positionToParameter(double pos) const
    {
        ...
        if (!m_bHorizontal) return 1.0 - val;
        return val;
    }
  private:
