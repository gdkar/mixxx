...
void WEffectButtonParameter::setup(QDomNode node, const SkinContext& context) {
    ...
    EffectParameterSlotBasePointer pParameterSlot = EffectWidgetUtils::getButtonParameterSlotFromNode(node, context, pEffectSlot);
    if (pParameterSlot) {setEffectParameterSlot(pParameterSlot);}
    else {SKIN_WARNING(node, context) << "EffectButtonParameter node could not attach to effect parameter";}
}
