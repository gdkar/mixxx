...
class KnobEventHandler {
  public:
    ...
    auto valueFromMouseEvent(T* pWidget, QMouseEvent* e)
    {
        auto cur = e->globalPos();
        auto  diff = cur - m_startPos;
        auto dist = sqrt(static_cast<double>(diff.x() * diff.x() + diff.y() * diff.y()));
        auto y_dominant = abs(diff.y()) > abs(diff.x());
        ...
        if ((y_dominant && diff.y() > 0) || (!y_dominant && diff.x() < 0)) dist = -dist;
