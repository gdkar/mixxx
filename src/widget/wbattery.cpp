...
void WBattery::setup(const QDomNode& node, const SkinContext& context)
{
    ...
    auto chargedPath = context.selectElement(node, "PixmapCharged");
    ...
    auto numberStates = context.selectInt(node, "NumberStates");
    if (numberStates < 0) {
        numberStates = 0;
    }
    ...
    auto pixmapsCharging = context.selectElement(node, "PixmapsCharging");
    if (!pixmapsCharging.isNull()) {
        // TODO(XXX) inline SVG support via context.getPixmapSource.
        auto chargingPath = context.nodeToString(pixmapsCharging);
        auto mode = context.selectScaleMode(pixmapsCharging,Paintable::TILE);
        for (auto i = 0; i < m_chargingPixmaps.size(); ++i) {
            auto source = context.getPixmapSource(chargingPath.arg(i));
            setPixmap(&m_chargingPixmaps[i], source, mode);
        }
    }

    auto pixmapsDischarging = context.selectElement(node, "PixmapsDischarging");
    if (!pixmapsDischarging.isNull()) {
        // TODO(XXX) inline SVG support via context.getPixmapSource.
        auto dischargingPath = context.nodeToString(pixmapsDischarging);
        auto mode = context.selectScaleMode(pixmapsDischarging, Paintable::TILE);
        for (auto i = 0; i < m_dischargingPixmaps.size(); ++i) {
...
{
    auto minutesLeft = m_pBattery ? m_pBattery->getMinutesLeft() : 0;
    auto  chargingState = m_pBattery ?
            m_pBattery->getChargingState() : Battery::UNKNOWN;
    auto dPercentage = m_pBattery ? m_pBattery->getPercentage() : 0;
    ...
    switch (chargingState) {
        case Battery::CHARGING:
            if (!m_chargingPixmaps.isEmpty()) {
                m_pCurrentPixmap = m_chargingPixmaps[
                    pixmapIndexFromPercentage(dPercentage,m_chargingPixmaps.size())];
            }
            if (minutesLeft == -1) {
                setBaseTooltip(tr("Time until charged unknown."));
            } else {
                setBaseTooltip(tr("Time until charged: %1").arg(formatMinutes(minutesLeft)));
            }
            break;
        case Battery::DISCHARGING:
            ...
            if (minutesLeft == -1) {
                ...
                setBaseTooltip(tr("Time left: %1").arg(formatMinutes(minutesLeft)));
            }
            break;
