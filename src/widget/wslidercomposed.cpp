...
void WSliderComposed::setup(QDomNode node, const SkinContext& context)
{
    ...
    {
        if (context.selectString(node, "EventWhileDrag").contains("no")) m_handler.setEventWhileDrag(false);
    }
...
void WSliderComposed::setHandlePixmap(bool bHorizontal,PixmapSource sourceHandle,Paintable::DrawMode mode)
{
    ...
    if (!m_pHandle) qDebug() << "WSliderComposed: Error loading handle pixmap:" << sourceHandle.getPath();
    else
    {
...
void WSliderComposed::paintEvent(QPaintEvent *)
{
    ...
    if (!m_pSlider.isNull() && !m_pSlider->isNull()) m_pSlider->draw(rect(), &p);
...
void WSliderComposed::fillDebugTooltip(QStringList* debug)
{
    ...
    auto sliderLength = m_bHorizontal ? width() : height();
