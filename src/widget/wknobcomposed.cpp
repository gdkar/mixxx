...
void WKnobComposed::setup(QDomNode node, const SkinContext& context)
{
    ...
    if (context.hasNode(node, "BackPath"))
    {
        ...
        setPixmapBackground(context.getPixmapSource(backPathElement),context.selectScaleMode(backPathElement, Paintable::STRETCH));
    }
    // Set knob pixmap if available
    if (context.hasNode(node, "Knob"))
    {
        auto knobNode = context.selectElement(node, "Knob");
        setPixmapKnob(context.getPixmapSource(knobNode),context.selectScaleMode(knobNode, Paintable::STRETCH));
    }
    if (context.hasNode(node, "MinAngle")) m_dMinAngle = context.selectDouble(node, "MinAngle");
    if (context.hasNode(node, "MaxAngle")) m_dMaxAngle = context.selectDouble(node, "MaxAngle");
    if (context.hasNode(node, "KnobCenterXOffset")) m_dKnobCenterXOffset = context.selectDouble(node, "KnobCenterXOffset");
    if (context.hasNode(node, "KnobCenterYOffset")) m_dKnobCenterYOffset = context.selectDouble(node, "KnobCenterYOffset");
}
...
void WKnobComposed::setPixmapKnob(PixmapSource source,Paintable::DrawMode mode)
{
    ...
    if (m_pKnob.isNull() || m_pKnob->isNull()) {
        qDebug() << metaObject()->className() << "Error loading knob pixmap:" << source.getPath();
    }
}
void WKnobComposed::onConnectedControlChanged(double dParameter, double dValue)
{
    ...
    if (std::abs(angle - m_dCurrentAngle) > 0.01) update();
}
