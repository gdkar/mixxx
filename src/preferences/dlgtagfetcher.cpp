...
DlgTagFetcher::~DlgTagFetcher() = default;
void DlgTagFetcher::init()
{
    ...
    connect(results, SIGNAL(currentItemChanged(QTreeWidgetItem*,QTreeWidgetItem*)),this, SLOT(resultSelected()));

    connect(&m_TagFetcher, SIGNAL(resultAvailable(const TrackPointer,const QList<TrackPointer>&)),this, SLOT(fetchTagFinished(const TrackPointer,const QList<TrackPointer>&)));
    connect(&m_TagFetcher, SIGNAL(fetchProgress(QString)),this, SLOT(fetchTagProgress(QString)));
    connect(&m_TagFetcher, SIGNAL(networkError(int, QString)),this, SLOT(slotNetworkError(int, QString)));
...
void DlgTagFetcher::loadTrack(const TrackPointer track)
{
    ...
    connect(track.data(), SIGNAL(changed(TrackInfoObject*)),this, SLOT(updateTrackMetadata(TrackInfoObject*)));
    updateStack();
}
...
void DlgTagFetcher::apply()
{
    ...
    if (resultIndex > -1) {
        if (!m_data.m_results[resultIndex]->getAlbum().isEmpty())  m_track->setAlbum(m_data.m_results[resultIndex]->getAlbum());
        if (!m_data.m_results[resultIndex]->getArtist().isEmpty()) m_track->setArtist(m_data.m_results[resultIndex]->getArtist());
        if (!m_data.m_results[resultIndex]->getTitle().isEmpty())  m_track->setTitle(m_data.m_results[resultIndex]->getTitle());
        if (!m_data.m_results[resultIndex]->getYear().isEmpty() && m_data.m_results[resultIndex]->getYear() != "0")
            m_track->setYear(m_data.m_results[resultIndex]->getYear());
        if (!m_data.m_results[resultIndex]->getTrackNumber().isEmpty() && m_data.m_results[resultIndex]->getTrackNumber() != "0")
...
void DlgTagFetcher::fetchTagFinished(const TrackPointer track, const QList<TrackPointer>& tracks)
{
    // check if the answer is for this track
    if (m_track->getLocation() != track->getLocation()) return;
...
void DlgTagFetcher::slotNetworkError(int errorCode, QString app)
{
    ...
    m_data.m_pending = false;
    auto httpStatusMessage = tr("HTTP Status: %1");
    httpStatus->setText(httpStatusMessage.arg(errorCode));
    auto unknownError = tr("Mixxx can't connect to %1 for an unknown reason.");
    cantConnectMessage->setText(unknownError.arg(app));
    auto cantConnect = tr("Mixxx can't connect to %1.");
...
void DlgTagFetcher::updateStack()
{
    ...
    for (auto  i=0 ; i<results->model()->rowCount() ; ++i) {
        auto index = results->model()->index(i, 0);
        auto id = index.data(Qt::UserRole);
...
void DlgTagFetcher::addTrack(const TrackPointer track, int resultIndex,
    ...
    auto item = new QTreeWidgetItem(parent, values);
...
{
  if (!results->currentItem()) return;
  auto resultIndex = results->currentItem()->data(0, Qt::UserRole).toInt();
