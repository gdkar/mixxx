...
DlgPrefLibrary::DlgPrefLibrary(QWidget * parent,
    ...
    connect(this, SIGNAL(requestAddDir(QString)),m_pLibrary, SLOT(slotRequestAddDir(QString)));
    connect(this, SIGNAL(requestRemoveDir(QString, Library::RemovalType)),m_pLibrary, SLOT(slotRequestRemoveDir(QString, Library::RemovalType)));
    connect(this, SIGNAL(requestRelocateDir(QString,QString)),m_pLibrary, SLOT(slotRequestRelocateDir(QString,QString)));
    connect(PushButtonAddDir, SIGNAL(clicked()),this, SLOT(slotAddDir()));
    connect(PushButtonRemoveDir, SIGNAL(clicked()),this, SLOT(slotRemoveDir()));
    connect(PushButtonRelocateDir, SIGNAL(clicked()),this, SLOT(slotRelocateDir()));
    ...
    auto plugins(SoundSourceProxy::getSupportedFileExtensionsByPlugins());
    if (plugins.length() > 0) {pluginsLabel->setText(plugins.join(", "));}
    // Set default direction as stored in config file
    auto rowHeight = m_pLibrary->getTrackTableRowHeight();
...
void DlgPrefLibrary::initialiseDirList() {
    ...
    auto selected = dirList->currentIndex().data().toString();
...
void DlgPrefLibrary::slotRemoveDir() {
    auto index = dirList->currentIndex();
    auto fd = index.data().toString();
    ...
    auto  cancelButton =removeMsgBox.addButton(QMessageBox::Cancel);
    auto hideAllButton = removeMsgBox.addButton(tr("Hide Tracks"), QMessageBox::AcceptRole);
    auto deleteAllButton = removeMsgBox.addButton(tr("Delete Track Metadata"), QMessageBox::AcceptRole);
    auto leaveUnchangedButton = removeMsgBox.addButton(tr("Leave Tracks Unchanged"), QMessageBox::AcceptRole);
    removeMsgBox.setDefaultButton(cancelButton);
    removeMsgBox.exec();
    if (removeMsgBox.clickedButton() == cancelButton) {return;}
    auto deleteAll = removeMsgBox.clickedButton() == deleteAllButton;
    auto hideAll = removeMsgBox.clickedButton() == hideAllButton;
    auto leaveUnchanged = removeMsgBox.clickedButton() == leaveUnchangedButton;
    auto removalType = Library::LeaveTracksUnchanged;
    if (leaveUnchanged) {
...
void DlgPrefLibrary::slotRelocateDir() {
    auto index = dirList->currentIndex();
    auto currentFd = index.data().toString();
    ...
    auto fd = QFileDialog::getExistingDirectory(this, tr("Relink music directory to new location"), startDir);
    if (!fd.isEmpty()) {
...
void DlgPrefLibrary::slotRowHeightValueChanged(int height) {emit(setTrackTableRowHeight(height));}
...
void DlgPrefLibrary::slotSelectFont() {
    ...
    auto ok = false;
    auto font = QFontDialog::getFont(&ok, m_pLibrary->getTrackTableFont(),this, tr("Select Library Font"));
