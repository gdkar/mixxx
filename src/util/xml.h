...
class XmlParse {
  public:
    ...
    static QDomNode selectNode(QDomNode nodeHeader,QString sNode);
    ...
    static QDomElement selectElement(QDomNode nodeHeader,QString sNode);
    ...
    static int selectNodeInt(QDomNode nodeHeader,QString sNode);
    // Searches for an element named sNode in the children of nodeHeader and
    // parses the text value of its children as a float. Returns 0.0f if sNode
    // is not found in nodeHeader's children.
    static float selectNodeFloat(QDomNode nodeHeader,QString sNode);
    // Searches for an element named sNode in the children of nodeHeader and
    // returns the text value of its children. Returns the empty string if sNode
    // is not found in nodeHeader's children.
    static QString selectNodeQString(QDomNode nodeHeader,QString sNode);
    // Open an XML file.
    ...
    static QDomElement addElement(QDomDocument& document, QDomElement& header,QString elementName, QString textValue);
  private:
    XmlParse() = delete;
};
