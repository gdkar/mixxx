...
StatsPipe::~StatsPipe() {
    if (m_pManager) {m_pManager->onStatsPipeDestroyed(this);}
}
...
void StatsManager::writeTimeline(const QString& filename) {
    ...
    // Sort by time.
    std::sort(m_events.begin(), m_events.end(), [](const Event&e1,const Event&e2)->bool{return e1.m_time<e2.m_time;});
    qint64 last_time = m_events.front().m_time;
...
void StatsManager::processIncomingStatReports() {
    ...
    for(auto &pStatsPipe: m_statsPipes) {
        while (pStatsPipe->read(&report, 1) == 1) {
            Stat& info = m_stats[report.tag];
            info.m_tag = report.tag;
            ...
            if (report.compute & Stat::STATS_EXPERIMENT) {
                Stat& experiment = m_experimentStats[report.tag];
                experiment.m_tag = report.tag;
                ...
                Stat& base = m_baseStats[report.tag];
                base.m_tag = report.tag;
...
//        m_statsPipeLock.unlock();
        if (m_emitAllStats.load()) {
            for ( auto & stat : m_stats){emit(statUpdated(stat));}
            m_emitAllStats = 0;
        }
        if (m_quit.load()) {
