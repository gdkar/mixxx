...
class parented_ptr {
  public:
    constexpr explicit parented_ptr(T* t)
    : m_pObject(t)
    {
        DEBUG_ASSERT(t->parent());
    }
    ...
    constexpr parented_ptr() : m_pObject(nullptr) {}
   ...
   ~parented_ptr() = default;
    ...
    constexpr bool operator== (const parented_ptr& other) const
    {
     ...
     */
    template <typename U>
    constexpr typename std::enable_if<std::is_convertible<U*, T*>::value, parented_ptr<T>&>::type
...
parented_ptr<T> make_parented(Args&&... args) {
    return parented_ptr<T>(new T(std::forward<Args>(args)...));
}

// Comparison operator definitions
template<typename T, typename U>
constexpr bool operator== (const T* lhs, const parented_ptr<U>& rhs) {
    return lhs == rhs.get();
}

template<typename T, typename U>
constexpr bool operator== (const parented_ptr<T>& lhs, const U* rhs) {
    return lhs.get() == rhs;
}

template<typename T, typename U>
constexpr bool operator== (const parented_ptr<T>& lhs, const parented_ptr<U>& rhs) const {
