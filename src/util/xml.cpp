...
QDomNode XmlParse::selectNode(const QDomNode& nodeHeader,const QString& sNode)
{
    ...
    while (!node.isNull())
    {
        if (node.nodeName() == sNode) return node;
...
QDomElement XmlParse::selectElement(const QDomNode& nodeHeader,const QString& sNode) {
    ...
    while (!node.isNull())
    {
        ...
        {
            if (node.isElement()) return node.toElement();
            else break;
        }
        node = node.nextSibling();
    }
...
QDomElement XmlParse::openXMLFile(const QString& path, const QString& name)
{
    ...
    if (!doc.setContent(&file, &error, &line, &col)) {
        auto errorString = QString("%1 at line %2, column %3") .arg(error).arg(line).arg(col);
        auto errorLog = QString("Error parsing XML file %1: %2").arg(file.fileName(), errorString);
        qWarning() << errorLog;
        // Set up error dialog
