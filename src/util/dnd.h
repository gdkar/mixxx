...
class DragAndDropHelper {
  public:
    ...
    static bool allowLoadToPlayer(const QString& group,ConfigObject<ConfigValue>* pConfig) {
        return allowLoadToPlayer(group, ControlObject::get(ConfigKey(group, "play")) > 0.0,pConfig);
    }
    ...
    static QList<QFileInfo> dropEventFiles(const QMimeData& mimeData,
                                           ...
                                           bool firstOnly,
                                           bool acceptPlaylists) {
        if (!mimeData.hasUrls() || (mimeData.hasText() && mimeData.text() == sourceIdentifier)) 
        {return QList<QFileInfo>();}
        QList<QFileInfo> files = DragAndDropHelper::supportedTracksFromUrls( mimeData.urls(), firstOnly, acceptPlaylists);
        return files;
    }
  ...
  private:
    static QDrag* dragUrls(const QList<QUrl>& locationUrls,
        ...
        if (locationUrls.isEmpty()) {return NULL;}
    ...
    static bool addFileToList(const QString& file, QList<QFileInfo>* files) {
        ...
        // filetypes, etc.)
        if (!SoundSourceProxy::isFilenameSupported(fileInfo.fileName())) {return false;}
