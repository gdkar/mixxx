...
struct avpacket {
    ...
    void swap(avpacket &o) noexcept { std::swap(m_d, o.m_d);}
    void unref() { av_packet_unref(m_d);}
    void ref(AVPacket *pkt)
    {
        if(pkt != m_d) {
            unref();
...
struct format_context{
   ...
   ~format_context() {
    ...
    std::pair<avpacket,int> read_frame() { auto p = std::make_pair(avpacket{},0); if((p.second = read_frame(p.first)) < 0) { p.first.unref();} return p;}
    ...
    void dump(int offset, const char *filename) const {
        av_dump_format(m_d, offset, filename, is_output());
    }
};
