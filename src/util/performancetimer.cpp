...
void PerformanceTimer::start(){t1 = mach_absolute_time();}
...
static qint64 getTimeFromTick(qint64 elapsed){
    static TInt freq = 0;
    if (!freq) HAL::Get(HALData::EFastCounterFrequency, freq);
    return (elapsed * 1000000000) / freq;
}

void PerformanceTimer::start(){t1 = User::FastCounter();}
...
#if (_POSIX_MONOTONIC_CLOCK-0 == 0)
    if (is_likely(load_acquire(monotonicClockChecked))) return;
