...
DlgDeveloperTools::DlgDeveloperTools(QWidget* pParent,
    ...
    QHash<ConfigKey, ConfigKey> controlAliases = ControlDoublePrivate::getControlAliases();
    for (QList<QSharedPointer<ControlDoublePrivate> >::const_iterator it = controlsList.begin();it != controlsList.end(); ++it) {
    ...
    if (!m_logFile.open(QIODevice::ReadOnly | QIODevice::Text)) {qDebug() << "ERROR: Could not open log file";}
...
void DlgDeveloperTools::slotControlSearch(const QString& search) {m_controlProxyModel.setFilterFixedString(search);}
void DlgDeveloperTools::slotControlSearchClear() {m_controlProxyModel.setFilterFixedString(QString());}
