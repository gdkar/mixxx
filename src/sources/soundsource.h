...
struct SoundSourceConfig {
    SINT channelCountHint = 2;
    SINT frameRateHint    = 44100;
};
...
public:
    ...
    virtual bool parseTrackMetadataAndCoverArt(TrackMetadata*, QImage*) const ;
    virtual bool open(SoundSourceConfig audioSrcCfg = SoundSourceConfig());
    virtual void close() = 0;
    explicit SoundSource(QUrl url);
    explicit SoundSource(QUrl url,QString type);

protected:
    virtual bool tryOpen(SoundSourceConfig ) = 0;
    bool isLocalFile() const;
    QString getLocalFileName() const;
    QByteArray getLocalFileNameBytes() const;
