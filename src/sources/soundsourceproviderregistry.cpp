...
SoundSourceProviderPointer SoundSourceProviderRegistry::registerEntry(const Entry& entry) {
    ...
    const QStringList supportedFileExtensions( entry.pProvider->getSupportedFileExtensions());
...
void SoundSourceProviderRegistry::finishRegistration() {
    ...
    const QString fileExtensionsRegex( RegexUtils::fileExtensionsRegex(supportedFileExtensions));
