...
bool SoundManagerConfig::readFromDisk() {
    ...
    for (int i = 0; i < devElements.count(); ++i) {
        ...
        auto outElements = (devElement.elementsByTagName("output"));
        auto inElements = (devElement.elementsByTagName("input"));
        for (int j = 0; j < outElements.count(); ++j) {
            auto  outElement = (outElements.at(j).toElement());
            if (outElement.isNull()) continue;
            auto  out = AudioOutput::fromXML(outElement);
            if (out.getType() == AudioPath::INVALID) continue;
            auto dupe = (false);
            for(auto otherOut: m_outputs) {
                if (out == otherOut && out.getChannelGroup() == otherOut.getChannelGroup()) {
                    dupe = true;
                    break;
                }
            }
        ...
        for (int j = 0; j < inElements.count(); ++j) {
            auto  inElement = (inElements.at(j).toElement());
            if (inElement.isNull()) continue;
            auto in = (AudioInput::fromXML(inElement));
            if (in.getType() == AudioPath::INVALID) continue;
            auto  dupe = (false);
            for(auto otherIn: m_inputs) {
                if (in == otherIn && in.getChannelGroup() == otherIn.getChannelGroup()) {
                    dupe = true;
                    break;
                }
            }
...
bool SoundManagerConfig::checkAPI(const SoundManager &soundManager) {
    if (!soundManager.getHostAPIList().contains(m_api) && m_api != "None") { return false; }
    return true;
}
unsigned int SoundManagerConfig::getSampleRate() const { return m_sampleRate; }
...
bool SoundManagerConfig::checkSampleRate(const SoundManager &soundManager) {
    if (!soundManager.getSampleRates(m_api).contains(m_sampleRate)) { return false; }
    return true;
}
...
void SoundManagerConfig::clearInputs() {m_inputs.clear(); }
/**
...
void SoundManagerConfig::filterOutputs(SoundManager *soundManager) {
    ...
    for(auto del: toDelete) { m_outputs.remove(del); }
}
/**
...
void SoundManagerConfig::loadDefaults(SoundManager *soundManager, unsigned int flags) {
    if (flags & SoundManagerConfig::API) {
        ...
        if (!apiList.isEmpty()) {
            ...
            if (apiList.contains(MIXXX_PORTAUDIO_JACK_STRING)) { m_api = MIXXX_PORTAUDIO_JACK_STRING;}
            else { m_api = MIXXX_PORTAUDIO_ALSA_STRING; }
#endif
#ifdef __WINDOWS__
            ...
            if (apiList.contains(MIXXX_PORTAUDIO_ASIO_STRING) && !soundManager->getDeviceList( MIXXX_PORTAUDIO_ASIO_STRING, true, false).isEmpty()) {
                m_api = MIXXX_PORTAUDIO_ASIO_STRING;
            } else { m_api = MIXXX_PORTAUDIO_DIRECTSOUND_STRING; }
#endif
#ifdef __APPLE__
    ...
    auto defaultSampleRate = kFallbackSampleRate;
    ...
    if (flags & SoundManagerConfig::OTHER) {
        ...
        if (sampleRates.contains(defaultSampleRate)) { m_sampleRate = defaultSampleRate;
        } else if (sampleRates.contains(kFallbackSampleRate)) { m_sampleRate = kFallbackSampleRate;
        } else if (!sampleRates.isEmpty()) { m_sampleRate = sampleRates.first();
        } else {
