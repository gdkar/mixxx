...
void ControlEffectKnob::setBehaviour(EffectManifestParameter::ControlHint type,double dMinValue, double dMaxValue)
{
    ...
    else if (type == EffectManifestParameter::CONTROL_KNOB_LOGARITHMIC)
    {
        if (dMinValue == 0)
        {
            if (dMaxValue == 1.0) m_pControl->setBehavior(new ControlAudioTaperPotBehavior(-20, 0, 1));
            else if (dMaxValue > 1.0) m_pControl->setBehavior(new ControlAudioTaperPotBehavior(-12, ratio2db(dMaxValue), 0.5));
            else m_pControl->setBehavior(new ControlLogPotmeterBehavior(dMinValue, dMaxValue, -40));
        }
        else m_pControl->setBehavior(new ControlLogPotmeterBehavior(dMinValue, dMaxValue, -40));
    }
}
