...
void ControlObjectSlave::initialize(const ConfigKey& key)
{
    ...
    if ( m_pControl )
    {
      ...
      connect(m_pControl.data(),&ControlDoublePrivate::valueChanged,this,&ControlObjectSlave::valueChanged,flags);
      connect(m_pControl.data(),&ControlDoublePrivate::nameChanged,this,&ControlObjectSlave::nameChanged,flags);
      connect(m_pControl.data(),&ControlDoublePrivate::descriptionChanged,this,&ControlObjectSlave::descriptionChanged,flags);
      connect(m_pControl.data(),&ControlDoublePrivate::parameterChanged,this,&ControlObjectSlave::parameterChanged,flags);
    }
}
...
const ConfigKey&ControlObjectSlave::getKey()const
{
  return m_key;
}
...
QString ControlObjectSlave::name() const
{
  if(m_pControl) return m_pControl->name(); else return QString{};
}
QString ControlObjectSlave::description() const
{
  if(m_pControl) return m_pControl->description(); else return QString{};
}
QString ControlObjectSlave::group() const
{
  return m_key.group;
}
QString ControlObjectSlave::item() const
{
  return m_key.item;
}
