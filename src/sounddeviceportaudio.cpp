...
namespace /* anonymous */ {
  ...
  int paV19CallbackDrift(const void *inputBuffer, void *outputBuffer,
                    ...
                    PaStreamCallbackFlags statusFlags,
                    void *soundDevice) {
      return ((SoundDevicePortAudio*)soundDevice)->callbackProcessDrift(framesPerBuffer,
              (CSAMPLE*)outputBuffer, (const CSAMPLE*)inputBuffer, timeInfo, statusFlags);
  }
  ...
  int paV19CallbackClkRef(const void *inputBuffer, void *outputBuffer,
                    ...
                    PaStreamCallbackFlags statusFlags,
                    void *soundDevice) {
      return ((SoundDevicePortAudio*)soundDevice)->callbackProcessClkRef(framesPerBuffer,
              (CSAMPLE*)outputBuffer, (const CSAMPLE*)inputBuffer, timeInfo, statusFlags);
  }
  const int kDriftReserve = 2; // Buffer for drift correction 1 full, 1 for r/w, 1 empty
  const int kFifoSize = 2 * kDriftReserve + 1; // Buffer for drift correction 1 full, 1 for r/w, 1 empty
}
// static
std::atomic<int> SoundDevicePortAudio::m_underflowHappened{0};
...
bool SoundDevicePortAudio::open(bool isClkRefDevice, int syncBuffers)
{
    ...
    auto bufferMSec = m_framesPerBuffer * 1e3 / m_dSampleRate;
    ...
    m_outputParams.suggestedLatency = m_framesPerBuffer / m_dSampleRate;
    m_outputParams.hostApiSpecificStreamInfo = nullptr;

    m_inputParams.device  = m_devId;
    m_inputParams.sampleFormat  = paFloat32;
    m_inputParams.suggestedLatency = m_framesPerBuffer / m_dSampleRate;
    ...
    {
        m_outputFifo = std::make_unique<FIFO<CSAMPLE> >(m_outputParams.channelCount * m_framesPerBuffer * kFifoSize);
        m_outputFifo->releaseWriteRegions(m_outputParams.channelCount * m_framesPerBuffer * kFifoSize / 2);
    }
    if (m_inputParams.channelCount)
    {
        m_inputFifo = std::make_unique< FIFO<CSAMPLE> >(m_inputParams.channelCount * m_framesPerBuffer * kFifoSize);
...
void SoundDevicePortAudio::readProcess( size_t framesPerBuffer)
{
    if (m_inputParams.channelCount && m_inputFifo)
    {
        ...
        auto readCount =    std::min<size_t >(readAvailable,inChunkSize);
        if( readCount < inChunkSize) m_underflowHappened = 1;
        if (readCount)
        {
            ...
            if(size1 > 0)composeInputBuffer(dataPtr1,size1 / m_inputParams.channelCount, 0,m_inputParams.channelCount);
            if(size2 > 0)composeInputBuffer(dataPtr2,size2 / m_inputParams.channelCount, size1 / m_inputParams.channelCount,m_inputParams.channelCount);
...
void SoundDevicePortAudio::writeProcess( size_t framesPerBuffer)
{
    if (m_outputParams.channelCount && m_outputFifo)
    {
        ...
        auto writeCount = std::min<size_t >(outChunkSize,writeAvailable);
        if ( writeCount < outChunkSize ) m_underflowHappened = 1;
        if (writeCount)
        {
            auto dataPtr1 = static_cast<CSAMPLE*>(nullptr);
            auto size1 = PaUtilRingBuffer<CSAMPLE>::size_type{0};
            ...
            // Fetch fresh samples and write to the the output buffer
            if (size1 > 0) composeOutputBuffer(dataPtr1, size1 / m_outputParams.channelCount, 0, static_cast<int>(m_outputParams.channelCount));
            if (size2 > 0) composeOutputBuffer(dataPtr2, size2 / m_outputParams.channelCount, size1 / m_outputParams.channelCount, static_cast<int>(m_outputParams.channelCount));
            m_outputFifo->releaseWriteRegions(writeCount);
        }
    }
}
int SoundDevicePortAudio::callbackProcessDrift(size_t  framesPerBuffer,
                                          ...
                                          PaStreamCallbackFlags statusFlags)
{
    ...
    if (statusFlags & (paOutputUnderflow | paInputOverflow)) m_underflowHappened = 1;
    ...
    if (m_inputParams.channelCount && !(statusFlags & paInputUnderflow))
    {
        auto inChunkSize    = framesPerBuffer * m_inputParams.channelCount;
        auto readAvailable  = m_inputFifo->readAvailable();
        ...
        } else m_underflowHappened = 1;
    }
    if (m_outputParams.channelCount && !(statusFlags & paOutputOverflow))
    {
        auto outChunkSize = framesPerBuffer * m_outputParams.channelCount;
...
int SoundDevicePortAudio::callbackProcess(size_t framesPerBuffer,
                                          CSAMPLE *out, const CSAMPLE *in,
                                          const PaStreamCallbackTimeInfo *timeInfo,
                                          PaStreamCallbackFlags statusFlags)
{
    ...
    if (m_inputParams.channelCount && !(statusFlags & paInputUnderflow))
    {
        auto inChunkSize = framesPerBuffer * m_inputParams.channelCount;
        auto writeAvailable = m_inputFifo->writeAvailable();
        if (writeAvailable >= inChunkSize) m_inputFifo->write(in, inChunkSize - m_inputParams.channelCount);
        else if (writeAvailable)
        {
            // Fifo Overflow
            m_inputFifo->write(in, writeAvailable);
    ...
    if (m_outputParams.channelCount && !(statusFlags&paOutputOverflow))
    {
        auto outChunkSize = framesPerBuffer * m_outputParams.channelCount;
        auto readAvailable = m_outputFifo->readAvailable();
        if (readAvailable >= outChunkSize) m_outputFifo->read(out, outChunkSize);
        else if (readAvailable)
        {
            m_outputFifo->read(out,readAvailable);
            // underflow
...
int SoundDevicePortAudio::callbackProcessClkRef(size_t framesPerBuffer,
                                                CSAMPLE *out, const CSAMPLE *in,
                                                const PaStreamCallbackTimeInfo *timeInfo,
                                                PaStreamCallbackFlags statusFlags)
{
    ...
    if (!m_bSetThreadPriority )
    {
        ...
        else qDebug() << "SSE: Denormals to zero mode already enabled";
        if (!_MM_GET_FLUSH_ZERO_MODE())
        {
             qDebug() << "SSE: Enabling flush to zero mode";
             _MM_SET_FLUSH_ZERO_MODE(_MM_FLUSH_ZERO_ON);
        }
        else qDebug() << "SSE: Flush to zero mode already enabled";
#endif
    }
    VisualPlayPosition::setTimeInfo(timeInfo);
    if (statusFlags & (paOutputUnderflow | paInputOverflow)) m_underflowHappened = true;
    if (m_underflowUpdateCount == 0)
    {
        if (auto happened = m_underflowHappened.exchange(0))
        {
            m_pMasterAudioLatencyOverload->set(1.0);
            m_pMasterAudioLatencyOverloadCount->set(m_pMasterAudioLatencyOverloadCount->get() + happened);
    ...
    {
        ScopedTimer t("SoundDevicePortAudio::callback, input %1", getInternalName());
        m_pSoundManager->readProcess( framesPerBuffer);
    }
    {
        ScopedTimer t("SoundDevicePortAudio::callback, prepare %1", getInternalName());
        m_pSoundManager->onDeviceOutputCallback( framesPerBuffer );
    }
    {
        ScopedTimer t("SoundDevicePortAudio::callback, writeProcess%1", getInternalName());
